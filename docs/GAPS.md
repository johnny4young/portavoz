...
longer the release target. The finite implementation scope is UI-test runtime
qualification followed by APT-1 reliability, APT-2 manual Ask, APT-3 explicit
meeting/library/web source policy, APT-4 cited and consented web, APT-5
interviews, APT-6 notes, and APT-7 bounded proactive assistance. Missing field
evidence is not a reason to add a substitute feature, and a green local test
cannot fill a physical or distribution cell.

| Classification | 1.0.0 treatment | Measurable exit |
|---|---|---|
| **Release blocker — finite product scope** | Complete UI-OPT and APT-1…APT-7 in order; every slice keeps explicit source/evidence policy, bounded work, cancellation, typed unavailable/insufficient states, and no autonomous external mutation. | Every slice has adversarial preflight, deterministic package/tooling evidence, minimum-safe real-app XCUITest, a local Conventional Commit, and updated durable truth. No optional workflow silently enters the sequence. |
| **Release blocker — autonomous realistic validation** | Routine QA must not require private user meetings. Use public/synthetic bilingual meeting/interview/note ground truth, deterministic local web fixtures including hostile prompt injection, installed-model opt-in lanes with content-free receipts, and cancellation/offline/relaunch/stress/no-crash coverage. | The exact frozen corpora, budgets, and complete EN/ES seed-demo journeys pass; irreducible permissions, accounts, physical accessibility, and real-field cells remain explicitly external. |
| **Release blocker — source integration** | The current candidate must not remain a local-only branch. | The exact candidate SHA has reviewed integration and hosted CI evidence. Push, PR, or merge requires explicit user authorization. |
| **Release blocker — deterministic qualification** | Re-run the canonical release receipt, required local model gate, authoritative release-host performance ledger, repository hygiene, and complete bilingual XCUITest on the frozen SHA. | Every command passes and every receipt names the exact version, build, and commit; skips remain only the explicitly reviewed environment/model set. Scoped feature evidence does not replace the complete bilingual integration gate. |
...
| **Release blocker — production sync truth** | Sync cannot be publicly enabled from local code evidence alone. | Production container/profile/account and the two-clean-Mac convergence matrix pass; otherwise the candidate must keep Sync unavailable and make no enabled-sync claim. |
| **Release blocker — final admission** | Adjacent green evidence and prior-version receipts do not count. | The fail-closed D147 scorecard reports **PASS** for the exact version, build, commit, and artifact digest. |
...
| **Post-1.0 backlog** | Quick Look, iOS/companion work, user-authored actions, standing rules, visual graph, broader graph CLI/MCP/sync/export, and alternate retrieval/ASR authorities. | Re-enter through a separately approved evidence-backed plan after 1.0.0 qualification. |

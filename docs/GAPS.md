...
| T11 | Post-recording workflow is only partially durable | **Slices 1A–1D-b2 plus the 1D-b2b control-plane/executor units:** capture persists the shell/assets first, writes crash-readable staging CAFs, validates checksum/media/health evidence, publishes without overwrite, and installs captured assets plus live content in one Unit of Work. StorageKit has idempotent owner-leased jobs; process launch recovers expired leases and reconciles interrupted meeting/staging/final evidence conservatively across both audio roots. Diarization and summary completion atomically fence source revisions and commit artifact + job success + dependent enqueue. A process-scoped supervisor now resumes already-enqueued supported jobs with exact operation fingerprints, heartbeats, bounded retries, optional-summary degradation, and one scheduled wake instead of polling. The normal Stop path still runs post-capture work synchronously and does not produce the initial durable job | Final Band 1 slice 1D-b2b Stop enqueue/kick cutover with Shortcut parity |

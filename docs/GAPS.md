...
| T28 | Transcript corrections do not yet reach every search/intelligence surface | **PARTIAL (D225/D229–D235)**: Meeting Detail composes current-revision text, speaker, split, explicit adjacent merge, and suppress corrections over immutable accepted evidence; derived artifacts are fenced by effective correction lineage; Markdown, PDF, SRT, WebVTT, CLI, and Gist share one composed projection with original timing; private sync converges compatible histories and fences conflicts. **PARTIAL (D313)**: an active `replaceText` correction serves its corrected text through the per-segment `segmentCorrectedText` FTS projection (v33, transactionally refreshed and backfilled on upgrade), a speaker-only correction no longer hides its line from search, and MCP appends the frozen-prefix `portavoz-reading/2` tools (`get_transcript_v2` composed and fail-closed, `get_summary_v2`/`get_action_items_v2` with correction provenance). **PARTIAL (D329)**: meeting-level Spotlight now reuses the revision-fenced text lane, excludes active structural targets, restores accepted rows on undo, and admits only summaries matching sparse v36 correction lineage; malformed or missing lineage fails closed and successful correction writes wake index reconciliation. **Still open:** split/merge/suppress composed content stays out of search and Spotlight (no shared structural identity), semantic search serves no text-replaced segment (stored embeddings describe the original text), and Apuntador is not automatically regenerated | Decide whether split/merge/suppress content earns a shared search identity story or stays excluded by design; define corrected-text re-embedding for the semantic lane; add explicit Apuntador regeneration policy only after source/evidence semantics are characterized |

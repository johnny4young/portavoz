...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2M established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, and expired-lease/evidence-first launch recovery behind characterized ports; and gave each Library window one tested `LibraryModel` state/action/effect owner. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, StorageKit-shaped Library projections, and global `libraryVersion` orchestration remain in the app** | **Next:** replace the Library's broad reload trigger and pull reads with query-specific read models plus scoped GRDB observations; retire other `libraryVersion` consumers only in their own characterized slices |

...
| 10 | Native App Intents and App Entities ✅ / entity Spotlight + Quick Look remain absent | **Start field-verified Jul 27; Stop and bounded entities implemented Aug 9, 2026.** The SDK-only shipping source now extracts exactly five actions, three entities, and three queries: Start/Stop plus exact open actions for meeting, canonical person, and confirmed commitment (D139/D140/D324/D325). Tahoe+ uses immediate foreground modes for recording actions while the compatibility property preserves Sequoia. Entity lookups are bounded, literal, database-backed, and route inside the chosen process with explicit recovery; exact Radar focus is visible and reversible. Stable, Dev, and UI-test identities remain distinct. Start's user-created Shortcut was field-verified from Shortcuts, Spotlight, and Siri. Stop and entity actions are locally covered through metadata, unit tests, and bilingual real-app handoff, but physical picker/search, Siri disambiguation, cold recovery, and registration still need Sequoia and Tahoe evidence. `IndexedEntity` is availability-gated on macOS 15+, but people/commitments are not yet published through protected Core Spotlight; D141 still omits the unsupported duplicate `AppShortcutsProvider`. The existing protected meeting-document index is separate | Add protected entity-native Spotlight publication only with reconciliation/privacy evidence; collect Stop/entity field evidence on physical Sequoia/Tahoe Macs. Quick Look genuinely needs an extension target and stays planned | D139–D141, D324–D325; AUTO-3c / QL M14a/M16 |

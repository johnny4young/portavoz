...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2S established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and every characterized local product/read policy behind inward contracts; gave Library, Insights, and each selected Meeting Detail tested feature-state owners; and replaced all three broad read reloads with storage-independent contracts plus query-scoped GRDB observations. Meeting Detail now observes transcript/cast, newest immutable summary/actions, and Companion independently. `UpcomingEvent` is a Core value while EventKit/RAG/external adapters remain in IntegrationsKit. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, direct Meeting Detail Store mutations, and Spotlight's compatibility counter remain in the app composition graph** | **Next:** route Meeting Detail title/speaker/action-item/Companion mutations through its model client; defer incremental Spotlight indexing/outbox work to Band 4 |

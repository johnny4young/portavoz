...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2N established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, and expired-lease/evidence-first launch recovery behind characterized ports; gave each Library window one tested `LibraryModel` state/action/effect owner; and replaced its broad reload with storage-independent read contracts plus scoped meeting/voice-mix, open-item, trash, and FTS observations. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, the Detail/Insights/Spotlight `libraryVersion` consumers, and pure product policies inside IntegrationsKit remain in the app composition graph** | **Next:** move the characterized meeting-review policy cluster out of IntegrationsKit; retire remaining broad invalidation consumers only in their own parity slices |

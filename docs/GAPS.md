...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2D established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, and moved Meeting Detail summary regeneration behind storage/preferences/provider ports. Import, refine, recording, and global `libraryVersion` orchestration remain in the app** | **Next after T16:** extract audio import with rollback/language/engine-release parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |
| T16 | A generated custom summary structure can disappear from Meeting Detail after broad reload | Regeneration passes and persists the selected `Recipe`, but D25 lookup and `reload()` still request the default `general` recipe, so a saved custom snapshot can be followed by the older general snapshot on screen | **Next:** make lookup recipe-aware and load/display the selected or newest immutable snapshot; add unit + XCUITest coverage and a user-facing CHANGELOG entry |

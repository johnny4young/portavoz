...
| 13 | A meeting's stated priority has nowhere to live | **CAPTURE AND TRANSCRIPTION ARE FINE; EXTRACTION HAS NO CONCEPT FOR IT.** Field evidence, Sep 9 2026: a real standup opened with "Priority is share link, whatever is in progress" and later "everything else on hold until those are essentially [done]". The transcript holds all 13 mentions verbatim, and the general summary reduced it to a per-person status line ("Me: … good progress on share and share link…"). That meeting produced zero action items, and the live Apuntador spent its 23 cards on ambient context | A first-class "what matters now" fact — the meeting-level priority or focus, with its exact evidence — that the summary ranks first, the live focus slot can hold (D504 built the slot), and Ask can answer from. Today the only meeting-level typed truths are decisions, action items, commitments and topics; a stated priority is none of those | Not planned yet. Needs a detector contract with exact evidence and an abstention rule before any UI, on the D26/D138 pattern: it must never infer a priority from emphasis alone |

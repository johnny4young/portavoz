...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2I established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, moved Meeting Detail summary regeneration behind storage/preferences/provider ports, closed its first recipe-state parity gap, moved audio import and refine behind typed ports, and moved durable recording Start/Stop policy behind runtime/filesystem/store/lifecycle ports. Concrete capture remains in a private app runtime; launch recovery and global `libraryVersion` orchestration remain in the app** | **Next:** extract `RecoverInterruptedMeetings` while preserving evidence-first multi-root scanning, expired-lease recovery, ready-meeting protection, no ML during reconciliation, worker startup order, and existing presentation/invalidation timing; then continue one characterized use case at a time before replacing broad invalidation with scoped observations |

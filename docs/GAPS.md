...
| T27 | Ask has stage evidence but no authoritative progressive benchmark report | D192 emits one content-free trace across current corpus readiness, expansion, lexical retrieval, query embedding, semantic scan, fusion, citation materialization, first evidence, first observable answer token, and completion. Deterministic observers and tests prove exact stage order and cancellation closure. The current answer provider is non-streaming, so its token milestone occurs only when the complete string returns | Aggregate every run with comparable wall/CPU p50/p95, host/OS/build/configuration, corpus generation/checksum, readiness, and warmup; fail on citation invalidity or nondeterminism before moving corpus writes out of the request path |

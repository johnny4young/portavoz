...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2H established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, moved Meeting Detail summary regeneration behind storage/preferences/provider ports, closed its first recipe-state parity gap, moved audio import and refine behind typed ports, and moved durable Stop evidence/fallback/job policy behind filesystem/store/lifecycle ports. Recording start/recovery and global `libraryVersion` orchestration remain in the app** | **Next:** extract `StartRecording` with atomic reservation, engine preparation, source-start rollback, live scheduler isolation, and presentation parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2E established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, moved Meeting Detail summary regeneration behind storage/preferences/provider ports, and closed its first recipe-state parity gap. Import, refine, recording, and global `libraryVersion` orchestration remain in the app** | **Next:** extract audio import with rollback/language/engine-release parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

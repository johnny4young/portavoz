...
| R15 | One user edit assembles the correction history four times and loads the full accepted transcript once per event inside a single write transaction | Still open. The repeated loads live inside `validateCorrectionAgainstAcceptedTranscript`, the most correctness-critical validator in the store; hoisting a snapshot through `appendCanonicalTranscriptCorrections` and `finalizeTranscriptCorrectionProjections` is a real refactor and R14 already removed the largest repeated read | Thread one history/transcript snapshot through the write, with the correction suites as the gate |

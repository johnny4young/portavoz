...
| T10 | No unified local diagnostics/provenance surface | **Band 3 slices 3A–3B now record content-free, attempt-level provenance for manual/post-refine and durable post-capture summary generation. Successful durable runs share the job's lease/revision-fenced artifact transaction; import/transcript/Companion producers and the user-facing diagnostics/privacy surface remain absent** | **Next:** slice 3C adopts external-audio import summaries; later Band 3 slices add the remaining producers, signposts, local diagnostics export, privacy receipt, and explicit opt-in for any transfer |
...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2U established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and every characterized local product/read policy behind inward contracts; gave Library, Insights, and each selected Meeting Detail tested feature-state owners; replaced all three broad read reloads with storage-independent contracts plus query-scoped GRDB observations; routed Meeting Detail persistence mutations through explicit model actions and a narrow app adapter; and removed two unused speculative package targets after a compatibility audit. Band 3A extends regeneration with atomic summary provenance; Band 3B extends the app-owned durable worker while preserving its existing job fence. Meeting Detail now observes transcript/cast, newest immutable summary/actions, and Companion independently. `UpcomingEvent` and `ContextItem` are Core values while EventKit/RAG/external adapters remain in IntegrationsKit. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, and Spotlight's compatibility counter remain in the app composition graph** | **Next:** continue Band 3 with external-audio import summary provenance; defer audio-path decomposition and incremental Spotlight indexing/outbox work to Band 4 |

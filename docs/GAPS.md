...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2K established the executable ApplicationKit boundary and moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import, refine, durable recording Start/Stop, and expired-lease/evidence-first launch recovery behind characterized ports. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, bundle export, and global `libraryVersion` orchestration remain in the app** | **Next:** extract `ExportMeetingBundle` from Meeting Detail while preserving format-v1 content, optional canonical audio, native save-panel/error behavior, and off-main file work; then introduce feature models and replace broad invalidation with scoped observations |

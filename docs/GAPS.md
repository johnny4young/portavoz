...
| T33 | ~~Unreadable encrypted voice identity can be hidden or overwritten~~ | **RESOLVED in code (D357):** existing ciphertext forbids replacement-key creation; missing/corrupt keys and unreadable/authentication/decoding failures propagate before every save, remember, or remove; AES-GCM serialization is guarded; and Settings keeps Retry plus explicit destructive reset visible. A temporary-store-only unavailable fixture and real-app XCUITest cover both stores without touching host biometric state. A real Keychain-reset journey and physical VoiceOver remain field evidence, not data-recovery promises. | ✅ |

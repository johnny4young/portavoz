...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2T established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and every characterized local product/read policy behind inward contracts; gave Library, Insights, and each selected Meeting Detail tested feature-state owners; replaced all three broad read reloads with storage-independent contracts plus query-scoped GRDB observations; and routed Meeting Detail persistence mutations through explicit model actions and a narrow app adapter. Meeting Detail now observes transcript/cast, newest immutable summary/actions, and Companion independently. `UpcomingEvent` is a Core value while EventKit/RAG/external adapters remain in IntegrationsKit. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, the two unused placeholder Kits, and Spotlight's compatibility counter remain in the app composition graph** | **Next:** resolve `ContextFeedKit`/`SyncKit` in slice 2U; defer audio-path decomposition and incremental Spotlight indexing/outbox work to Band 4 |

...
| T11 | Post-recording workflow is only partially durable | **Slices 1A–1D-a:** capture persists the shell/assets first, writes crash-readable staging CAFs, validates checksum/media/health evidence, publishes without overwrite, and installs captured assets plus live content in one Unit of Work. StorageKit now has idempotent owner-leased jobs and repeat-safe expired-lease recovery. The app still runs post-capture work synchronously and does not reconcile interrupted meeting/staging state on launch | Band 1 slice 1D-b |

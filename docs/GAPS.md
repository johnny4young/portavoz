...
| T11 | Post-recording workflow is only partially durable | **Slices 1A–1D-b1:** capture persists the shell/assets first, writes crash-readable staging CAFs, validates checksum/media/health evidence, publishes without overwrite, and installs captured assets plus live content in one Unit of Work. StorageKit has idempotent owner-leased jobs; process launch now recovers expired leases and reconciles interrupted meeting/staging/final evidence conservatively across both audio roots. The normal app path still runs post-capture work synchronously and has no concrete durable workers or artifact-plus-job completion transaction | Band 1 slice 1D-b2 |

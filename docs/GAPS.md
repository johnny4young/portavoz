...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2Q established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and every characterized local product/read policy behind inward contracts; gave each Library window one tested `LibraryModel` state/action/effect owner; and replaced its broad reload with storage-independent read contracts plus scoped meeting/voice-mix, open-item, trash, and FTS observations. `UpcomingEvent` is now a Core value while EventKit/RAG/external adapters remain in IntegrationsKit. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, and the Detail/Insights/Spotlight `libraryVersion` consumers remain in the app composition graph** | **Next:** retire Insights broad invalidation through its own read-model/observation parity slice; keep Detail and Spotlight independent |

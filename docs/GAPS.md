...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2B established the executable ApplicationKit boundary and moved all app delete/restore writes behind characterized use cases. Most workflows and global `libraryVersion` invalidation remain in the app** | **Next:** finish the trash boundary with permanent/expired purge; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

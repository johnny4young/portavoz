...
| T10 | No unified local diagnostics/provenance surface | **Band 3 slice 3A now records content-free, attempt-level provenance for manual/post-refine summary generation and links successful summaries atomically. Durable post-capture/import/transcript/Companion producers and the user-facing diagnostics/privacy surface remain absent** | **Next:** slice 3B adopts durable post-capture summaries; later Band 3 slices add the remaining producers, signposts, local diagnostics export, privacy receipt, and explicit opt-in for any transfer |

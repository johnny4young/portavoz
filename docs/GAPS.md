...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2G established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, moved Meeting Detail summary regeneration behind storage/preferences/provider ports, closed its first recipe-state parity gap, moved audio import behind typed ports with staged rollback and atomic aggregate persistence, and moved refine draft/apply behind typed ports with explicit cancellation plus revision-fenced atomic acceptance. Recording/recovery and global `libraryVersion` orchestration remain in the app** | **Next:** extract `StopRecording` from `RecordingController` with immediate navigation, atomic durable handoff, degradable derivation, engine-release, Shortcut, and no-audio parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

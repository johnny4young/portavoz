...
| T34 | Live dictation vocabulary and same-alphabet language restriction remain limited | **PARTIAL (Sep 2026).** The live `ParakeetEngine` now forwards supported language hints to the native manager, and Settings explains that the backend filters alphabets, not Spanish versus English. Segments still carry the requested hint rather than a detected-language classification. Live Parakeet still never reads `hints.vocabulary`: `customVocabulary` biases only the batch Whisper/SpeechAnalyzer engines. Streaming vocabulary boosting needs the unpinned `parakeet-ctc-110m` CTC model; preferred spellings therefore still come only from the deterministic `DictationTextRules` tier (spec 06). | Evaluate a verified, consented CTC model and wire `configureVocabularyBoosting` only with call-site quality and resource evidence. Do not claim the existing language hint separates same-alphabet languages or improves recognition quality. |
| T35 | The reviewed speech-engine pin freezes Portavoz behind published upstream fixes | **ACCEPTED (D516, Sep 2026).** `Package.swift` requires exact FluidAudio 0.15.6 (19 Aug 2026). Upstream has since shipped 0.15.7 (10 Sep) and 0.15.8 (20 Sep), and both touch mechanisms Portavoz depends on: streaming seam reconciliation and final-window re-decode (upstream #855, #895, #897, #903, #904, #908 — the same seam `ParakeetSegmentMapper` trims by hand under D16), streaming vocabulary text loss (#851/#898), offline-diarizer cancellation propagation and speaker-cap correctness (#886, #891), and a documented macOS 14 BNNS diarizer-crash advisory (#878/#885) that lands on Portavoz's macOS 14.4 deployment floor. None of this is qualified against Portavoz evidence, so the pin holds and the lag grows | Re-evaluate when a release candidate is being prepared, or sooner if a user reports a caption-seam, dictation-vocabulary or macOS 14 diarization failure. An upgrade needs matched recognition, diarization, latency and memory evidence (D516); an evaluation may still reject it. |
...
| R11 | ~~`CapturePCMGeometry` resamples each buffer with no fractional phase carry, so a rate-mismatched replacement device drifts~~ | **RESOLVED (Sep 2026, D503).** `LinearResampler` carries the fractional phase and the last sample across buffers; `MicrophoneSource` and `ProcessTapSource` each own one under their delivery lock. `LinearResamplerTests` checks streamed shape and carry admission; native callback fixtures additionally cover unnotified rate changes, passthrough intervals and muted history (D510). Physical device continuity remains a field gate | ✅ |

...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2F established the executable ApplicationKit boundary, moved every app trash mutation behind characterized storage/filesystem ports, moved Meeting Detail summary regeneration behind storage/preferences/provider ports, closed its first recipe-state parity gap, and moved audio import behind typed file/preference/processor/store/summary ports with staged rollback and atomic aggregate persistence. Refine, recording, and global `libraryVersion` orchestration remain in the app** | **Next:** extract `RefineMeeting` with draft/compare/apply, cancellation, language, Companion, engine-release, and immutable-history parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

...
| T5 | Exact semantic retrieval remains O(n) and the latest authoritative checkout run misses its scale budget | D83 retained the Accelerate exact scan after its original 100,000-vector evidence met the accepted latency budget. The 8 Aug 2026 authoritative `make perf-ledger` run on the reference Tahoe host measured 139.64 ms wall / 141.98 ms CPU against the 100 ms budget; no later passing run exists, so release evidence remains red even though incremental footprint stayed at 8.42 MiB. D206–D221 correctly keep sqlite-vec and mutation paths research-only because no accepted cross-host Sequoia/Tahoe baseline or engine decision exists. | Keep exact control as the correctness authority, but fix its streaming/cancellation/materialization path or prove a stable exact sqlite-vec replacement before release. Collect the accepted D215/D219 multi-host receipts and require bilingual quality, memory, correction, rebuild, and rollback parity before ANN complexity or product authority. |
...
| T31 | Strict lint is configured but the current local branch is red | The Aug 8 audit found 20 unsuppressed first-party violations: oversized Meeting Detail, processing, correction, job, and graph files; complex graph/decision/search functions; a seven-parameter Skills storage helper; one long identifier; and one `for`/`where` style defect. Hosted CI would reject this checkout if it were published. | Refactor by cohesive owners rather than blanket suppressions: first split Meeting Detail actions/state and decision/graph query row mapping, then extract processing/job storage units, and keep `swiftlint lint --strict --no-cache` as the merge gate. |
| T32 | Database-open failure still terminates the app at launch | `AppServices.init` calls `fatalError` when the meeting store cannot open. This avoids a misleading half-functional UI, but corruption, permissions, disk-full, or migration failure produces an abrupt exit with no user recovery surface. | Add a separate launch state machine that preserves the failing database, offers support-safe diagnostics and backup/restore or read-only recovery, and never auto-deletes or silently recreates user data. |

...
| 13 | ~~A meeting's stated priority has nowhere to live~~ | **DETECTOR LANDED (Sep 2026, D505); PERSISTENCE STILL OPEN.** Field evidence: a real standup declared its priority out loud and later put everything else on hold. Capture and transcription were exact — the statement is in the transcript verbatim — but the general summary reduced it to a per-person status line, the meeting produced zero action items, and the live Apuntador spent all 23 of its cards on ambient context. A stated priority is not a decision, an action item, a commitment or a topic, which are the only meeting-level typed truths | `StatedPriorityDetector` now reads an explicit copular statement and offers the subject as an objective, which persists through `contextItems` and shapes the summary. What is still missing is a first-class typed priority: the offer is inert until accepted, an unaccepted one is lost at Stop, and Ask cannot answer "what was the priority?" | Deterministic detector shipped, widened and field-measured across five real meetings and 3 754 finalized captions — one detection, correct, no false positives (D505/D506). The model lane was spiked and rejected in its naive shape: an on-device model behind guided generation and a grounding gate proposed 26 priorities for a control meeting whose transcript never contains the word, none correct. A model lane is defensible only as stage three of a cascade behind a classifier, which needs a labeled bilingual corpus first. A typed, persisted priority with its own evidence chain remains unplanned; it needs a schema slice and an Ask lane of its own |
...
| **Release blocker — source integration** | **PRODUCER IMPLEMENTED (D402/D438) AND REVIEW BASELINE FROZEN (D461):** a manual, read-only GitHub workflow now owns the receipt and admits only the exact current `main` merge with a current independent human approval, no outstanding change request, and one first-attempt successful hosted CI run containing the required current-SDK macOS suite, iOS portability compile, Sequoia suite, lint, and hygiene jobs. It checks out trusted `main` rather than caller-selected code and rejects side branches, stale ancestors, self/bot/stale/dismissed review state, duplicate unchanged runs, reruns, missing jobs, and caller-supplied proof flags. Before final review, the PR branch must converge the current `main` dependency graph and treat that two-parent head as new source for local candidate and hosted qualification; an older green receipt is historical only. The current candidate is still local-only and therefore has no receipt. | With explicit authorization, integrate the candidate through a reviewed PR, let exact-main CI pass once, rerun candidate automation on that integrated commit, then manually dispatch from `main` before another commit advances it and download the content-free source-integration artifact for the same version/build/commit. A push, PR, merge, or workflow dispatch remains a remote mutation and requires explicit user authorization. |
| **Release blocker — deterministic qualification** | **OWNER IMPLEMENTED (D391/D392–D401/D447–D467):** the schema-2 D147 ledger has explicit fail-closed cells for frozen scope, autonomous validation, the installed-model lane, authoritative automated performance, current-host resource/memory, long capture, upgrade/recovery, and complete bilingual XCUITest. One clean-commit sequential runner owns and validates those artifacts. It builds the exact Release CLI once, binds every performance harness to its SHA-256, and admits scale, semantic, and Spotlight independently only after ten bounded CPU/load/recognized-interference/no-other-Portavoz-app/power/thermal observations at 500-millisecond intervals across a 4.5-second span plus five source-independent SHA-256 throughput samples; the denser window closes periodic symbolication phase aliasing without changing thresholds, an active Portavoz app blocks even at zero sampled CPU, and JavaScript, non-Swift build, browser-automation, media-processing, and container/virtualization work now enters the same closed two-percent interference boundary; later XCTest, model, leak/symbolication, and resource work therefore cannot contaminate its own sample, while unsettled or unclassified external contention remains fail-closed through aggregate CPU/load. Readiness schema 5/policy v5 retains the active Portavoz app count, attributes interfering CPU only to a pinned closed executable-class vocabulary, and retains bounded calibration timings with numeric aggregates, never names, paths, PIDs, arguments, meeting state, or payloads, so a timeout remains actionable without weakening admission. Isolated resource processes also suppress the production-only opportunistic MLX descriptor verification; exact scenario preparation remains explicit, comparable samples stay in contiguous scenario-family blocks, and Summary force-verifies its pinned descriptor before measurement. Candidate contract schema 9 also requires one exact content-free zero-leak/zero-byte receipt across four real-app Apuntador paths with the fixed 100/100/10/10 operation matrix. Its fixed PERF-008 confirmation set still blocks a stable same-metric regression or an inconclusive set, while any selected clean ledger retains every observation, all three readiness receipts, binary identity, and digest. Its receipt is bound to one exact source identity; a prior receipt never qualifies a changed source. This durable product-gap contract defines that invariant rather than embedding an ephemeral latest-run status. | `make candidate-automation` passes and writes its exact version/build/commit receipt. Six model classes must execute rather than all skip; all 12 automated performance metrics are authoritative while the exact 13 non-autonomous metrics remain visibly not measured; all nine resource scenarios have three stable current-host samples; all four leak scenarios pass through the disposable Release app with exact per-path repetition (100/100/10/10); and the complete current EN plus ES XCUITest catalogues pass from one build. Scoped feature evidence does not replace this complete integration gate. |
...
## Findings from the 1.0 integration review (Sep 2026)

An independent multi-module review of the integration PR produced findings in
five areas. The confirmed defects were fixed under D497; each remaining entry
was then verified against the real code and either closed in its own reviewed
slice (D499, D501, D502, D503) or rejected with the test that contradicts it.
All eighteen are closed — every fix carries a test that fails without it.

| # | Finding | Resolution | Status |
|---|---|---|---|
| R1 | ~~A superseded diarization or transcription job is cancelled with no replacement~~ | **RESOLVED (Sep 2026, D501).** `PostCaptureProcessingError.inputSuperseded` now carries a replacement for every lane, so a drifted prediction costs a retry instead of the meeting's diarization or transcript | ✅ |
| R2 | ~~`installPublishedCapture` drops `transcriptRequiresRecovery`~~ | **RESOLVED (Sep 2026, D501).** The ladder gains its missing rung: when the finalized audio admits no recovery transcription the meeting is preserved as needs-attention with `transcription.recovery.unavailable`, never completed with partial captions | ✅ |
| R3 | ~~A CloudKit account switch leaves attempts fenced by cleared replays~~ | **RESOLVED (Sep 2026, D501).** Both the account switch and a non-blocking replacement reopen the attempt whose blocker is gone, in the shape `validateAttempts` accepts; the staged payload is untouched | ✅ |
| R4 | ~~Every sync commit re-reads and SHA-256s every staged payload~~ | **RESOLVED (Sep 2026, D502).** Validation is layered: a commit proves shape and field invariants with no filesystem access, and the full proof — every payload re-read and re-hashed, record system fields, engine-state blob — runs when the snapshot is loaded, where corruption is actually detectable | ✅ |
| R5 | ~~Seven graph read paths load the whole live `topic` table~~ | **RESOLVED (Sep 2026, D502).** The four paths that recomputed a family now call the existing recursive-CTE twins `topicFamilyRootID`/`topicFamilyMemberIDs`, and the two duplicated `…FamilyIDs` helpers are gone | ✅ |
| R6 | ~~Graph and person-commitment queries hydrate every admitted candidate~~ | **RESOLVED (Sep 2026, D502).** Both loops stop one fact past the page, which is all `hasMore` needs; the omitted counts describe what the page inspected | ✅ |
| R7 | ~~`MeetingStore+Exports` degrades every failure with `try?`~~ | **RESOLVED (Sep 2026, D502).** Only a `StorageError` degrades an optional payload to empty, exactly as the released contract intended; a database failure now reaches the caller instead of producing a quietly incomplete export | ✅ |
| R8 | ~~Meeting passages enter the RAG prompt unfenced~~ | **RESOLVED (Sep 2026, D502).** Each passage is `<passage id="N">`, XML-escaped with bracketed numbers neutralized, so transcript text cannot forge a passage boundary or a citation the answer parser would resolve | ✅ |
| R9 | ~~Live speaker relabeling is O(turns² × captions) on the main actor and republishes the whole caption array per turn~~ | **RESOLVED (Sep 2026, D503).** `SpeakerAttributor` resolves each segment through a sorted `TurnIndex` with a running maximum-end prefix and two binary searches, and `RecordingController` republishes only when `LiveSpeakerHints.changed(from:to:)` sees a real change | ✅ |
| R10 | ~~`ArchitectureDependencyTests` is a 13.5k-line source/doc grep~~ | **RESOLVED (Sep 2026, D503).** Split into eleven `ArchitectureDependencyTests+*.swift` families over a shared root; all 228 tests and every assertion moved unchanged, and repository hygiene now fails any test file over 2 400 lines. The policy assertions stay in XCTest by decision — they are the enforcement, they run in 14 s, and the checks that read other test files pin that a named UI journey still exists | ✅ |
| R11 | ~~`CapturePCMGeometry` resamples each buffer with no fractional phase carry, so a rate-mismatched replacement device drifts~~ | **RESOLVED (Sep 2026, D503).** `LinearResampler` carries the fractional phase and the last sample across buffers; `MicrophoneSource` and `ProcessTapSource` each own one under their delivery lock. `LinearResamplerTests` measures the old per-buffer floor losing 46 frames over 200 callbacks and pins the streamed form within one frame of exact | ✅ |
| R12 | ~~A `StorageError` escaping the correction merge is not reported as `.correctionConflict`~~ | **NOT A GAP (Sep 2026).** Deliberate and tested: `testPendingRemoteCorrectionStorageFailureIsNotReportedAsUserConflict` pins that an invalid remote correction fails loudly, leaves local history untouched, and preserves the pending change rather than appearing as a convergence conflict the user must resolve. The mapping now says so in place. The real residual is the absence of a quarantine for an envelope that keeps failing | Quarantine a repeatedly failing envelope, without reclassifying it |
| R13 | ~~`migrateAudio`'s `skipping:` guard is never passed by production~~ | **RESOLVED (Sep 2026, D499).** `ManageRecordingStorage` does exist (in `SettingsResources.swift`) and refuses a move while capture is busy, but that phase is sampled once. The app now samples the live capture's audio directory again immediately before the first rename and passes it as `skipping:`, and the doc states that contract | ✅ |
| R14 | ~~Structural projection loads the whole transcript on every correction write~~ | **RESOLVED (Sep 2026, D499).** Stale rows are cleared before the refresh, so a history with no `.split`/`.merge` returns before the fetch; only those two kinds project rows | ✅ |
| R15 | ~~One user edit loads the full accepted transcript once per event inside a single write transaction~~ | **RESOLVED (Sep 2026, D503).** Corrections never write `segment`, so a batch reads the accepted transcript once per meeting and passes it to the otherwise unchanged validator; the single-event replica-merge path keeps its own read | ✅ |
| R16 | ~~Merge provenance order relied on undefined `GROUP_CONCAT` behaviour~~ | **RESOLVED (Sep 2026, D499).** `ORDER BY` inside an aggregate needs SQLite 3.44 and the deployment floor is older, so every lane now concatenates `ordinal:segmentID` and the decoder sorts by ordinal and rejects a missing, non-numeric or duplicated one | ✅ |
| R17 | ~~The accepted-lane scan predicate was written twice~~ | **RESOLVED (Sep 2026, D499).** One `acceptedSemanticScanBranchSQL` is interpolated into both scans | ✅ |
| R18 | ~~A migration backfill threw instead of degrading~~ | **RESOLVED (Sep 2026, D499).** The three backfill loops skip a meeting whose history the current policy rejects. The projection is disposable and is rebuilt on that meeting's next correction write, so one bad meeting can no longer leave the database unopenable | ✅ |

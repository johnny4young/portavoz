...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2L established the executable ApplicationKit boundary and moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, and expired-lease/evidence-first launch recovery behind characterized ports. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, feature state, and global `libraryVersion` orchestration remain in the app** | **Next:** introduce `LibraryModel` as the first feature-scoped state owner without changing UI/reload behavior; then replace broad invalidation with scoped observations in a separate slice |

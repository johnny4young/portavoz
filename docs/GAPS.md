...
| T28 | Transcript corrections do not yet reach every search/intelligence surface | **PARTIAL (D225/D229–D235)**: Meeting Detail composes current-revision text, speaker, split, explicit adjacent merge, and suppress corrections over immutable accepted evidence; derived artifacts are fenced by effective correction lineage; Markdown, PDF, SRT, WebVTT, CLI, and Gist share one composed projection with original timing; private sync converges compatible histories and fences conflicts. **PARTIAL (D313)**: an active `replaceText` correction serves its corrected text through the per-segment `segmentCorrectedText` FTS projection (v33, transactionally refreshed and backfilled on upgrade), a speaker-only correction no longer hides its line from search, and MCP appends the frozen-prefix `portavoz-reading/2` tools (`get_transcript_v2` composed and fail-closed, `get_summary_v2`/`get_action_items_v2` with correction provenance). **PARTIAL (D329)**: meeting-level Spotlight reuses the revision-fenced text lane, excludes active structural targets, restores accepted rows on undo, and admits only summaries matching sparse v36 correction lineage; malformed or missing lineage fails closed and successful correction writes wake index reconciliation. **PARTIAL (D330)**: schema v37 gives current replacement text a correction-fenced semantic vector under the accepted segment identity; the existing background owner/readiness contract indexes it, stale publication fails closed, accepted-only search keeps its fast exact path, and restore reuses the cached accepted vector. **PARTIAL (D331)**: stale Apuntador cards now offer one explicit whole-snapshot refresh over the composed transcript; generated split/merge evidence maps back to immutable accepted IDs, a completed pass replaces atomically, and unavailable/incomplete/stale publication preserves the prior cards. Correction writes still start no model work and automatic refresh remains excluded by design. **Still open:** split/merge/suppress composed content stays out of search, semantic search, and Spotlight because no shared structural result identity exists; correction-heavy semantic performance and physical Sequoia/Tahoe asset behavior are not field-proven. | Decide whether split/merge/suppress content earns a shared search identity story or stays excluded by design; collect correction-heavy semantic latency/memory and real Sequoia/Tahoe evidence before claiming universal readiness |

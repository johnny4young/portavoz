...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2J established the executable ApplicationKit boundary, moved all trash mutations, Meeting Detail regeneration, external-audio import, refine, durable recording Start/Stop, and expired-lease/evidence-first launch recovery behind characterized ports. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, bundle import, and global `libraryVersion` orchestration remain in the app** | **Next:** begin composition-root reduction with `ImportMeetingBundle`, preserving identity remapping, optional attachments, immutable summaries, notes/cards, and exact presentation timing while replacing sequential aggregate writes with one atomic characterized boundary; then introduce feature models and replace broad invalidation with scoped observations |

...
|---|---|---|---|---|
| 1 | **Distribución cero** | DMG notarizado + cask + appcast listos, README con benchmarks ✅, higiene OSS ✅ (SwiftLint + templates + CI lint), repo privado, 0 usuarios | localización EN + las acciones públicas (push, release, tap — requieren autorización) | M9 — prep interna hecha; falta publicar |
...
| 5 | **UI solo en español** | ~145 strings hardcodeados en 4 vistas (MeetingDetailView 64, SettingsView 43, RecordingView 18, LibraryView 16) | localización (mínimo EN — "talla mundial" implica bilingüe también en la UI, no solo en los resúmenes) | M9, **diferida a justo antes de publicar** (jul 2026). Enfoque decidido: `.lproj/Localizable.strings` (es base + en) copiados por `make-app.sh` a `Contents/Resources` + `CFBundleLocalizations`; lookup por `Bundle.main` deja los `Text("…")` intactos (sin `bundle:`). La app es ejecutable SPM envuelto por script, no target Xcode |

...
| T11 | Post-recording workflow is not durable | **Foundation landed in Band 1 slice 1A:** schema v6 now has lifecycle, assets, jobs, provenance links, outbox, and meeting preferences. Runtime still persists after capture and does not recover partial assets/jobs, so audio and partial work can remain undiscoverable when stop-time derivation fails | Band 1 slices 1B+ |

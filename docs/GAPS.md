...
| T11 | Post-recording workflow is only partially durable | **Slices 1A–1C:** capture persists the shell/assets first, writes crash-readable staging CAFs, validates checksum/media/health evidence, publishes without overwrite, and installs captured assets plus live content in one Unit of Work. Audio survives empty captions and later required-write failures. Durable jobs and idempotent launch reconciliation of interrupted lifecycle/staging state are not adopted yet | Band 1 slice 1D+ |

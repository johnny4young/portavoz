...
| **Accepted 0.8 limitation / claim gate** | Quick Look is absent; exact semantic retrieval remains O(n); model-gated coverage remains a required local lane; graph value is limited to six explicit source-backed Ask explorers; other pending real-feature checks stay honestly open. | Release notes and marketing exclude any unverified performance, hardware, App Intent, graph, search, sync, or external-effect claim. A reproduced crash, data-loss defect, or D147 failure is promoted to a code blocker. |
| **Commercial launch blocker, not an OSS RC blocker** | Purchase, activation, and restore are still missing. | Implement and qualify licensing only before selling 0.8 as a paid PRO launch; a free/open-source 0.8 candidate must not imply that commerce exists. |
| **Post-release backlog** | Quick Look, iOS/companion work, user-authored actions, standing rules, free-form or visual graph, broader graph CLI/MCP/sync/export, and alternate retrieval/ASR authorities. | Re-enter through a separately approved evidence-backed plan after 0.8.0 qualification. |

...
| T30 | Meeting Memory Graph serves all six source-backed jobs within the relational budget, but released query adoption, field evidence, and downgrade recovery remain open | **PARTIAL (D270–D286/D308–D314):** the deterministic 36-case public-synthetic corpus defines six longitudinal jobs; schemas v25–v32 hold explicit topic, decision, commitment-change, question, blocker, and decision-topic authority beside a disposable relational graph. All six adapters (active blockers, authoritative first discussion, current person commitments, decision conflicts, change since, and current decision history) cross public product boundaries, exact filters apply before limits, and typed synthesis reserves transcript rank. Meeting Detail ships exact decision confirmation plus independent topic-link withdrawal. At 10k meetings all six reads measured 2.3–76.1 ms p95 against 250 ms; recursive probes stayed under 6 ms, and D314 reduced a full rebuild from 17.6 minutes to 27.2 seconds. SQLite remains authoritative; the graph only checks or selects topology and never becomes source evidence. **Still open:** ordinary Ask, command palette, CLI, MCP, and meeting briefs remain transcript-only because no released surface extracts an exact graph job from natural language; returning to a previously completed graph profile at the same source generation stalls until the next authority write; private owner-reviewed evidence, sync/export, dedicated query UX, graph telemetry, and physical VoiceOver/Sequoia/Tahoe validation remain absent. | First make graph-profile re-admission recover without an unrelated authority write. Keep SQLite and the exact adapters. Design an explicit released query surface or conservative extraction contract without guessing identity, then collect private owner-reviewed and supported-host evidence before making graph-aware answer claims. Define sync/export and CLI/MCP contracts separately; do not add another graph engine or rebuild adapter without evidence. |

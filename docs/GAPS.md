...
| T26 | Whole-library Markdown backup is not relaunch-resumable | D180 prevents a backup requested during protected capture from reading SQLite or publishing files. D181 creates one coherent private SQLite stage, checkpoints bounded page copy and one-at-a-time render/publication work, and resumes process-locally without republishing completed files. D182 gives current-format stages kernel-owned leases and removes only provably abandoned stages at process launch while preserving live and unknown workspaces. D183 retains regular bookmark identity in the active actor, follows a moved destination, and reacquires/closes one access lease per execution interval without misrepresenting the current non-App-Sandbox app as security-scoped. D184 persists that bookmark plus exact pending/completed filename, digest, and byte-count evidence around the atomic move; post-move save failure cannot republish in the same process, and cleanup prunes only journals for exact UUID stages it proved abandoned. D185 adds a content-free keyset cursor and a strict read-only StorageKit adoption primitive that rejects active ownership, symlinks, malformed work, and cursor mismatch without deleting evidence. D186 maps that cursor into optional recovery metadata only after publication completion, retries a failed checkpoint without repeating the destination move, and freezes advancement after any process-local source, document, or publication failure. D187 binds that cursor to each safe reservation and provides exact no-follow destination evidence plus missing/matching/conflicting reconciliation; conflicting evidence and every cursor-less match remain blocked. Launch still cleans rather than invoking reconciliation or adoption | Journal or otherwise reconstruct typed failure outcomes and any required render boundary, invoke pending reconciliation plus exact stage adoption before cleanup, and prove restart semantics before claiming relaunch-durable backup |

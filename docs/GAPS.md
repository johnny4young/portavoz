...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2P established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, the chapter/playback/summary-section/speaker-hue review policies, and the Insights scope/statistics/findings policies behind characterized inward contracts; gave each Library window one tested `LibraryModel` state/action/effect owner; and replaced its broad reload with storage-independent read contracts plus scoped meeting/voice-mix, open-item, trash, and FTS observations. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, the Detail/Insights/Spotlight `libraryVersion` consumers, and remaining brief/reminder/mirror policies inside IntegrationsKit remain in the app composition graph** | **Next:** move the final characterized local product-policy cluster inward; retire remaining broad invalidation consumers only in their own parity slices |

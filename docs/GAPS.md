...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2R established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and every characterized local product/read policy behind inward contracts; gave each Library and Insights window one tested feature-state owner; and replaced both broad reloads with storage-independent read contracts plus query-scoped GRDB observations. Insights now reads independent meeting, participant/commitment, voice-balance, and scope-bounded finding streams. `UpcomingEvent` is a Core value while EventKit/RAG/external adapters remain in IntegrationsKit. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, and only the Meeting Detail/Spotlight `libraryVersion` consumers remain in the app composition graph** | **Next:** retire Meeting Detail broad invalidation through its own meeting-scoped read-model/observation parity slice; keep Spotlight independent and defer incremental indexing/outbox work to Band 4 |

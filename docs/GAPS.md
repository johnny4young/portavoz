...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2O established the executable ApplicationKit boundary; moved all trash mutations, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, refine, durable recording Start/Stop, expired-lease/evidence-first launch recovery, and the chapter/playback/summary-section/speaker-hue review policies behind characterized inward contracts; gave each Library window one tested `LibraryModel` state/action/effect owner; and replaced its broad reload with storage-independent read contracts plus scoped meeting/voice-mix, open-item, trash, and FTS observations. Concrete capture, CAF/root mechanics, worker execution, fixture/log mapping, the Detail/Insights/Spotlight `libraryVersion` consumers, and remaining Insights/brief/reminder/mirror policies inside IntegrationsKit remain in the app composition graph** | **Next:** move the characterized Insights read-policy cluster inward; retire remaining broad invalidation consumers only in their own parity slices |

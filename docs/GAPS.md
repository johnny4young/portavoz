...
| 10 | Native recording App Intents ✅ / Quick Look remains absent | **Start field-verified Jul 27; Stop implemented Aug 9, 2026.** `StartRecordingIntent` and `StopRecordingIntent` are extracted from the SDK-only source under the shipping module name — no Xcode app target needed — and packaging requires exactly both actions. Tahoe+ uses immediate foreground intent modes while the compatibility property preserves Sequoia. Both route inside the chosen process; stable, Dev, and UI-test identities remain distinct (D139/D140/D324). Start's user-created Shortcut was field-verified from Shortcuts, Spotlight, and Siri. Stop is locally covered through metadata, lifecycle/disposition tests, and bilingual real-app handoff, but its physical picker/Siri/Shortcut invocation still needs Sequoia and Tahoe evidence. D141 still omits the unsupported duplicate `AppShortcutsProvider`. Post-meeting Shortcut hook, `portavoz://record`, and meeting Spotlight indexing already existed | Quick Look genuinely needs an extension target and stays planned; collect Stop action field evidence on physical Sequoia/Tahoe Macs | D139–D141, D324; QL M14a/M16 |

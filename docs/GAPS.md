...
| T27 | Ask benchmark authority exists, but no accepted comparable multi-host before-state has been collected | D192 emits one content-free trace across current corpus readiness, expansion, lexical retrieval, query embedding, semantic scan, fusion, citation materialization, first evidence, first observable answer token, and completion. D193 now pairs every passing Ask resource run with a strict native sidecar and aggregates per-stage/milestone wall and CPU p50/p95, corpus generation/checksum/readiness, and validated deterministic citation evidence across profiles. The current answer provider is non-streaming, so its token milestone occurs only when the complete string returns | Run the clean Release matrix on the supported memory profiles, accept only stable comparable evidence, reconcile the historical semantic measurements, and add the full multilingual quality fixture pack before removing request-time corpus writes |

...
| T11 | Post-recording workflow is only partially durable | **Slices 1A–1D-b2 plus the first 1D-b2b control-plane unit:** capture persists the shell/assets first, writes crash-readable staging CAFs, validates checksum/media/health evidence, publishes without overwrite, and installs captured assets plus live content in one Unit of Work. StorageKit has idempotent owner-leased jobs; process launch recovers expired leases and reconciles interrupted meeting/staging/final evidence conservatively across both audio roots. Diarization and summary completion atomically fence source revisions and commit artifact + job success + dependent enqueue; optional work can cancel without failing the meeting, and workers can discover their next durable wake without polling. The normal app path still runs post-capture work synchronously and has no concrete durable executors | Remaining Band 1 slice 1D-b2b |

...
| T11 | Post-recording workflow is only partially durable | **Slices 1A/1B:** schema v6 has lifecycle/assets/jobs/provenance/outbox/preferences, and live capture now persists the shell plus pending assets before sources start. Audio survives empty captions and later required-write failures as a discoverable `needsAttention` meeting. Files still publish directly to final CAF paths, captured data is installed across transactions, and launch recovery/jobs are not adopted | Band 1 slices 1C+ |

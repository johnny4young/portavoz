...
| R12 | `mergeRemoteTranscriptCorrectionHistory` can throw `StorageError.invalidTranscriptCorrection` (the round-trip assertion, and replica validation against the accepted transcript). Only `TranscriptCorrectionReplicaMergeError` and `TranscriptCorrectionValidationError` map to `.correctionConflict`, so those escape `database.write` and the same envelope re-throws on every pull | The round-trip assertion is an internal invariant, not peer disagreement; mapping it to a conflict would silently mask a data-integrity failure. The two causes need separating first | Split the invariant failure (stays loud) from replica validation (becomes a conflict), then add a quarantine for a repeatedly failing envelope |
| R13 | `RecordingsLocation.migrateAudio(from:to:skipping:)` documents `skipping:` as "the last line of defence behind `ManageRecordingStorage`'s activity gate", but no such type exists and the only production caller omits the argument, taking the empty default | Passing live meeting directories changes what a storage move does to an in-flight recording; `resolve` also falls back only to `defaultRoot`, so a skipped directory under a custom origin becomes unresolvable after the switch | Decide the contract: wire the live meetings through, or delete the parameter and the comment, with a test either way |
| R14 | `refreshStructuralSearchProjection` materializes the meeting's entire live transcript on every correction append, tombstone, refine, re-transcription and sync replay, although `TranscriptStructuralSearchProjection.activeRows` returns nothing unless a split/merge exists | Pure performance on a user-facing write; the early return needs a test that proves the projection stays correct for a chain that later gains a split | Return before the fetch when the history has no `.split`/`.merge` |
| R15 | One user edit assembles the correction history four times and loads the full accepted transcript twice inside a single write transaction | Same class as R14; hoisting the snapshot changes the shape of `appendCanonicalTranscriptCorrections` and `finalizeTranscriptCorrectionProjections` | Thread one history/transcript value through the write |
| R16 | `MeetingStore+Search.swift` builds merge provenance with `GROUP_CONCAT` over an `ORDER BY` subquery, and SQLite does not guarantee that ordering; the result is decoded positionally as ordered evidence | Undefined behaviour that happens to hold today; changing it touches decoded evidence order | `GROUP_CONCAT(… ORDER BY ordinal)` where the deployment SQLite allows it, else concatenate `ordinal:segmentID` and sort in Swift |
| R17 | `acceptedSemanticScanSQL` is byte-for-byte the first `UNION ALL` branch of `correctedSemanticScanSQL` | Two copies of the accepted-lane predicate will drift | Interpolate the shared fragment |
| R18 | The v16+ migration backfills call production projection refreshes that throw when `TranscriptCorrectionPolicy` rejects a row, which would make a database permanently unopenable rather than degrading | Theoretical today: v16–v51 are unreleased together, and released stores sit at v15 where `transcriptCorrection` does not exist, so every backfill loop is empty | Skip and log an unparseable meeting inside the migration loop before any of these ship |

...
| T15 | Broad app invalidation and orchestration concentration | **Band 2 slices 2A–2C established the executable ApplicationKit boundary and moved every app trash mutation behind characterized storage/filesystem ports. Summary, import, refine, recording, and global `libraryVersion` orchestration remain in the app** | **Next:** extract Meeting Detail summary regeneration with full cache/pivot/provider parity; continue one characterized use case at a time, then replace broad invalidation with scoped observations |

...
| Cluely ($20–75/mes) + interview companions + **Teams "Facilitator" (~ago-sep 2026)** | Overlays de respuesta en vivo | El patrón del Companion D26 SIN dueño en meeting-notes: Cluely promete 300 ms y entrega 5–10 s reales + estigma "cheating"; Microsoft valida la detección proactiva de preguntas y pone fecha límite competitiva; los interview companions (~$9/mes) prueban la demanda. Nuestro ángulo: local (latencia real), transparente, y `contexto` responde desde TU historial |
...
- **Companion en vivo (D26)**: preguntas detectadas en la conversación ("¿cuál es la diferencia entre `var` y `let`?") → tarjeta con respuesta sugerida en <5 s; `contexto` responde el RAG local, `conocimiento` responde FM on-device (o BYOK con disclosure).
...
2. **La tarjeta del Companion** (M11): alguien pregunta algo técnico y la respuesta ya está en tu panel antes de que termines de procesar la pregunta.
...
| RAM grabando (Mac, STT + diarización EN VIVO) | < 800 MB pico grabando · < 200 MB idle post-reunión (target revisado jul 2026: el original de 500 MB se fijó sin diarización viva) | ✅ por fases (`--bench-record 60 --bench-log`, vía `open -n`): 20 MB sin modelos → ~515 MB engines → **569–795 MB pico grabando** → **140–160 MB tras la reunión** (idle release + páginas CoreML reclamables). El resumen embebido MLX usa ~2.4 GB transitorios y se libera solo a los 120 s (antes quedaba residente para siempre) |

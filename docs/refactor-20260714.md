...
Status: **approved direction; Bands 0–2 complete, Band 3 in progress; slices 3A–3J complete; Sequoia stabilization units 1–6 complete; the App Sandbox capability spike is next**
...
Current verified quality baseline: 671 package tests (13 model-gated), zero
strict-lint violations across 250 Swift source files, and 24 XCUITest cases
...
program; slices 3H, 3I, and 3J are now complete, and the App Sandbox capability
spike is next.
...
6. Next: run and document the App Sandbox capability spike.
...
**Next slice:** run the App Sandbox capability spike against real capture,
model storage, updates, export/import, Keychain, calendar, and local automation
requirements. Record measured evidence and make an explicit enable/defer
decision; do not ship a sandbox entitlement based on static assumptions.
...
| Band 3 — Provenance/privacy | In progress — slices 3A–3J complete (D62–D77) | Manual/post-refine, durable post-capture, and external-audio import summary attempts, accepted refined transcripts, and generated durable Companion cards persist content-free terminal provenance. Durable success shares its owning transaction and revision/lease fence. Every current meeting-content HTTP path crosses one content-free policy port with operation-specific consent and canonical endpoint validation. Schema v7 records immutable attempts before transport plus an honest historical-coverage boundary; receipt failure fails closed, transport failure remains visible, redirects are blocked, and Meeting Detail renders an independently observed content-free receipt. One explicit local support export consumes an atomic redacted projection with pseudonymous identities, Meeting Detail observes durable jobs as a fifth section with bounded recovery, and signposts carry no meeting content. Recording Start/Stop now preserve exact durable outcomes as stable coded failures until the app localizes an explicit recovery route. No-attempt/cache/discard/stale/no-card paths create no orphaned success; migration, failure, cancellation, mismatch, rollback, observation, localization, UI, redaction, typed-boundary, and network-bypass prevention are characterized | Run the App Sandbox capability spike and record an explicit enable/defer decision |

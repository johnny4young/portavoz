...
Current verified quality baseline: 962 package tests (13 model-gated), zero
...
**Next macOS conformance gate:** audit the implemented package and executable
dependency graph and close only evidence-backed boundary violations. Preserve
direct construction in executable composition, live capture, diagnostics, and
benchmark harnesses. The later 6D gate remains the
...
10. **Resolved by D98–D112 for adopted interfaces:** resident menu-bar reads use one scoped
    ...
    behind ApplicationKit. The final macOS conformance audit continues before
    the deferred iOS target.

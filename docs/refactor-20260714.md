...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline complete; Band 4B health optimization is next**
...
Current verified quality baseline: 673 package tests (13 model-gated), zero
...
**Next slice:** Band 4B removes the measured `MeetingHealth` quadratic common
case with characterization for arbitrary long overlaps, then reruns both scale
reports before selecting any view decomposition, cache, DatabasePool, vector,
or Spotlight-outbox change.
...
1. **4B:** optimize `MeetingHealth` without changing interruption semantics;
   retain adversarial overlap characterization and compare the same matrix.
2. Re-run first-content/Hangs evidence. Decompose the detail state or derived
   panes only if the 300 ms first-content budget still misses.
3. Improve broad OR retrieval selectivity before considering `sqlite-vec` or a
   persisted vector-layout migration. Exact FTS remains unchanged.
4. Measure waveform hit/miss behavior with real scratch audio before selecting
   a content-addressable cache.
5. Measure Spotlight delivery/backlog behavior before replacing its current
...
| Band 4 — Detail/performance | In progress; 4A measurement complete (D79) | Release 1k/10k/50k/100k library matrix; 30m/2h/8h detail algorithm matrix; temp-store 5k app signpost/Hangs/Time Profiler evidence; functional scoped-summary XCUITest and screenshot; explicit Xcode 26.6 SwiftUI-lane limitation | 4B: optimize `MeetingHealth` with arbitrary-overlap conservation, rerun the same reports, then reassess first-content decomposition |

...
Status: **approved direction; Band 0 complete, Band 1 in progress (through the 1D-b2b process executor; normal Stop adoption remains)**
...
Current verified quality baseline: 446 package tests (13 model-gated) and 17 XCUITest cases
...
4. **Partially resolved in Band 1 slices 1B–1D-b2b:** the meeting is now discoverable
   ...
   future wake (D42). Only the normal Stop producer cutover remains.
...
one durable future wake instead of polling. The remaining work is the normal
Stop producer cutover. Every recovery path remains safe to run repeatedly and
may resume work or mark a meeting `needsAttention`; it may never silently
delete usable audio.
...
4. ◐ Add idempotent processing jobs and launch recovery. Slice 1D-a owns
   ...
   remaining 1D-b2b work must make normal Stop enqueue the exact initial job and
   kick that executor without losing immediate navigation or shortcut behavior.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 446-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and the 1D-b2b control-plane/executor units complete | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); `recording → captured → processing → ready/needsAttention`; D36–D42; 446-test package baseline | Final slice 1D-b2b cutover: normal Stop enqueues and kicks while retaining released UI/Shortcut parity |

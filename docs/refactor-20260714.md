...
Current verified quality baseline: 586 package tests (13 model-gated) and 20 XCUITest cases
...
3. **Partially resolved through Band 2 slice 2R:** Library and Insights no
   longer consume the global `libraryVersion` integer. Their query families
   observe only explicit GRDB regions through storage-independent feature read
   contracts, and each window owns the matching state. Meeting Detail and
   Spotlight still consume the broad counter and require independent slices;
   incremental Spotlight indexing remains coupled to Band 4 scale work.
...
10. Decide whether placeholder Kits gain a boundary or are removed.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 586-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Complete — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and 1D-b2b | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); atomic snapshot/initial-job handoff, immediate detail navigation, and Shortcut parity (D43); `recording → captured → processing → ready/needsAttention`; 449-test Band 1 baseline retained by the current 586-test package suite | None; preserve these invariants while ApplicationKit owns the launch workflow |
| Band 2 — Application layer | In progress — slices 2A through 2R | ApplicationKit contract/rules, storage-independent Library and Insights updates, four inward meeting-review policies, three inward Insights policies, three inward meeting-preparation/mirror policies, and one Core event value; StorageKit, IntelligenceKit, TranscriptionKit, and DiarizationKit ratchets; characterized lifecycle, regeneration, import/export, refine, Start/Stop, and launch recovery workflows; per-window Library and Insights state owners; explicit-region observations with partial failure isolation and bounded Insights evidence; private app adapters; nineteen architecture rules; direct policy, model, observation, workflow, and UI coverage; only Meeting Detail/Spotlight broad invalidation retained as explicit migration seams (D44–D58) | Replace Meeting Detail broad invalidation with a meeting-scoped read model and observations; keep Spotlight independent and defer its incremental indexing to Band 4 |

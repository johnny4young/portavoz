...
Status: **approved direction; Band 0 complete, Band 1 in progress (through 1D-b2a plus the first 1D-b2b control-plane unit)**
...
Current verified quality baseline: 442 package tests (13 model-gated) and 16 XCUITests
...
4. **Partially resolved in Band 1 slices 1B–1D-b2:** the meeting is now discoverable
   ...
   scheduled-wake query avoids worker polling; concrete app producers/executors
   remain.
...
Slice 1D-b2a now provides stale-safe atomic diarization/summary artifact
completion (D41). The first 1D-b2b control-plane unit adds owner-fenced
cancellation and capability-filtered scheduled-wake discovery; the remaining
work must resume derived `processing` through concrete durable app
producers/executors. Every recovery path remains safe to run repeatedly and may
resume work or mark a meeting `needsAttention`; it may never silently delete
usable audio.
...
4. ◐ Add idempotent processing jobs and launch recovery. Slice 1D-a owns
   ...
   capability-filtered scheduled-wake discovery. The remaining 1D-b2b work must
   adopt concrete queue producers/executors in the app.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 442-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and the first 1D-b2b control-plane unit complete | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); `recording → captured → processing → ready/needsAttention`; D36–D41; 442-test package baseline | Slice 1D-b2b: concrete app producers/executors and queue adoption |

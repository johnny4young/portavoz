...
Status: **approved direction; Band 0 complete, Band 1 in progress (slices 1A and 1B complete)**
...
Current verified quality baseline: 415 package tests (13 model-gated) and 15 XCUITests
...
4. **Partially resolved in Band 1 slice 1B:** the meeting is now discoverable
   before capture, and audio survives empty captions or later derivation
   failure. Captured-state installation still spans independent transactions;
   atomic finalization, Unit of Work, jobs, and launch recovery remain.
...
7. **Partially resolved in Band 1 slices 1A and 1B:** schema v6 contains the
   complete `audioAsset` row shape and constraints, and new recordings reserve
   typed pending assets before capture. `meeting.audioDirectory` remains the
   product read path; metadata finalization and reader parity remain, and the
   migration deliberately performs no filesystem backfill.
...
Slice 1B implements steps 2–4 using the current final `channel.caf` paths so
the reservation exactly matches the as-built writer. Atomic `.partial`
publication and finalized metadata belong to slice 1C; the distinction avoids
describing the target path shape as already shipped.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 419-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A and 1B complete | Additive v6 migration; real-v5 scratch migration; typed assets/records; atomic `beginRecording`; pending per-channel reservations; guarded provisional rollback; `recording → captured → processing → ready/needsAttention` reconciliation; D36/D37 boundaries; 419-test package baseline | Slice 1C: publish/finalize channel files atomically and install the captured snapshot through one Unit of Work |

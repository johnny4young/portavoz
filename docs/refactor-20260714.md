...
Current verified quality baseline: 759 package tests (13 model-gated), zero
strict-lint violations across 275 Swift source files, and 31 XCUITest cases
...
**Next program gate:** 6B2 may add only the first bounded Apple transport
vertical: encrypted inline/asset record codecs, persisted CKSyncEngine state, explicit
account/consent and initial-seed policy, plus retry/tombstone/conflict tests.
...
2. **6B in progress — private CloudKit adapter vertical:** keep StorageKit as
   ...
   - **6B2 next:** add encrypted inline CKRecord values plus an
     encrypted-by-default CKAsset fallback for oversized meetings, persist
     CKSyncEngine delegate/state and exact in-flight generations independently,
     and characterize initial opt-in, account loss, retry, partial failure,
     tombstone, replay, and restart behavior. Audio remains out.
...
| Band 6 — Platform expansion | In progress — 6A and 6B1 complete (D92–D93) | Schema v14 content-free local/ack generation state plus exact-generation text-first aggregate projection; deterministic codec; atomic validated remote replay; local-derivation preservation; live/live deferral; deletion priority; immutable-collision and echo fences; no CloudKit/network/iOS behavior | 6B2 encrypted inline/asset CKRecord adapter plus persisted CKSyncEngine/account/retry/replay state; no iOS shell yet |
...
7. **Partially resolved by D92/D93:** local mutation admission, explicit
   ...
   live/live deferral, and deletion priority are fixed. CKRecord inline/asset
   thresholds, server-conflict replay, exact in-flight persistence, account
   transitions, retries, and CKSyncEngine state recovery remain 6B2 decisions.

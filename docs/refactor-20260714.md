...
Status: **approved direction; Bands 0–2 complete, Band 3 in progress; slices 3A–3F complete and 3G next**
...
Current verified quality baseline: 617 package tests (13 model-gated) and 20 XCUITest cases
...
**Slice 3G is next:** migrate the remaining meeting-content network operations
through the shared gateway, beginning with OpenAI-compatible summary generation
and then explicit Gist/GitHub/Linear publishing. Each vertical must define
its own operation, classification, consent, and meeting identity while retaining
the current explicit invocation and fallback/error semantics. Later slices add
the privacy receipt, diagnostics/signposts, typed error adoption, and sandbox
spike.

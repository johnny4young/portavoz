...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline, Band 4B health optimization, Band 4C bounded lexical retrieval, Band 4D semantic-scale measurement, Band 4E semantic adapter optimization, and Band 4F stateless waveform optimization complete; Band 4G Spotlight delivery measurement is next**
...
**Next slice:** Band 4G measures Spotlight full-rebuild delivery, mutation
bursts, failure recovery, and pending-work visibility against disposable
1k/10k/100k meeting metadata before replacing its compatibility counter with
an outbox consumer. Detail, lexical, semantic, and waveform paths pass, so view
decomposition, `DatabasePool`, vector storage, and derived-audio caching remain
...
7. **4G next:** measure Spotlight delivery/backlog behavior before replacing its current
...
| Band 4 — Detail/performance | In progress; 4A measurement, 4B health optimization, 4C bounded lexical retrieval, 4D isolated semantic scale, 4E exact adapter optimization, and 4F stateless waveform optimization complete (D79–D84) | Reproducible Release/app/audio reports; health p95 9.94 ms at 5k; first content 91.87 ms; exact/lexical p95 30.99/66.89 ms at 100k; semantic wall/CPU p95 90.22/91.26 ms; 55.9-minute waveform first wall/CPU 109.25/94.81 ms and repeat p95 70.11/71.33 ms; exact-result, replacement, and resource guards | 4G: measure Spotlight delivery/backlog before an outbox consumer; keep detail decomposition, `DatabasePool`, sqlite-vec, and derived-audio caches evidence-gated |

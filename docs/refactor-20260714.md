...
Status: **approved direction; Bands 0–2 complete, Band 3 next at slice 3A**
...
Current verified quality baseline: 596 package tests (13 model-gated) and 20 XCUITest cases
...
Execution starts with **slice 3A — summary generation provenance**. Add typed
start/finish persistence around one characterized summary workflow and commit
the immutable summary plus its `generationRunID` atomically. Record provider,
model/revision, configuration and input fingerprints, language, outcome,
timing, and safe metrics without storing duplicate meeting content. Preserve
cache reuse, pivot translation, failure policy, and visible output exactly.
Only after that vertical contract is proven should later slices adopt refine,
post-capture, Companion, and other generated artifacts, followed by egress,
...
| Band 3 — Provenance/privacy | Next — slice 3A | Schema-v6 `generationRun` envelope and egress target are defined | Adopt provenance atomically in one summary workflow, then expand by generated-artifact kind |

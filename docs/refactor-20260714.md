...
Status: **approved direction; Band 0 complete, Band 1 in progress (slices 1A, 1B, and 1C complete)**
...
Current verified quality baseline: 425 package tests (13 model-gated) and 15 XCUITests
...
4. **Partially resolved in Band 1 slices 1B and 1C:** the meeting is now discoverable
   ...
   install in one Unit of Work. Durable jobs and launch recovery remain.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 425-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A, 1B, and 1C complete | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; one captured Unit of Work; `recording → captured → processing → ready/needsAttention`; D36–D38; 425-test package baseline | Slice 1D: idempotent processing jobs and launch reconciliation for lifecycle plus staging files |

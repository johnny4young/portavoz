...
Current storage schema: `v9`

Current verified quality baseline: 708 package tests (13 model-gated), zero
strict-lint violations across 259 Swift source files, and 27 XCUITest cases
...
**Next slice:** Band 5C adds local, exportable correction feedback for that
overview claim before widening the pattern to decisions, action items, or
Companion cards. Detail decomposition, `DatabasePool`, vector storage,
...
3. **5C next:** add local correction/unsupported feedback for the overview
   claim, define its `.portavoz` export semantics, and only then extend the
   typed pattern to decisions, action items, or cards.
...
| Band 5 — Evidence/people | In progress — 5A and 5B complete (D86/D87) | Schema v8 canonical people/aliases plus schema v9 typed overview claims and ordered segment links; explicit identity confirmation; exact E-tag admission; source-revision, tombstone, and deletion fail-closed semantics; portable bundle remapping; transcript/audio navigation; and Core/Storage/Intelligence/UI plus EN/ES coverage | 5C: add local, exportable correction feedback before extending evidence to decisions, action items, or Companion cards |

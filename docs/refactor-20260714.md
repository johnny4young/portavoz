...
Status: **approved direction; Bands 0–2 complete, Band 3 in progress; slices 3A–3G-b complete; Sequoia stabilization units 1–4 complete; 3H resumes after the remaining stabilization work**
...
Current verified quality baseline: 652 package tests (13 model-gated), zero strict-lint violations across 244 Swift source files, and 21 XCUITest cases passing in both default and forced-Spanish runs

Owners: project maintainers and coding agents working under `AGENTS.md`

## Jul 16 Sequoia stabilization interrupt

Before slice 3H, field feedback is being resolved as independently shippable
...
The reported Refine failure and the Homebrew-versus-DMG installation difference
remain in this interrupt.
...
**Slice 3H is next:** assemble a local privacy receipt from existing
content-free provenance and egress declarations. It must answer whether meeting
content left the device without copying transcript, prompt, summary, notes, or
action-item text into diagnostics. Later slices add diagnostics/signposts,
typed error adoption, and the sandbox spike.
...
| Band 3 — Provenance/privacy | In progress — slices 3A–3G-b complete (D62–D69) | Manual/post-refine, durable post-capture, and external-audio import summary attempts, accepted refined transcripts, and generated durable Companion cards persist content-free terminal provenance. Durable success shares the job's lease/revision fence; import success shares the immutable summary transaction after the required aggregate commits; Refine success shares accepted transcript publication; Companion success shares Stop or current-revision card replacement and records actual BYOK fallback plus conservative destination-scope facts. Companion question-only, OpenAI-compatible complete-summary, Gist document, GitHub action-item, and Linear action-item requests cross one content-free policy port with operation-specific consent and canonical endpoint validation before URLSession. No-attempt/cache/discard/stale/no-card paths create no orphaned success; failure, cancellation, mismatch, rollback, real worker/import/Refine/Stop/Card Store paths, and direct meeting-content network-bypass prevention are characterized | Slice 3H: assemble a local content-free privacy receipt |

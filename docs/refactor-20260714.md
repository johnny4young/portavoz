...
Current verified quality baseline: 911 package tests (13 model-gated), zero
strict-lint violations across 329 Swift source files, and 38 XCUITest cases
...
**Next macOS conformance gate:** extract calendar-backed speaker naming and
local-provider discovery that still coordinate attendee evidence or provider
probing in SwiftUI.
...
10. **Resolved by D98–D106 for adopted interfaces:** resident menu-bar reads use one scoped
    ...
    model, and encrypted-storage capabilities remain in composition. Speaker
    naming and provider discovery are the next audited macOS boundaries before

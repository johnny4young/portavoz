...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline, Band 4B health optimization, Band 4C bounded lexical retrieval, Band 4D semantic-scale measurement, and Band 4E semantic adapter optimization complete; Band 4F waveform measurement is next**
...
Current verified quality baseline: 682 package tests (13 model-gated), zero
...
**Next slice:** Band 4F measures waveform generation, repeat-hit behavior,
memory, and invalidation against real scratch audio before selecting a durable
content-addressable cache. The detail, lexical, and semantic paths pass, so
view decomposition, `DatabasePool`, vector storage, and chapter caching remain
...
5. **4E complete:** stream semantic BLOBs and rowids, score in place with
   ...
   90.22/91.26 ms.
6. **4F next:** measure waveform hit/miss behavior with real scratch audio before selecting
...
| Band 4 — Detail/performance | In progress; 4A measurement, 4B health optimization, 4C bounded lexical retrieval, 4D isolated semantic scale, and 4E exact adapter optimization complete (D79–D83) | Reproducible Release/app reports; health p95 9.94 ms at 5k; first content 91.87 ms; exact/lexical p95 30.99/66.89 ms at 100k; semantic wall/CPU p95 90.22/91.26 ms, down 72.3%/72.2%; exact-ranking and resource guards | 4F: measure waveform generation/repeat hits before caching; keep detail decomposition, `DatabasePool`, sqlite-vec, and Spotlight outbox evidence-gated |

...
Status: **approved direction; Band 0 complete, Band 1 in progress (slice 1A complete)**
...
7. **Partially resolved in Band 1 slice 1A:** schema v6 contains the complete
   `audioAsset` row shape and constraints, but `meeting.audioDirectory` remains
   the only runtime audio reference; no `AudioAsset` domain/read path exists
   yet and the migration deliberately performs no filesystem backfill.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 415-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slice 1A complete | Atomic additive v6 migration; lifecycle/revision mapping; complete constrained table shapes; v5 migration, bundle-compatibility, and invalid-state fixtures; copied real-v5 scratch migration with legacy truth preserved and zero FK violations; D36 rollback boundary | Slice 1B: persist the `recording` shell and pending assets before capture behind a Strangler boundary |

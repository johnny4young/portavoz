...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline, Band 4B health optimization, Band 4C bounded lexical retrieval, and Band 4D semantic-scale measurement complete; Band 4E semantic adapter optimization is next**
...
Current verified quality baseline: 679 package tests (13 model-gated), zero
strict-lint violations across 253 Swift source files, and 25 XCUITest cases
...
**Next slice:** Band 4E removes semantic adapter amplification through streamed
rows, allocation-free Accelerate dot products, and bounded top-k retention,
then reruns the same matrix. sqlite-vec or a persisted vector layout remains
unselected until that after report still misses 100 ms. The detail and lexical
paths pass, so view decomposition, `DatabasePool`, and chapter caching also
...
5. **4E next:** stream semantic rows, score BLOB bytes with Accelerate, retain
   only top-k, and compare against the tracked 325.41 ms p95 before selecting
   sqlite-vec or moving vectors out of segment rows.
6. Measure waveform hit/miss behavior with real scratch audio before selecting
   a content-addressable cache.
7. Measure Spotlight delivery/backlog behavior before replacing its current
   compatibility invalidation with an outbox consumer.
...
| Band 4 — Detail/performance | In progress; 4A measurement, 4B health optimization, 4C bounded lexical retrieval, and 4D isolated semantic scale complete (D79–D82) | Reproducible Release/app reports; health p95 9.94 ms at 5k; first content 91.87 ms; exact/lexical p95 30.99/66.89 ms at 100k; semantic wall/CPU p95 325.41/328.43 ms with 8.50 MiB incremental footprint; correctness and resource-counter guards | 4E: optimize streamed/vectorized bounded semantic top-k and rerun; keep sqlite-vec, detail decomposition, waveform caching, Spotlight outbox, and storage complexity evidence-gated |

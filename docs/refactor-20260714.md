...
Current verified quality baseline: 573 package tests (13 model-gated) and 19 XCUITest cases
...
8. 🔄 Move pure product policies out of IntegrationsKit. Next, characterize and
   relocate the meeting-review cluster (`ChapterExtractor`, `PlaybackRanges`,
   `SummarySections`, and `VoiceHue`) to an inward application/domain boundary,
   update all consumers, and add a dependency guard. Keep external formats,
   egress, calendar, and RAG adapters in IntegrationsKit.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 573-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Complete — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and 1D-b2b | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); atomic snapshot/initial-job handoff, immediate detail navigation, and Shortcut parity (D43); `recording → captured → processing → ready/needsAttention`; 449-test Band 1 baseline retained by the current 573-test package suite | None; preserve these invariants while ApplicationKit owns the launch workflow |
| Band 2 — Application layer | In progress — slices 2A, 2B, 2C, 2D, 2E, 2F, 2G, 2H, 2I, 2J, 2K, 2L, 2M, and 2N | ApplicationKit contract/rules and storage-independent Library updates; StorageKit, IntelligenceKit, TranscriptionKit, and DiarizationKit ratchets; characterized lifecycle, regeneration, import/export, refine, Start/Stop, and launch recovery workflows; one per-window Library state/action/effect owner; explicit-region Library observations with partial failure isolation; private app adapters; fifteen architecture rules; direct model, observation, and workflow coverage; Detail/Insights/Spotlight broad invalidation retained only as explicit migration seams (D44–D54) | Move the meeting-review policy cluster out of IntegrationsKit and guard the narrower boundary |

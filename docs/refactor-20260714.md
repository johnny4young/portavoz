...
Current verified quality baseline: 699 package tests (13 model-gated), zero
strict-lint violations across 259 Swift source files, and 26 XCUITest cases
...
**Next slice:** Band 5B implements the smallest typed claim-evidence vertical.
Start with one durable summary claim, bind its evidence to the source transcript
revision and concrete segment IDs, and navigate to transcript/audio. Do not add
a generic EAV artifact table or widen the pattern to every generated surface
until stale-input, deletion, export, and feedback semantics pass. Detail
...
2. **5B next:** add one typed summary-claim evidence link with source-revision
   fences and transcript/audio navigation. Characterize stale or deleted
   evidence before extending the pattern to decisions, action items, or cards.
...
| Band 5 — Evidence/people | In progress — 5A complete (D86) | Schema v8 canonical people/aliases, nullable observed-speaker links, exact candidate-only lookup, explicit atomic create/link, duplicate-name preservation, `Me` exclusion, bundle stripping, Refine non-inheritance, off-MainActor voice-gallery reads, temp-store biometric isolation, and Core/Storage/Application/UI plus EN/ES coverage | 5B: implement one source-revision-fenced typed summary-claim evidence link and transcript/audio navigation |

...
Current verified quality baseline: 834 package tests (13 model-gated), zero
strict-lint violations across 302 Swift source files, and 35 XCUITest cases
...
1. **Partially resolved through Band 6 slice 6C4:** `AppServices` remains the
   composition root and still owns business/platform adapters, but Library,
   Insights, Meeting Detail, and the resident menu bar now have route/window/
   scene-scoped models behind narrow clients. They receive storage-independent
   scoped updates, Meeting Detail mutations no longer coordinate Store/
   lifecycle calls from SwiftUI, and the menu bar no longer queries Store or
   EventKit from its view. Whole-library Markdown backup now has one process-
   scoped application owner over a coherent StorageKit snapshot and injected
   document/filesystem adapters. Ask and first-run eligibility remain explicit
   macOS shell convergence seams.
...
**Next program gate:** while the product remains macOS-only, 6C6 moves first-
run eligibility, local Settings ledger reads, and upcoming-meeting brief
coordination behind narrow ApplicationKit contracts. It must preserve first-
run suppression for existing libraries, exact local counts, related meetings,
open commitments, and EventKit's no-prompt behavior while removing the next
Store-shaped values and capability coordination from presentation.
The later 6D
...
3. **6C — macOS consent/status surface:** expose opt-in, account readiness,
   ...
   - **6C6 next:** move first-run eligibility, local Settings ledger reads, and
     upcoming-meeting brief coordination behind narrow application contracts,
     preserving existing-library onboarding suppression, exact local counts,
     related meetings/open commitments, and calendar privacy behavior.
...
10. **Partially resolved by D98–D100:** resident menu-bar reads now use one scoped
    ...
    Ask now uses one workflow plus window/process-scoped presentation owners.
    First-run eligibility, Settings ledger reads, and meeting briefs remain the
    next macOS shell convergence seams before the deferred iOS target.

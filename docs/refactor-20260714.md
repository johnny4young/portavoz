...
Current verified quality baseline: 891 package tests (13 model-gated), zero
strict-lint violations across 325 Swift source files, and 37 XCUITest cases
...
**Next macOS conformance gate:** run one repository-wide dependency and
workflow audit. Remove only cross-capability product policy that still bypasses
ApplicationKit; preserve direct construction in executable composition, live
capture, diagnostics, and benchmark harnesses. The later 6D gate remains the
...
10. **Resolved by D98–D104 for adopted interfaces:** resident menu-bar reads use one scoped
    ...
    lifetime supervision. A final repository-wide macOS conformance audit
    precedes the deferred iOS target.

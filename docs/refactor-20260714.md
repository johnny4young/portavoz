...
default. Private bytes enter a private `0600` sibling through one POSIX
descriptor and synchronize with `fsync`; no Foundation descriptor bridge is
used. Complete protection is then applied and verified before one same-volume
atomic rename, so partial or unprotected content never occupies the reader-
visible path.

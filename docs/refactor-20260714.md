...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline, Band 4B health optimization, and Band 4C bounded lexical retrieval complete; Band 4D semantic-scale measurement is next**
...
Current verified quality baseline: 675 package tests (13 model-gated), zero
strict-lint violations across 252 Swift source files, and 25 XCUITest cases
...
**Next slice:** Band 4D measures brute-force semantic cosine latency, CPU, and
memory at 1k/10k/50k/100k embedded segments. sqlite-vec or a persisted vector
layout remains unselected until that exact path misses a budget. The detail and
lexical paths now pass, so view decomposition, `DatabasePool`, and chapter
caching also remain deliberately unselected.
...
4. **4D next:** measure brute-force semantic cosine latency, CPU, and memory at
   the same embedded-corpus sizes before selecting sqlite-vec or moving vectors
   out of segment rows.
5. Measure waveform hit/miss behavior with real scratch audio before selecting
   a content-addressable cache.
6. Measure Spotlight delivery/backlog behavior before replacing its current
...
| Band 4 — Detail/performance | In progress; 4A measurement, 4B health optimization, and 4C bounded lexical retrieval complete (D79–D81) | Reproducible before/after Release and native-app reports; arbitrary-overlap conservation; health p95 9.94 ms at 5k; first content 91.87 ms with zero hangs; exact/lexical p95 30.99/66.89 ms at 100k; BM25 equivalence, multi-term relevance, complete-context, and long-query-fallback characterizations | 4D: measure semantic cosine latency/CPU/memory before vectors; keep detail decomposition, waveform caching, Spotlight outbox, and storage complexity evidence-gated |

...
Current verified quality baseline: 458 package tests (13 model-gated) and 17 XCUITest cases
...
3. Extract recording, stop, recovery, refine, import, and regeneration use
   cases one at a time.
4. Reduce `AppServices` to dependency composition.
5. Introduce feature models through the Strangler pattern.
6. Replace `libraryVersion` with scoped observations.
7. Move pure product policies out of IntegrationsKit.
8. Decide whether placeholder Kits gain a boundary or are removed.

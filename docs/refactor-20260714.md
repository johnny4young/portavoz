...
Status: **approved direction; Band 0 runtime execution in progress**
...
Current verified quality baseline: 409 package tests (13 model-gated) and 15 XCUITests
...
4. Stop-time persistence occurs after capture and across several independent
   transactions. Audio can exist without a discoverable meeting if captions
   are empty or a later step fails.
...
7. `meeting.audioDirectory` is the only durable audio reference. There is no
   implemented `AudioAsset` domain record or table yet.
...
9. Summary-language defaults differ among recording, import, and regeneration.
...
| Initial meeting/cast/transcript writes must agree | Unit of Work | One StorageKit transaction installs the captured snapshot. |
...
3. Atomically publish valid files from `.partial` to final names.
4. Install the captured meeting, speakers, live segments, notes, and Companion
   cards through one database Unit of Work.
5. Enqueue idempotent refine/diarization/summary/index jobs.
6. Navigate immediately to the captured meeting.

### Recovery

At launch, reconcile `recording`, `processing`, leased jobs, and `.partial`
assets. Recovery must be safe to run repeatedly. It may resume work or mark a
meeting `needsAttention`; it may never silently delete usable audio.

## Target storage evolution

The current schema remains v5 until Band 1 lands. Proposed migrations are
additive first; destructive cleanup happens only after the new read path has
shipped and migration fixtures are green.

### Schema v6 — durability, policy, and provenance
...
```swift
enum TranscriptLanguagePolicy: Sendable, Equatable {
    case preserveOriginalPerSegment
    case force(Language) // advanced, explicit user action only
}

enum SummaryLanguagePolicy: Sendable, Equatable {
    case appDefault
    case meetingDominant
    case fixed(Language)
}
```
...
- `preserveOriginalPerSegment` is the default.
...
- Summary, Companion, translated captions, and other generated artifacts use
  an independently configured output language.
- A global default and a per-meeting override are both persisted.
...
Current execution status: scopes 1, 2, 4, and 5 are implemented. Scope 3 and
its mixed-language/output-language acceptance evidence remain.
...
3. Introduce typed transcript and summary language policies with a single
   persisted default and per-meeting override.
...
- ⏳ mixed ES/EN segments remain in the spoken language after refine;
- ⏳ summary language follows the configured policy without mutating transcript;
- ⏳ all Band 0 validation is green. Slice 0A requires build, package tests,
  lint, and documentation checks; EN/ES XCUITests remain the full-band gate
  when the language/UI path changes.
...
2. Persist the meeting shell and pending assets before capture.
3. Finalize files atomically and install captured state through one Unit of
   Work.
4. Add idempotent processing jobs and launch recovery.
...
1. Add `ApplicationKit` and enforce its dependency direction.
2. Extract recording, stop, recovery, refine, import, and regeneration use
   cases one at a time.
3. Reduce `AppServices` to dependency composition.
4. Introduce feature models through the Strangler pattern.
5. Replace `libraryVersion` with scoped observations.
6. Move pure product policies out of IntegrationsKit.
7. Decide whether placeholder Kits gain a boundary or are removed.
...
|---|---|---|---|
| Band 0 — Integrity and truth | In progress — slice 0A strict truth boundary implemented | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore projection tests; 409 package tests (13 gated), SwiftLint 0, D34 English-docs guard | Typed transcript/summary language policy with one persisted default and per-meeting override |
| Band 1 — Indestructible recording | Not started | Current workflow characterized | Wait for Band 0 |
| Band 2 — Application layer | Not started | Target dependency graph defined | Wait for Band 1 durability contracts |
...
6. Whether `AudioAsset`, jobs, provenance, and outbox ship in one v6 migration
   or in smaller additive versions.

...
Current verified quality baseline: 560 package tests (13 model-gated) and 19 XCUITest cases
...
5. 🔄 Reduce `AppServices` to dependency composition. Slice 2K moves
   ...
   behavior remain in Meeting Detail (D52).
6. 🔄 Introduce feature models through the Strangler pattern. Next, extract a
   feature-scoped `LibraryModel` with explicit state/actions while retaining
   the current `libraryVersion` trigger as characterized compatibility; scoped
   observations land in the following independent slice.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 560-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Complete — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and 1D-b2b | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); atomic snapshot/initial-job handoff, immediate detail navigation, and Shortcut parity (D43); `recording → captured → processing → ready/needsAttention`; 449-test Band 1 baseline retained by the current 560-test package suite | None; preserve these invariants while ApplicationKit owns the launch workflow |
| Band 2 — Application layer | In progress — slices 2A, 2B, 2C, 2D, 2E, 2F, 2G, 2H, 2I, 2J, 2K, and 2L | ApplicationKit contract/rules; StorageKit, IntelligenceKit, TranscriptionKit, and DiarizationKit ratchets; all trash workflows, Meeting Detail regeneration, external-audio and `.portavoz` aggregate import/export, reviewable refine draft/apply, durable recording Start/Stop, and launch recovery adopted; recipe-scoped immutable summaries; typed storage/filesystem/preferences/provider/processor/progress/runtime/lifecycle/activity/document ports; staged import rollback/compensation; canonical bundle attachments; complete bundle write Unit of Work and read-consistent export projection; explicit refine cancellation; revision-fenced acceptance; pre-source atomic reservation; source-start and launch evidence reconciliation; direct live-lane isolation; audio-first Stop fallbacks; atomic captured snapshot/job admission; expired-lease-first no-ML recovery; worker ordering; private app adapters; bypass, failure-policy, expiry, reuse/pivot, history, conservation, language, release, cancellation, stale-write, security, rollback, ordering, and real-adapter coverage (D44–D52) | Introduce `LibraryModel` as the first feature-scoped state owner while retaining current UI, navigation, accessibility, and broad reload behavior |

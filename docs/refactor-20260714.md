...
Current verified quality baseline: 971 package tests (13 model-gated), zero
...
use encrypted values; oversized payloads use a complete-protection,
backup-excluded CKAsset staging file whose contents CloudKit encrypts by
default. One POSIX descriptor creates a private `0600` sibling; complete
protection and backup exclusion are applied while the sibling is empty, then
that same descriptor writes and synchronizes the private bytes with `fsync`.
No Foundation reopen or late protection mutation occurs. Size and protection
are verified before one same-volume atomic rename, so partial or unprotected
content never occupies any path.
...
engine, account request, entitlement, network path, or UI (D94).

**Band 6B2B complete:** a separate complete-protected IntegrationsKit store now
...
`0600` files. Account loss pauses; account switch resets old account metadata;
...
StorageKit without app runtime or network composition (D95).
...
2. **6B complete — private CloudKit adapter vertical:** keep StorageKit as
   ...
   - **6B2A complete (D94):** one deterministic private-zone record stores the
     portable envelope in encrypted values or, above the conservative 512 KiB
     policy, a protected encrypted-by-default CKAsset. The encrypted digest,
     strict identity/format/storage validation, existing-record reuse, and
     saved deletion tombstone are covered without a container or runtime.
   - **6B2B complete (D95):** complete-protected account/consent/seed, opaque
     engine/system-field, exact attempt, bounded retry, replay-cursor, and
     deferred-fetch state survives restart outside schema v14. Account loss and
...
- large envelopes use a protected, backup-excluded local CKAsset staging file
  and round-trip without adding audio;
...
- the JSON snapshot contains no meeting text and exact envelope bytes use
  complete-protection, backup-excluded `0600` files;

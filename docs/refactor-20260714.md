...
Current verified quality baseline: 793 package tests (13 model-gated), zero
strict-lint violations across 286 Swift source files, and 31 XCUITest cases
...
**Next program gate:** 6C2 may compose the private container/engine only behind
capability detection and account-scoped macOS consent, then expose the already
fixed lifecycle through bilingual Settings. It must not add an iOS shell
or imply audio sync. Real-model evidence quality and next-release checks remain
...
3. **6C — macOS consent/status surface:** expose opt-in, account readiness,
   ...
   - **6C2 next:** add the real fail-closed macOS platform adapter, entitlements,
     process composition, account-change wake, and bilingual Settings surface.
...
| Band 6 — Platform expansion | In progress — 6A through 6C1 complete (D92–D96) | Schema v14 content-free local/ack generation state; deterministic text-first projection and atomic replay; encrypted private-zone inline/protected-asset tombstone records; separate complete-protected account/consent/seed, opaque engine/system-field, exact attempt/retry/cursor/deferred-replay state; thin injected CKSyncEngine callback boundary; platform-neutral zero-touch lifecycle with explicit enable/seed/retry/pause/remove-device and truthful status; no app runtime/network/UI/iOS behavior | 6C2 macOS capability/entitlement/runtime composition and bilingual Settings surface; no audio sync or iOS shell yet |
...
7. **Transport and lifecycle semantics resolved by D92/D93/D94/D95/D96:** local mutation admission, explicit
   ...
   and remove-device policy are fixed. App runtime composition, capability and
   entitlement admission, and user-facing status/actions remain 6C2.

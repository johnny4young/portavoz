...
Current verified quality baseline: 882 package tests (13 model-gated), zero
strict-lint violations across 322 Swift source files, and 37 XCUITest cases
...
**Next macOS conformance gate:** characterize the process-scoped post-capture
executor as one durable application workflow. Move cross-capability job policy
behind ApplicationKit ports while leaving concrete engines, filesystem access,
Shortcut invocation, and real-time capture in app composition. The later 6D
gate remains the in-person iOS recorder shell from `docs/IOS.md`, reusing the
proven codec/lifecycle without macOS call-capture assumptions. Real-account/
two-Mac convergence, real-model evidence quality, and next-release checks
remain independent validation work. Detail
...
| Band 6 — Platform expansion | In progress — 6A through 6C7 complete (D92–D103) | Schema v14 content-free local/ack generation state; deterministic text-first projection and atomic replay; encrypted private-zone inline/protected-asset tombstone records; separate complete-protected account/consent/seed, opaque engine/system-field, exact attempt/retry/cursor/deferred-replay state; thin CKSyncEngine callback boundary; platform-neutral zero-touch lifecycle; inert signed-capability-gated macOS container owner; process-scoped serialized actions and content-free wakes; exact Developer ID release admission; bilingual explicit consent/status/actions; resident menu-bar scoped read ownership; coherent whole-library Markdown backup with portable collision-safe names, typed partial results, and atomic non-replacing publication; one ApplicationKit Ask search/evidence/answer workflow with storage-independent, stale-safe macOS presentation state plus CLI/MCP reuse; process-scoped first-run and local-receipt owners; batched source-grounded meeting preparation; Core secret port plus outer PlatformKit Keychain/microphone adapters; one CLI composition surface whose meeting/Ask/MCP, file-analysis, persisted-refine, publication, local-voice, and pinned-model product workflows enter ApplicationKit while concrete adapters remain at the executable edge | Close the process-scoped post-capture durable-workflow seam on macOS; real-account/two-Mac field evidence remains independent, iOS stays deferred, and audio does not sync |
...
10. **Resolved by D98–D103 for adopted interfaces:** resident menu-bar reads use one scoped
    ...
    workflows. Process-scoped post-capture job execution is the remaining
    macOS conformance seam before the deferred iOS target.

...
Current verified quality baseline: 575 package tests (13 model-gated) and 19 XCUITest cases
...
`PortavozCore`, eleven Kit libraries, the macOS app, the CLI, and the package test
target.
...
flowchart TB
    ...
    APPLICATION["ApplicationKit\ncharacterized application workflows"]
    ...
    INTEGRATIONS["IntegrationsKit\nexternal adapters + remaining read/product policies"]
...
1. **Partially resolved through Band 2 slice 2N:** `AppServices` remains the
   composition root and still owns several business/platform adapters, but the
   Library now has one per-window model behind a narrow client and receives
   storage-independent scoped updates instead of coordinating Store calls from
   SwiftUI.
...
3. **Partially resolved in Band 2 slice 2N:** Library no longer consumes the
   global `libraryVersion` integer and its query families observe only their
   explicit GRDB regions. Detail, Insights, and Spotlight still consume the
   broad counter after unrelated writes and require independent parity slices.
...
10. Generated artifacts do not have a shared provider/model/input provenance
    envelope.
11. **Partially resolved through Band 2 slice 2P:** chapter extraction,
    playback ranges, summary sections, speaker hues, Insights scope/statistics,
    and local finding detection moved to ApplicationKit. `IntegrationsKit`
    still contains outbound adapters plus brief, reminder, and mirror
    product/read policies, so the boundary remains broader than its target.
...
7. ✅ Replace the Library's `libraryVersion` dependency with scoped
   ...
   regions; the app adapter maps and merges them. Detail, Insights, and
   Spotlight remain independent parity slices (D54).
8. 🔄 Move pure product policies out of IntegrationsKit. Slice 2O relocates the
   ...
   changing dashboard behavior (D56). Next, move `BriefRelevance`,
   `ReminderPolicy`, `MirrorStats`, and the calendar-neutral event contract
   inward as the final characterized local-policy cluster. Keep external
   formats, egress, EventKit, RAG, and MCP adapters in IntegrationsKit.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 575-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Complete — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and 1D-b2b | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); atomic snapshot/initial-job handoff, immediate detail navigation, and Shortcut parity (D43); `recording → captured → processing → ready/needsAttention`; 449-test Band 1 baseline retained by the current 575-test package suite | None; preserve these invariants while ApplicationKit owns the launch workflow |
| Band 2 — Application layer | In progress — slices 2A, 2B, 2C, 2D, 2E, 2F, 2G, 2H, 2I, 2J, 2K, 2L, 2M, 2N, 2O, and 2P | ApplicationKit contract/rules, storage-independent Library updates, four inward meeting-review policies, and three inward Insights policies; StorageKit, IntelligenceKit, TranscriptionKit, and DiarizationKit ratchets; characterized lifecycle, regeneration, import/export, refine, Start/Stop, and launch recovery workflows; one per-window Library state/action/effect owner; explicit-region Library observations with partial failure isolation; private app adapters; seventeen architecture rules; direct policy, model, observation, and workflow coverage; Detail/Insights/Spotlight broad invalidation retained only as explicit migration seams (D44–D56) | Move the remaining brief/reminder/mirror policy cluster and calendar-neutral event contract inward |

...
Status: **approved direction; Band 0 complete, Band 1 in progress (slices 1A, 1B, 1C, and 1D-a complete)**
...
Current verified quality baseline: 432 package tests (13 model-gated) and 15 XCUITests
...
4. **Partially resolved in Band 1 slices 1B–1D-a:** the meeting is now discoverable
   ...
   owner leases, retries, and terminal lifecycle derivation; app adoption and
   launch reconciliation remain.
...
4. ◐ Add idempotent processing jobs and launch recovery. Slice 1D-a now owns
   ...
   lease recovery (D39). Slice 1D-b still must adopt the queue in the app and
   reconcile `recording`/`processing` meetings plus staging files on launch.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 432-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A, 1B, 1C, and 1D-a complete | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; one captured Unit of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation (D39); `recording → captured → processing → ready/needsAttention`; D36–D39; 432-test package baseline | Slice 1D-b: app queue adoption and launch reconciliation for lifecycle, expired leases, and staging files |

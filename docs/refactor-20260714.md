...
Current verified quality baseline: 569 package tests (13 model-gated) and 19 XCUITest cases
...
1. **Partially resolved through Band 2 slice 2M:** `AppServices` remains the
   composition root and still owns several business/platform adapters, but the
   Library now has one per-window model behind a narrow client instead of
   coordinating those calls from SwiftUI.
...
3. **Partially resolved in Band 2 slice 2M:** a global integer
   `libraryVersion` still broadly invalidates Library, Detail, Insights, and
   Spotlight after unrelated writes, but Library reload/search state now has
   a version/query-fenced owner ready for a scoped observation adapter.
...
5. 🔄 Reduce `AppServices` to dependency composition. Slice 2K moves
   ...
   or platform calls directly (D53).
6. ✅ Introduce the first feature model through the Strangler pattern. Each
   ...
   private-write value snapshot, enum actions/effects, and stale reload/search
   fences while the current `libraryVersion` trigger remains characterized
   compatibility (D53).
7. 🔄 Replace `libraryVersion` with scoped observations. Next, introduce
   query-specific Library read models and GRDB observation, then retire only
   the Library trigger; keep Detail, Insights, and Spotlight for independent
   parity slices.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 569-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Complete — slices 1A, 1B, 1C, 1D-a, 1D-b1, 1D-b2a, and 1D-b2b | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation plus degradable cancellation and scheduled wake discovery (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); stale-safe atomic diarization/summary artifact completion (D41); process-scoped exact heartbeat/retry execution with optional-summary degradation and no polling (D42); atomic snapshot/initial-job handoff, immediate detail navigation, and Shortcut parity (D43); `recording → captured → processing → ready/needsAttention`; 449-test Band 1 baseline retained by the current 569-test package suite | None; preserve these invariants while ApplicationKit owns the launch workflow |
| Band 2 — Application layer | In progress — slices 2A, 2B, 2C, 2D, 2E, 2F, 2G, 2H, 2I, 2J, 2K, 2L, and 2M | ApplicationKit contract/rules; StorageKit, IntelligenceKit, TranscriptionKit, and DiarizationKit ratchets; characterized lifecycle, regeneration, import/export, refine, Start/Stop, and launch recovery workflows; one per-window Library state/action/effect owner; private app adapters; fifteen architecture rules; direct model and workflow coverage; broad Library invalidation retained only as an explicit migration seam (D44–D53) | Introduce query-specific Library read models and scoped GRDB observation; retire only the Library's `libraryVersion` trigger after parity |

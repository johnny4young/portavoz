...
Status: **approved direction; Band 0 complete, Band 1 next**
...
Current verified quality baseline: 413 package tests (13 model-gated) and 15 XCUITests
...
- Recording, rolling summary, import, and regeneration use the same app
  ...
  operation.
- Durable per-meeting transcript/summary defaults remain part of the schema-v6
  `meetingPreference` migration in Band 1; Band 0 deliberately leaves schema
  v5 unchanged.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, D35 independent language policies; 413 package tests (13 gated), SwiftLint 0, English-docs guard, 15-test XCUITest suite green in EN/ES | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | Next | Current workflow characterized; schema-v6 target and rollback boundary defined | Slice 1A: additive lifecycle/audio-asset/preference migration with fixtures |

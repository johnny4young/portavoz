...
Current verified quality baseline: 782 package tests (13 model-gated), zero
strict-lint violations across 284 Swift source files, and 31 XCUITest cases
...
**Next program gate:** 6C may compose the private container/engine only behind
capability detection and account-scoped macOS consent, with truthful local-only,
pending, synchronized, paused, retrying, and failed status plus explicit
initial-seed and disable/remove-device semantics. It must not add an iOS shell
...
| Band 6 — Platform expansion | In progress — 6A through 6B2 complete (D92–D95) | Schema v14 content-free local/ack generation state; deterministic text-first projection and atomic replay; encrypted private-zone inline/protected-asset tombstone records; separate complete-protected account/consent/seed, opaque engine/system-field, exact attempt/retry/cursor/deferred-replay state; thin injected CKSyncEngine callback boundary; no app runtime/network/UI/iOS behavior | 6C macOS capability/consent/status/runtime composition with explicit initial seed and disable semantics; no audio sync or iOS shell yet |
...
7. **Transport semantics resolved by D92/D93/D94/D95:** local mutation admission, explicit
   ...
   recovery are fixed. App runtime composition, capability/consent/status UX,
   a user-facing initial-seed action, and disable/remove-device policy remain 6C.

...
Current verified quality baseline: 435 package tests (13 model-gated) and 16 XCUITests
...
4. **Partially resolved in Band 1 slices 1B–1D-b1:** the meeting is now discoverable
   ...
   concrete app producers/workers and artifact-plus-job completion remain.
...
Slice 1D-b2 must resume derived `processing` work through concrete durable
producers/workers. Every recovery path remains safe to run repeatedly and may
...
4. ◐ Add idempotent processing jobs and launch recovery. Slice 1D-a owns
   ...
   including conservative ambiguity handling (D40). Slice 1D-b2 still must
   adopt concrete queue producers/workers in the app and commit generated
   artifacts with their job outcomes atomically.
...
|---|---|---|---|
| Band 0 — Integrity and truth | Complete — slices 0A and 0B | Typed `StorageError` decoding, zero random UUID fallbacks, live-meeting joins, delete/restore conservation, and D35 independent language policies retained by the 435-test package suite | None; preserve these invariants in every later band |
| Band 1 — Indestructible recording | In progress — slices 1A, 1B, 1C, 1D-a, and 1D-b1 complete | Additive v6 migration; real-v5 scratch migration; atomic pending reservations; D37 guarded rollback; staged CAF validation, SHA-256, media/level/health metadata, no-overwrite rename; captured/recovery Units of Work; typed immutable-key jobs with owner leases/retries/lifecycle derivation (D39); evidence-first launch reconciliation across audio roots and expired leases (D40); `recording → captured → processing → ready/needsAttention`; D36–D40; 435-test package baseline | Slice 1D-b2: concrete app producers/workers and atomic artifact-plus-job completion |

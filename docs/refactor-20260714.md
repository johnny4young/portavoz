...
Status: **approved direction; Bands 0 and 1 complete, Band 2 in progress through slice 2S**
...
Current verified quality baseline: 593 package tests (13 model-gated) and 20 XCUITest cases
...
3. **Partially resolved through Band 2 slice 2S:** Library, Insights, and
   Meeting Detail reads no
   ...
   contracts, and each route/window owns the matching state. Direct Meeting
   Detail mutations still increment the broad counter for Spotlight; moving
   those view bypasses behind its model client is slice 2T. Incremental
   Spotlight indexing remains coupled to Band 4 scale work.
...
10. 🔄 Replace Meeting Detail's `libraryVersion` read dependency. Slice 2S adds
    ...
    reload. Slice 2T moves the remaining direct detail mutations behind the
    model client. Spotlight stays independent for Band 4 (D59).
11. Decide whether placeholder Kits gain a boundary or are removed.

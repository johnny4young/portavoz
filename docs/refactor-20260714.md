...
Current verified quality baseline: 825 package tests (13 model-gated), zero
strict-lint violations across 298 Swift source files, and 33 XCUITest cases
...
**Next program gate:** while the product remains macOS-only, 6C5 gives AskView
and the resident command palette one ApplicationKit retrieval/answer workflow
plus storage-independent presentation models. It must preserve instant FTS,
local RAG fallback, citations, copy, and exact meeting/audio navigation while
removing Store, IntegrationsKit, and IntelligenceKit coordination from SwiftUI.
The later 6D
...
3. **6C — macOS consent/status surface:** expose opt-in, account readiness,
   ...
   - **6C5 next:** converge AskView and the resident command palette on one
     ApplicationKit retrieval/answer workflow and storage-independent feature
     state while preserving instant search, local answers, citations, copy, and
     exact seek navigation.
...
10. **Partially resolved by D98/D99:** resident menu-bar reads now use one scoped
    ...
    Ask and first-run eligibility remain the next macOS shell convergence seams
    before the deferred iOS target.

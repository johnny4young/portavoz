...
Current storage schema: `v10`

Current verified quality baseline: 723 package tests (13 model-gated), zero
strict-lint violations across 266 Swift source files, and 29 XCUITest cases
...
5. **5E next:** characterize action-item identity and extend the typed
   source-revision/evidence contract without coupling it to decision Markdown
   or Companion cards.
...
| Band 5 — Evidence/people | In progress — 5A through 5D complete (D86–D89) | Schema v8 canonical people/aliases, schema v9 typed overview claims/ordered links, schema v10 active-claim-fenced feedback, and schema v11 position-typed decision evidence/ordered nullable links; explicit identity confirmation; exact E-tag and provider-shape admission; source-revision, coordinate, deletion, text-erasing clear, translation, portable bundle remapping, and model/diagnostics isolation; per-claim and per-decision transcript/audio navigation plus reversible overview review; Core/Storage/Intelligence/UI and EN/ES coverage | 5E: characterize and add typed action-item evidence without broadening to Companion cards |

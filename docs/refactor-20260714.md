...
Current storage schema: `v12`

Current verified quality baseline: 731 package tests (13 model-gated), zero
strict-lint violations across 268 Swift source files, and 30 XCUITest cases
...
**Next slice:** Band 5D characterizes and adds typed evidence for summary
decisions before widening the pattern to action items or Companion cards.
Detail decomposition, `DatabasePool`, vector storage,
...
6. **5F next:** characterize Companion-card source identity and provenance
   without coupling it to summary or action-item evidence.
...
| Band 5 — Evidence/people | In progress — 5A through 5E complete (D86–D90) | Schema v8 canonical people/aliases, schema v9 typed overview claims/ordered links, schema v10 active-claim-fenced feedback, schema v11 position-typed decision evidence, and schema v12 identity-typed action-item evidence; explicit identity confirmation; exact E-tag and provider-shape admission; source-revision, coordinate/task-identity, deletion, text-erasing clear, translation, portable bundle remapping, and model/diagnostics isolation; per-overview, per-decision, and per-action transcript/audio navigation plus reversible overview review; Core/Storage/Intelligence/UI and EN/ES coverage | 5F: characterize Companion-card evidence as its own artifact contract |

...
Current verified quality baseline: 813 package tests (13 model-gated), zero
strict-lint violations across 294 Swift source files, and 32 XCUITest cases
...
1. **Partially resolved through Band 6 slice 6C3:** `AppServices` remains the
   ...
   EventKit from its view. Ask, whole-library Markdown backup, and first-run
   eligibility remain explicit macOS shell convergence seams.
...
**Next program gate:** while the product remains macOS-only, 6C4 moves the
whole-library Markdown backup behind one ApplicationKit workflow with injected
format/filesystem adapters, atomic per-file publication, typed partial results,
and no Store/IntegrationsKit reach-through from Settings SwiftUI. The later 6D
...
3. **6C — macOS consent/status surface:** expose opt-in, account readiness,
   ...
   - **6C4 next:** move whole-library Markdown backup behind one application
     workflow and injected format/filesystem adapters; Settings must retain the
     native folder panel and visible progress without coordinating Store reads.
...
10. **Partially resolved by D98:** resident menu-bar reads now use one scoped
    presentation owner and narrow observations. Whole-library Markdown backup,
    Ask, and first-run eligibility remain the next macOS shell convergence
    seams before the deferred iOS target.

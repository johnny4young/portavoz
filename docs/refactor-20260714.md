...
Status: **approved direction; Bands 0–3 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A measured baseline and Band 4B health optimization complete; Band 4C broad-OR selectivity is next**
...
**Next slice:** Band 4C improves broad OR candidate selectivity against the
tracked 100k-segment miss before any vector or segment-layout migration. The
detail path now passes its budget, so view decomposition, `DatabasePool`, and
chapter caching remain deliberately unselected.
...
3. **4C next:** improve broad OR retrieval selectivity before considering `sqlite-vec` or a
...
| Band 4 — Detail/performance | In progress; 4A measurement and 4B health optimization complete (D79/D80) | Reproducible before/after Release and native-app reports; arbitrary-overlap conservation; health p95 9.94 ms at 5k and 41.39 ms at 20k; first content 91.87 ms; zero measured hangs; unchanged 25-case UI baseline and screenshot | 4C: improve broad OR candidate selectivity before vectors; keep detail decomposition and storage complexity evidence-gated |

...
Status: **approved direction; Bands 0–5 complete; slices 3A–3K and Sequoia stabilization units 1–6 complete; Band 4A–4G measured scale work complete; Band 5A canonical people, 5B overview evidence, 5C local claim feedback, 5D decision evidence, 5E action-item evidence, and 5F role-separated Companion evidence complete; Band 6 in progress with 6A durable mutation journal, 6B1 portable aggregate/replay, 6B2 dormant encrypted CloudKit transport, 6C1–6C6 macOS lifecycle/read convergence, 6C7 executable/platform convergence, the D104 post-capture application boundary, D105 review-document/participant-voice convergence, D106 local-voice enrollment convergence, D107 calendar-backed speaker naming, D108 local-provider discovery, D109 Settings device-resource convergence, D110 reminder convergence, D111 Meeting Detail metadata convergence, D112 Meeting Detail audio convergence, D113 verified model lifecycle, D114 final macOS conformance, D115 private-iCloud receipt disclosure, D116 filesystem-capability-safe protected publication, D117 OSS/scoped-UI gates, and D118 current-SDK diagnostic closure complete; iOS remains deferred**
...
Current storage schema: `v14`

Current verified quality baseline: 973 package tests (13 model-gated), zero
strict-lint violations across 344 Swift source files, and 39 XCUITest cases
...
Later hardening keeps exact recovery comparisons and Refine operation-
fingerprint assembly in bounded, explicitly typed steps. This is a compiler-
compatibility constraint for the supported Sequoia Swift 6.2 lane, not a
change to equality or fingerprint order. High-cardinality characterization
fixtures use the same typed construction rule without changing their evidence.
SwiftUI constructors with overlapping
SDK defaults and presentation math spanning `CGFloat` and `Double` likewise
use explicit signatures and types so the same behavior compiles on Sequoia and
the latest SDK.
...
describe only the architecture that exists at the current commit. Unimplemented
direction remains in this document rather than appearing in the as-built view.
...
flowchart TB
    ...
    CORE["PortavozCore\nshared domain types, ports + UpcomingEvent"]
    PLATFORM["PlatformKit\nKeychain + microphone permission adapters"]
    MODEL["ModelStoreKit"]
    CAPTURE["AudioCaptureKit"]
    STT["TranscriptionKit"]
    DIAR["DiarizationKit"]
    INTEL["IntelligenceKit"]
    STORE["StorageKit\nGRDB + FTS5, schema v14"]
...
1. **Resolved for adopted macOS presentation reads and executable product
   commands through Band 6 slice 6C7:**
   `AppServices` remains the composition root and owns concrete platform
   adapters, while Library, Insights, Meeting Detail, Ask, first-run guidance,
   the local-data receipt, whole-library backup, the resident menu bar, and
   meeting preparation enter through scoped owners and storage-independent
   application contracts. SwiftUI no longer coordinates Store, EventKit,
   document, filesystem, or model work for those adopted paths. Core now owns
   only secret identifiers and a storage port; PlatformKit owns concrete
   Keychain and microphone authorization. CLI product commands share one
   platform/database composition, and meeting/Ask/MCP reads enter through
   ApplicationKit. File analysis, persisted refinement, export/publication,
   issue publication, local voice management, and pinned-model lifecycle now
   also enter ApplicationKit; command files retain only parsing and terminal
   presentation while concrete adapters remain at composition (D103).
...
### Additive schemas — implemented identity and future evidence
...
#### People — implemented in Band 5A / schema v8

```text
person(id, preferredName, createdAt, updatedAt, deletedAt)
personAlias(id, personID, normalizedAlias, source, confidence, createdAt, updatedAt, deletedAt)
speaker.personID NULLABLE
```

`speaker.personID` is indexed and uses `ON DELETE SET NULL`. A normalized alias
is unique only within one person; several people may own the same alias so
ambiguity remains representable. Lookup returns exact candidates and never
links by itself. One explicit user selection atomically creates/links or links
an existing person; the structural `Me` speaker is excluded. Voiceprints remain
encrypted local files with no schema-v8 person reference; they never enter
SQLite as plaintext or sync. Bundles strip canonical IDs, and Refine's fresh
observed speakers require fresh confirmation (D86).
...
**Slice 3H — local privacy receipts are complete (D75/D115).** Schema v7 persists
...
The same receipt now reads the schema-v14 journal's acknowledged generation:
only a positive acknowledgement discloses an encrypted private-iCloud copy,
and the disclosure never reverts after later edits or pause. Product copy does
not claim end-to-end encryption because that depends on the user's optional
Advanced Data Protection setting.
...
**Band 5A complete:** Core separates observed speakers from canonical people;
schema v8 adds duplicate-safe normalized aliases and nullable person links;
ApplicationKit separates candidate lookup from explicit create/link selection;
Meeting Detail offers the action only after a non-user name is accepted and
marks a confirmed link. `Me`, Calendar, VoiceGallery, bundles, and Refine all
retain their stricter D86 boundaries. VoiceGallery reads stay off MainActor,
and disposable UI fixtures treat the sensitive gallery as empty. Migration,
ambiguity, rollback, privacy, localization, and EN/ES UI tests protect the vertical.

**Band 5B complete:** the smallest typed claim-evidence vertical binds one
overview claim to a source transcript revision and ordered segment IDs, keeps
that evidence portable through bundles, and navigates to transcript/audio only
when the complete set is current. Schema v9, exact provider tags, deletion and
staleness semantics, migration, and EN/ES UI evidence protect D87 without a
generic EAV artifact table.

**Band 5C complete:** the overview claim now owns one explicit, reversible
current assessment: a bounded correction or an unsupported mark. Schema v10
keeps that mutable child separate from immutable generated Markdown and
evidence, fences writes to the newest cross-recipe summary, erases correction
text on clear while retaining a sync-ready tombstone, and carries feedback
only through explicit format-v1 `.portavoz` export/import. Provider prompts,
telemetry, privacy receipts, and support diagnostics do not consume it (D88).

**Band 5D complete:** built-in decision sections now own position-typed
evidence with exact provider shapes, rendered-coordinate validation, schema
v11 persistence, portable identity remapping, and fail-closed navigation
(D89).

**Band 5E complete:** each action item now owns task-identity-keyed immutable
evidence independent from its mutable checkbox. Schema v12, provider tags,
translation/bundle remapping, deletion handling, and per-task navigation
protect D90.

**Band 5F complete:** each Companion card now owns role-separated question and
answer evidence. Schema v13 persists ordered nullable links behind a card and
revision fence; exact closed/coalesced turns identify the question, while only
exact local-RAG citations identify answer support. Knowledge answers and pings
do not fabricate answer evidence. Bundles remap every identity and Meeting
Detail exposes both roles independently (D91). Band 5 is complete.

**Band 6A complete:** the user's separate product decision opened platform
expansion at its transport-independent seam. Schema v14 owns a content-free,
per-meeting local/acknowledged generation fence. Portable root, child,
feedback, and evidence mutations update it transactionally; migration does not
seed it, device-local derivations do not dirty it, and deletion state survives
physical purge (D92).

**Band 6B1 complete:** StorageKit now joins only the newest pending generation
to a versioned text-first aggregate, strips device-local state, validates and
replays a remote aggregate atomically, preserves matching local derivations,
and settles accepted trigger noise without echo. Live remote work waits behind
unsent local work; remote deletion wins that race but remains recoverable;
immutable summary collisions fail closed. IntegrationsKit owns deterministic
bytes, not CloudKit behavior (D93).

**Band 6B2A complete:** IntegrationsKit maps each portable envelope to one
deterministic private-zone `MeetingReplica`. Small payloads and their digest
use encrypted values; oversized payloads use a private CKAsset staging file
whose contents CloudKit encrypts by default. Content-free destination probes
apply and read back complete protection and backup exclusion independently;
only `EINVAL`/`ENOTSUP` can mark a key unavailable. Supported metadata is
applied while one private `0600` sibling is empty, then one POSIX descriptor
writes and synchronizes the private bytes with `fsync`. No Foundation reopen or
late protection mutation occurs. Exact size, owner-only permissions, and every
supported metadata key are verified before one same-volume atomic rename, so
partial content never occupies the final path.
Matching records retain system fields, malformed records fail closed,
and deletion saves an encrypted tombstone. The codec creates no container,
engine, account request, entitlement, network path, or UI (D94/D116).

**Band 6B2B complete:** a separate owner-only IntegrationsKit store now
persists content-free account-scoped consent/seed, opaque CKSyncEngine state,
record system fields, exact attempts, bounded retries, per-meeting/source replay
cursors, and protected fetched deferrals. Exact payloads stay in validated
`0600` files with destination-supported protection metadata. Account loss
pauses; account switch resets old account metadata;
late callbacks settle only exact generations; physical CKRecord deletion is
metadata-only; and a thin injected delegate forwards domain replay to
StorageKit without app runtime or network composition (D95/D116).

**Band 6C1 complete:** a platform-neutral IntegrationsKit lifecycle now owns
zero-platform local-only launch, explicit enable/seed/retry/pause/remove-device,
account changes, and truthful status. It observes only StorageKit's content-free
pending count and the protected D95 transport state. Account loss preserves
consent and exact attempts; account switch requires new consent; retry keeps the
exact payload/generation plus attempt history; pause/remove cannot delete
meeting or remote content (D96).

**Band 6C2 complete:** one inert fail-closed macOS adapter owns the private
container/engine behind exact signed capabilities and D96 consent. A process-
scoped model serializes explicit actions and content-free wakeups; bilingual
Settings exposes truthful state without implying audio sync. Local/test builds
remain no-cloud, while Developer ID distribution requires an unexpired matching
production CloudKit/APNs profile before notarization and after extraction
(D97).

**Band 6C3 complete:** the resident macOS menu bar now renders one
storage-independent `MenuBarModel` instead of coordinating one-shot Store and
EventKit reads in SwiftUI. A bounded three-meeting StorageKit observation and
the independent latest-action observation merge behind an app adapter; partial
failure preserves healthy state and reopening converges to current local truth
without a global invalidation counter (D98).

**Band 6C4 complete:** whole-library Markdown backup now enters one
ApplicationKit workflow over a newest-first read-consistent StorageKit snapshot
and injected canonical-document/filesystem ports. Required aggregate corruption
is isolated per meeting, General-summary selection preserves released behavior,
portable names include existing disk state and canonical collisions, and each
complete file publishes through an atomic same-directory temporary write plus a
non-replacing move. Typed partial results and progress belong to one process-
scoped model; Settings retains only the native panel and localized presentation
(D99).

**Band 6C5 complete:** `AskMeetings` is now the single ApplicationKit workflow
for instant search, hybrid evidence, and optional local answers across AskView,
the resident command palette, CLI, MCP, and meeting-brief evidence. Typed
storage-independent results preserve citations when generation is unavailable;
per-window and process-scoped presentation models own cancellable tasks and
stale-result fences; exact seek, Markdown copy, and window reopening remain
unchanged (D100).

**Band 6C6 complete:** first-run eligibility, the local Settings receipt, and
upcoming-meeting preparation now enter through narrow ApplicationKit contracts.
One process-scoped welcome owner suppresses setup for an existing live library
without making model readiness a prerequisite and transfers its single sheet
to another active main window when its host closes. Independent exact local metrics
degrade per source and describe network behavior as explicit action/opt-in
rather than an invented byte count. Meeting preparation reuses Ask evidence,
one batched latest-summary projection, independently loaded open commitments,
and source-indexed optional synthesis; EventKit remains a no-prompt app adapter,
and agenda actions cannot race the meeting-selection route (D101). The verified
gate is 856 package tests (13 model-gated), zero strict-lint violations across
311 Swift source files, and 37 real-app XCUITest cases per locale.

**Band 6C7 first unit complete:** `PortavozCore` contains stable secret
identifiers and a Sendable storage port but no Security import. The new
Core-only `PlatformKit` owns the this-device-only Keychain implementation and
microphone authorization; app and CLI composition roots inject one adapter set
into async credential management, encrypted voice stores, publishing, and
explicit Companion configuration. Onboarding delegates microphone and calendar
permission work instead of calling platform frameworks from SwiftUI.
`QueryMeetingLibrary` owns normalized bounded list/detail/search/open-item reads,
while StorageKit returns current meeting content plus its latest live General
summary from one SQLite read snapshot. Meeting, Ask, and MCP commands format
only ApplicationKit values. Model-heavy and mutation commands share the
composition surface but retain their concrete executable pipelines. The
verified package gate is 863 tests (13 model-gated), zero strict-lint violations
across 318 Swift source files, plus disposable CLI/MCP output/protocol smoke
(D102).

**Band 6C7 second unit complete:** standalone transcription, diarization,
summarization, persisted quality refinement, meeting document publication,
pending action-item publication, local voice identity, and pinned-model
lifecycle now enter narrow ApplicationKit workflows. Command bodies retain
only syntax, explicit warnings, and terminal rendering. `CLIComposition` owns
one process platform/database dependency set; `CLIProductAdapters` owns the
concrete filesystem, streaming fingerprints, models, Store, providers,
integrations, and encrypted voice adapters. Saved external summarization
persists meeting truth before gateway egress, Refine keeps the same atomic
revision fence, publication prepares credentials only after local
meeting/document/pending-work admission, and model-download callbacks are
ordered and drained before a terminal result. Capture diagnostics and benchmark
harnesses retain deliberate direct construction. The verified package gate is
882 tests (13 model-gated) with zero strict-lint violations across 322 Swift
source files (D103).

**Post-capture application boundary complete:**
`ApplicationKit.ProcessPostCaptureJobs` owns serial supported-job execution,
lease heartbeats, exact operation fingerprints, transcript cleanup and
attribution, dependent-job admission, summary attempt provenance, retries,
cancellations, terminal action and engine-release timing, and persisted wake
selection through narrow ports. StorageKit remains the atomic artifact and
lease authority. The app retains one process supervisor and concrete
recording-path, model, preference, Shortcut, fixture, and content-free telemetry
adapters. Focused cases characterize mixed-language first-pass recovery,
diarization-to-summary publication, unavailable-provider retry, exhausted
optional summary, superseded input, publication lease loss, typed diagnostics,
and no-poll scheduling (D104).

The repository-wide dependency and workflow audit found Meeting Detail
document and participant-voice policy bypasses. D105 closes them: in-memory
Markdown/PDF preparation now loads one coherent current detail through
ApplicationKit and returns canonical bytes plus the released suggested
filename; explicit secret-Gist publication uses the same document workflow,
with the app adapter owning lazy credential resolution and the gateway-backed
publisher. A separate workflow owns degradable participant-voice suggestions,
duplicate-offer admission, one-to-one matching, and explicit persistence while
the app adapter retains encrypted gallery access, audio ranges, transient
embeddings, and model loading. The route-owned feature model owns document and
voice-memory actions/effects plus one-shot suggestion state. SwiftUI retains
only explicit gestures, native presentation, chips, and localized results.
Eleven focused workflow/model cases and two architecture ratchets establish
the boundaries without changing released behavior or privacy receipts.

**Local voice application boundary complete:** Settings and Onboarding now
enter `ManageLocalVoiceIdentity` for supplied or freshly captured samples. The
workflow bounds capture requests, requires at least four seconds of finite
audio, owns capture/extraction/persistence order and destructive outcomes, and
persists only after successful extraction. App composition retains exact raw
versus echo-cancelled microphone behavior, guaranteed shutdown, verified model
loading, transient embedding extraction, encrypted device-only storage, cache
invalidation, and disposable-test isolation. Seven workflow cases plus one
architecture ratchet cover mode forwarding, first-listen reuse, invalid input,
capture/extraction failure, destructive failure, and zero partial success
(D106).

**Calendar-backed speaker naming complete:**
`ApplicationKit.SuggestMeetingSpeakerNames` owns coherent meeting admission,
eligible-label selection, optional candidate coordination, and independent
complete-token verification of untrusted proposals with typed transcript or
calendar evidence. EventKit and Foundation Models remain app adapters; the
route model owns loading, suggestions, persistence-aware acceptance removal,
and visible failure effects. Five workflow cases, expanded shared-filter and
model tests, one architecture ratchet, and a bilingual real-app unnamed-speaker
screenshot establish D107.

**Local-provider discovery complete:**
`ApplicationKit.DiscoverLocalSummaryProviders` evaluates one capability-neutral
profile through a pure policy shared by Settings and Onboarding. Ollama is
ready only when its running service exposes a nonempty model not classified as
OCR, embedding, reranking, or Whisper work;
Apple Foundation Models and hardware-eligible MLX retain their exact priority
and typed setup guidance. `ConfigureInitialSummaryProvider` writes only an
absent preference, re-checks at a main-actor guarded write, and reports whether
that write won, so an explicit selection is never overwritten. The app adapter
retains concrete capability,
content-free localhost, hardware/disk, DTO, and UserDefaults work while
presentation localizes typed results. Eleven focused cases and one architecture
ratchet establish D108.

**Settings device-resource convergence complete:**
`ApplicationKit.LoadAudioInputOptions`, `ManageRecordingStorage`, and
`ManageRememberedVoices` now own the capability-neutral operations used by
Settings. App adapters retain Core Audio enumeration, resumable recording-root
migration and marker publication, and encrypted participant-voice access.
Recording progress is drained in order before completion, the marker changes
only after migration succeeds, same-root aliases are safe no-ops, remembered-
voice projections omit embeddings, and destructive failures remain visible.
Eight workflow cases, two storage regressions, and one architecture ratchet
establish D109 without changing native folder selection, preferences, or
released Settings behavior.

**Pre-meeting reminder convergence complete:**
`ApplicationKit.ResolveMeetingReminder` owns lead-window admission,
session-deduplication input, deterministic earliest-event selection, and display
minutes derived from one sampled time. Disabled reminders do not query the
calendar. A private app adapter retains UserDefaults, the clock, EventKit
mapping, and utility-priority projection; the process controller retains only
its timer, floating panel, and recording route. Five workflow cases, an
unsorted-policy regression, and one architecture ratchet establish D110 without
changing the released reminder surface.

**Meeting Detail metadata convergence complete:**
`ApplicationKit.SuggestMeetingReviewMetadata` owns eligibility, bounded output
admission, known-recipe mapping, cancellation, and independent best-effort
degradation for title, structure, and chapter suggestions over one review
projection. The app adapter retains Foundation Models capability and concrete
generators. The route model owns request fencing, one-shot completion,
cancellation retry, chapter-label state, and explicit dismissal; SwiftUI renders
inert suggestions only. Failed title persistence keeps the chip visible and no
longer requests a false search reindex. Five workflow cases, three route-model
cases, and one architecture ratchet establish D111 without changing automatic
availability, chapter fallback, or explicit acceptance behavior.

**Meeting Detail audio convergence complete:**
`ApplicationKit.PrepareMeetingPlayback`, `CompressMeetingAudio`, and
`ExportMeetingAudioClip` now own the operation order for channel resolution,
synchronized playback, bounded waveform/filter derivation, failure-safe
compression, and current-file clip export. The app adapter retains configured
recording-root lookup and concrete codec work. The route model owns one-shot
preparation, directory-scoped cancellation retry, playback invalidation,
compression progress, and explicit effects; independent review revisions
cannot consume the audio load attempt. SwiftUI retains rendering, transport
controls, and the native save panel. Compression refuses to overwrite an
existing canonical output, queries live filesystem size, keeps every original until every output
verifies, and removes generated work after failure or cancellation. Focused
workflow, route-model, transcoder, localization, architecture, and bilingual UI
coverage establish D112 without changing released audio behavior.

**Verified model lifecycle complete:** `ModelStoreKit` now returns typed
installation evidence only after every artifact in the exact pinned descriptor
passes SHA-256. One process-scoped `VerifiedModelLifecycle` coalesces checks,
caches only successful evidence by ID and revision, keeps missing/corrupt
results re-checkable, serializes same-descriptor mutations, and makes a
superseded waiter resolve current evidence instead of returning stale state.
Verified artifacts publish from a destination-volume sibling with one atomic
rename or replacement; cancellation remains honest across that commit point.
The macOS composition shares it across Settings, MLX provider resolution,
Import, durable processing, diagnostics, speech engines, and participant-voice
extraction. Disposable UI composition uses an empty root. Filename and
size-only readiness probes have been removed, and Settings exposes an honest
asynchronous verification state (D113).

**Final macOS conformance audit complete:** the exact SwiftPM internal graph is
now an executable ratchet for every production target, and a repository-wide
presentation ratchet rejects concrete capability construction, direct Store
calls, and adapter-framework imports in every SwiftUI `View` type. The audit
found no additional production boundary violation after the verified-model
extraction. Direct construction remains intentionally confined to executable
composition, nonvisual live capture and dictation owners, diagnostics, and
disposable benchmark harnesses (D114). The later 6D gate remains the
in-person iOS recorder shell from `docs/IOS.md`, reusing the proven
codec/lifecycle without macOS call-capture assumptions. Real-account/two-Mac
convergence, real-model evidence quality, and next-release checks remain
independent validation work. Detail
decomposition, `DatabasePool`, vector storage, derived-audio caching, and a
Spotlight outbox remain deliberately unselected.
...
1. **5A complete (D86):** add schema-v8 canonical people/aliases and one
   explicit Meeting Detail create/link vertical. Exact aliases remain
   candidates, duplicate names are preserved, biometrics remain separate,
   bundles strip person IDs, and Refine requires fresh confirmation.
2. **5B complete (D87):** add one typed overview claim with source-revision
   fences, ordered segment links, portable bundle remapping, and transcript/
   audio navigation. Stale, tombstoned, physically deleted, unknown-tag, and
   invalid-aggregate paths fail closed before any broader artifact adoption.
3. **5C complete (D88):** add one local correction/unsupported assessment for
   the active overview claim, keep it separate from generated output and every
   model/diagnostics path, erase text on clear, and preserve it only through an
   explicit `.portavoz` export.
4. **5D complete (D89):** add position-typed evidence for explicitly
   classified built-in decision sections, validate rendered Markdown
   coordinates and exact provider shapes, preserve valid translation/bundle
   coordinates with fresh IDs, and navigate each decision to transcript/audio.
5. **5E complete (D90):** key a separate immutable evidence aggregate to each
   durable action-item ID, preserve it across checkbox completion, enforce the
   shared source-revision/live-segment contract, remap fresh identities through
   translation and bundles, and navigate its exact transcript/audio source.
6. **5F complete (D91):** key one evidence aggregate to each Companion card,
   separate ordered question and answer roles, retain exact closed/coalesced
   question rows, admit answer sources only from exact local-RAG citations,
   persist nullable links in schema v13, remap every identity through bundles,
   and navigate both roles independently without coupling them to summary or
...
1. **6A complete (D92) — transport-independent mutation journal:** schema v14
   stores only meeting identity, monotonic local/acknowledged generations,
   change time, and deletion state. Forty-eight transactional triggers cover
   portable roots and typed relations, including evidence-only replacement;
   null-safe predicates reject unchanged whole-row saves. Initial seeding is
   explicit, acknowledgement is generation-fenced, and deletion survives
   physical purge. Paths, embeddings, generation links, canonical people,
   jobs, receipts, audio, model state, secrets, and voiceprints are excluded.
   No transport, account behavior, conflict resolver, iOS target, or UI lands.
2. **6B complete — private CloudKit adapter vertical:** keep StorageKit as
   the mutation authority.
   - **6B1 complete (D93):** exact-generation text-first envelopes include all
     live portable history, strip device-local state, encode deterministically,
     and replay atomically with explicit live/live deferral, deletion priority,
     immutable-collision rejection, local-derivation preservation, and echo
     suppression. There is no CloudKit import or network behavior.
   - **6B2A complete (D94/D116):** one deterministic private-zone record stores the
     portable envelope in encrypted values or, above the conservative 512 KiB
     policy, an encrypted-by-default CKAsset staged through mandatory private
     durable atomic publication plus destination-supported protection metadata.
     The encrypted digest, strict identity/format/storage validation, existing-
     record reuse, and saved deletion tombstone are covered without a container
     or runtime.
   - **6B2B complete (D95/D116):** owner-only account/consent/seed, opaque
     engine/system-field, exact attempt, bounded retry, replay-cursor, and
     deferred-fetch state survives restart outside schema v14. Complete
     protection and backup exclusion are required whenever the actual
     destination supports them. Account loss and
     switches, partial failure, stale callbacks, privacy tombstones, physical
     record deletion, N/N+1 work, and a thin injected delegate are characterized.
     No app composition creates a container or performs network work; audio is out.
3. **6C — macOS consent/status surface:** expose opt-in, account readiness,
   progress, retry, and disable/remove-device semantics only after 6B is
   deterministic. The user must be able to distinguish local-only, pending,
   synchronized, paused, and failed without reading logs.
   - **6C1 complete (D96):** a CloudKit-free lifecycle owns those actions and
     phases through injected account/driver protocols. It performs no platform
     call without prior consent and cannot delete local or remote meeting data.
   - **6C2 complete (D97):** one inert fail-closed macOS platform actor, exact
     local/distribution entitlements, process-scoped serialized lifecycle,
     content-free account/push/journal/retry wakeups, and bilingual Settings
     expose the private text-first transport without weakening opt-in.
   - **6C3 complete (D98):** the resident menu-bar scene consumes one scoped
     presentation model. ApplicationKit owns storage-independent updates,
     StorageKit observes only three live meeting roots plus latest open items,
     and the app adapter retains calendar access and partial-failure isolation.
   - **6C4 complete (D99):** whole-library Markdown backup owns one read-
     consistent application workflow, injected renderer/filesystem ports,
     portable collision-safe names, typed partial completion, and atomic non-
     replacing publication. One process-scoped model owns progress; Settings
     retains the native folder panel without Store/IntegrationsKit reach-through.
   - **6C5 complete (D100):** AskView, the resident command palette, CLI, MCP,
     and meeting-brief evidence use one ApplicationKit search/evidence/answer
     workflow. Storage-independent feature state preserves instant search,
     local answers, citations, copy, exact seek, and evidence-only degradation;
     task cancellation and generation fences prevent stale panel publication.
   - **6C6 complete (D101):** first-run eligibility, exact local Settings
     receipt metrics, and upcoming-meeting preparation use narrow application
     contracts and scoped owners. Restored windows share one welcome decision;
     unavailable metrics remain distinct from zero; Ask evidence, batched live
     summaries, open commitments, and source-indexed synthesis remain
     navigable; EventKit retains its no-prompt app boundary.
   - **6C7 complete (D102–D103):** one reusable CLI composition surface injects
     PlatformKit Keychain/microphone adapters and serves bounded meeting, Ask,
     MCP, file-analysis, persisted-refine, document/action publication,
     local-voice, and pinned-model workflows through ApplicationKit. Core and
     capability modules construct no Keychain. Product commands keep parsing
     and presentation only; concrete adapters retain files, models, Store,
     providers, integrations, and streaming fingerprints. Capture diagnostics,
     output contracts, benchmark harnesses, local MCP behavior, and the
     non-sandboxed release boundary remain unchanged.
4. **6D — iOS recorder shell:** add the Xcode iOS target only after the shared
   codec and state machine are proven on macOS. Preserve D11: in-person
   microphone recorder and Companion, never third-party call capture. Reuse
   the Kits that compile under iOS and keep voice enrollment device-local.
5. **6E — optional audio and collaboration ladder:** CKAsset audio remains
   explicit per meeting and separately budgeted. CKShare and richer remote
   control follow D12 only after private single-user sync is field-proven.

XPC isolation remains evidence-gated rather than part of platform expansion.
A local memory graph follows people and evidence; it does not precede them.

Acceptance for 6A:

- upgrading queues nothing and causes no network or account side effect;
- every portable aggregate mutation commits its dirty state atomically;
- device-local-only writes remain clean;
- acknowledging an in-flight generation cannot hide a newer edit;
- typed evidence-only replacement queues the owning meeting;
- soft delete, restore, and physical purge leave the correct durable state;
- no CloudKit import, speculative SyncKit target, iOS target, or visible
  feature is claimed.

Acceptance for 6B1:

- only the newest pending generation can label a portable snapshot;
- every live summary version and typed evidence artifact round-trips while
  paths, audio, embeddings, provenance, canonical people, jobs, receipts,
  secrets, and voiceprints remain absent;
- remote replay validates before one atomic replacement and suppresses echo;
- matching local paths, people links, embeddings, and provenance survive;
- unsent local live work is not overwritten by a live remote aggregate;
- remote deletion wins that race without physically purging the meeting;
- immutable identity reuse, partial/foreign evidence, and invalid aggregate
  relations fail before replacement;
- no CloudKit import, entitlement, account request, network behavior, UI, or
  iOS target is present.

Acceptance for 6B2A:

- one meeting maps to one deterministic private custom-zone record;
- small envelopes and their digest use encrypted values;
- large envelopes use a private `0600` local CKAsset staging file, apply and
  verify complete protection and backup exclusion when the destination supports
  each key, and round-trip without adding audio;
- downloaded matching records are reused so their system fields remain
  available to the later sender;
- invalid type, zone, format, storage, checksum, or envelope identity fails
  before replay;
- deletion is an encrypted tombstone record save, never a CKRecord delete;
- no container, account request, CKSyncEngine, entitlement, network behavior,
  status UI, or iOS target is present.

Acceptance for 6B2B:

- the JSON snapshot contains no meeting text and exact envelope bytes always use
  durable atomic `0600` files, with complete protection and backup exclusion
  applied and verified when supported;
- consent and initial seeding are explicit/account-scoped, account loss pauses,
  and switching accounts clears old engine/system-field/replay/seed state while
  preserving device-owned outgoing attempts;
- exact outgoing generations, retry deadlines, system fields, replay cursors,
  and fetched deferrals survive restart and fail closed on corruption;
- partial record success is independent and a late callback cannot erase newer
  local or deferred work;
- only a saved encrypted tombstone may delete domain content; physical CKRecord
  deletion invalidates transport metadata only;
- CKSyncEngine callbacks forward conflict/replay work through the coordinator to
  StorageKit and cannot own domain replacement rules;
- no app-composed container, account request, entitlement, sync network side
  effect, status UI, audio sync, or iOS target is present.
...
| Band 5 — Evidence/people | Complete — 5A through 5F (D86–D91) | Schema v8 canonical people/aliases, schema v9 typed overview claims/ordered links, schema v10 active-claim-fenced feedback, schema v11 position-typed decision evidence, schema v12 identity-typed action-item evidence, and schema v13 card-identity-keyed question/answer Companion evidence; explicit identity confirmation; exact E-tag/provider-shape/RAG-citation admission; source-revision, coordinate/task/card identity, deletion, text-erasing clear, translation, portable bundle remapping, and model/diagnostics isolation; per-overview, per-decision, per-action, and per-Companion-role transcript/audio navigation plus reversible overview review; Core/Storage/Intelligence/UI and EN/ES coverage | None; preserve these completed evidence invariants during Band 6 |
| Band 6 — Platform expansion | In progress — macOS convergence complete through D118 | Schema v14 content-free local/ack generation state with permanent receipt disclosure of acknowledged encrypted private-iCloud copies; deterministic text-first projection and atomic replay; encrypted private-zone inline/protected-asset tombstone records with capability-probed protection metadata over mandatory private `0600` durable atomic publication; separate owner-only account/consent/seed, opaque engine/system-field, exact attempt/retry/cursor/deferred-replay state under the same publication primitive; thin CKSyncEngine callback boundary; platform-neutral zero-touch lifecycle; inert signed-capability-gated macOS container owner; process-scoped serialized actions and content-free wakes; exact Developer ID release admission; bilingual explicit consent/status/actions; resident menu-bar scoped read ownership; coherent whole-library Markdown backup with portable collision-safe names, typed partial results, and atomic non-replacing publication; one ApplicationKit Ask search/evidence/answer workflow with storage-independent, stale-safe macOS presentation state plus CLI/MCP reuse; process-scoped first-run and local-receipt owners; batched source-grounded meeting preparation; Core secret port plus outer PlatformKit Keychain/microphone adapters; one CLI composition surface whose product workflows enter ApplicationKit; one ApplicationKit durable post-capture state machine with app-owned concrete adapters and process supervision; coherent macOS/terminal meeting-document preparation and explicit publication; degradable participant-voice suggestions and explicit memory; bounded local voice enrollment with app-owned microphone/model/encrypted-storage adapters; verified calendar-backed speaker naming with route-owned state; shared typed local-provider discovery; Settings device resources; reminder and Meeting Detail metadata policy; route-owned Meeting Detail audio preparation, failure-safe compression, and current-file clip export; process-scoped catalog-verified model readiness; exact dependency-graph and SwiftUI presentation ratchets; conservative scoped UI evidence; and current-SDK warning-safe SwiftUI, MLX, and AVAudioConverter boundaries | Preserve macOS conformance; real-account/two-Mac field evidence remains independent, iOS stays deferred, and audio does not sync |
...
7. **Transport, lifecycle, and macOS composition resolved by D92–D97:** local mutation admission, explicit
   initial seeding, generation-aware acknowledgement, purge-surviving deletion,
   portable aggregate membership, exact-generation encoding, storage replay,
   live/live deferral, deletion priority, deterministic record identity,
   encrypted inline/asset placement, existing-record reuse, saved CloudKit
   tombstones, server-conflict replay, exact in-flight persistence, account
   transitions, bounded retries, fetched deferral, and CKSyncEngine state
   recovery, lifecycle phases, account transitions, retry, initial-seed, pause,
   and remove-device policy are fixed. The inert private-container owner,
   signed capability/profile admission, serialized app runtime, content-free
   wakeups, and bilingual user-facing status/actions are fixed by 6C2. Real
   production-account convergence remains a field gate; iOS remains 6D.
...
10. **Resolved by D98–D112 for adopted interfaces:** resident menu-bar reads use one scoped
    presentation owner and narrow observations; whole-library Markdown backup
    now uses one process-scoped application owner, one coherent Store snapshot,
    injected document/filesystem adapters, and atomic non-replacing publication;
    Ask uses one workflow plus window/process-scoped presentation owners;
    first-run eligibility, exact Settings receipts, and meeting preparation now
    use narrow application contracts and scoped owners. Core secret storage and
    microphone authorization now enter through PlatformKit. CLI meeting, Ask,
    MCP, file-analysis, persisted-refine, publication, local-voice, and
    pinned-model product behavior shares one composition plus ApplicationKit
    workflows. Process-scoped post-capture job execution now enters one
    ApplicationKit workflow while app composition retains concrete adapters and
    lifetime supervision. Meeting Detail Markdown/PDF/Gist actions share the
    coherent application document workflow and keep native presentation at the
    edge. Participant voice memory has a separate application workflow with
    concrete biometric adapters at composition. Local enrollment now shares one
    application contract for CLI, Settings, and Onboarding while microphone,
    model, and encrypted-storage capabilities remain in composition.
    Calendar-backed speaker naming, provider discovery, Settings device
    resources, reminder resolution, Meeting Detail metadata, and Meeting Detail
    audio preparation/compression/export now own policy and operation order
    behind ApplicationKit. D113 centralizes verified model readiness and D114
    freezes the complete production graph plus the SwiftUI presentation
    boundary. macOS convergence is complete before the deferred iOS target.

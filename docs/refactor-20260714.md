...
Status: **approved direction; Bands 0–2 complete, Band 3 in progress; slices 3A–3E complete and 3F next**
...
**Slice 3F is next:** introduce the first `DataEgressGateway` vertical path for
explicitly enabled Companion BYOK. It must distinguish localhost from remote
destinations, carry meeting/operation/classification/consent metadata without
duplicating content, preserve question-only transfer and on-device fallback,
and establish an architecture test against direct meeting-content network
bypass. Later slices extend the gateway to remaining integrations, then add the
privacy receipt, diagnostics/signposts, typed error adoption, and sandbox spike.
...
| Band 3 — Provenance/privacy | In progress — slices 3A–3E complete (D62–D66) | Manual/post-refine, durable post-capture, and external-audio import summary attempts, accepted refined transcripts, and generated durable Companion cards persist content-free terminal provenance. Durable success shares the job's lease/revision fence; import success shares the immutable summary transaction after the required aggregate commits; Refine success shares accepted transcript publication; Companion success shares Stop or current-revision card replacement and records actual BYOK transfer/fallback facts. No-attempt/cache/discard/stale/no-card paths create no orphaned success; failure, cancellation, mismatch, rollback, and real worker/import/Refine/Stop/Card Store paths are characterized | Slice 3F: introduce `DataEgressGateway` for explicitly enabled Companion BYOK |

...
| **M8 — Dev moat** | 🚧 **Servidor MCP local funcionando** (`portavoz-cli mcp`, stdio JSON-RPC; tools: list/search/transcript/summary/action items — sesión E2E verificada por pipes). Registro: `claude mcp add portavoz -- portavoz-cli mcp`. Falta: RAG local (`ask`), export GitHub/Linear, App Intents. |

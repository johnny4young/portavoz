...
| **M13b — Meeting health + Recipes** | **Núcleo implementado (jul 2026)**: panel de salud por reunión 100% local (`MeetingHealth` puro: talk-time por hablante con share, preguntas ¿…?/…?, interrupciones por solape ≥ 0.5 s, monólogo más largo; `MeetingHealthView` en el detalle con ≥2 hablantes). Falta: Recipes avanzadas; auto-detección de tipo de reunión → Recipe; brief pre-reunión desde calendario (patrón Granola Briefs) | Panel de salud por reunión ✅; Recipe sugerida correcta en ≥ 3 tipos de reunión |

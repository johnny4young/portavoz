...
DiarizationKit are admitted only with real vertical use cases. Forty-six
...
empty/degraded/failed state, and last-healthy-section preservation. The current
gate is 813 package tests (13 gated), zero strict-lint violations across 294
Swift source files, and 32 XCUITest cases passing in both English and Spanish.
Throwaway
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 813-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 6 — Platform expansion | In progress — 6A through 6C3 complete; private macOS text-first sync is composed behind explicit consent and exact release capabilities, and the resident menu bar consumes one scoped live read owner (D92–D98) | Schema v14 remains the mutation authority; deterministic text-first envelopes replay atomically and private-zone records store encrypted inline/protected-asset tombstones. Separate complete-protected transport state preserves exact delivery, while a platform-neutral lifecycle owns zero-touch local-only launch, explicit consent/seed/retry/pause/remove-device, account transitions, and truthful status. One process-scoped macOS owner composes the private CKContainer/CKSyncEngine behind signature/profile checks and a bilingual surface; local/test builds remain no-cloud. The menu bar now observes bounded local truth without Store/EventKit reach-through. Next: 6C4 moves whole-library Markdown backup behind one application workflow before 6D; real-account field evidence remains independent and audio does not sync |

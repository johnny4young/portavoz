...
The current gate is 609 package tests (13 gated), zero
strict-lint violations across 231 Swift source
...
Companion remains the next Band 3 producer (D62–D65).
...
13. **Generated success is one atomic fact:** a successful model run and the immutable artifact it produced commit together; durable outputs share the same owner lease and source-revision fence as their processing job. Optional imported derivation begins only after its required aggregate commits and can never roll that aggregate back. A reviewable Refine keeps success ephemeral until the user accepts it; Apply then links the run and every replacement segment in the existing source-revision transaction. Failed/cancelled attempts are separate terminal records, while cache hits, unavailable providers, silent channels, discarded drafts, and inputs rejected before a model attempt create no synthetic run. Provenance stores identities, fingerprints, configuration, timing, outcome, and aggregate metrics but never meeting content (D62–D65).
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the 609-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |

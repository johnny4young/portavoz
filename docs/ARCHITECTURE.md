...
into the current schema v11), `DatabaseQueue`, and the MIT/no-GPL dependency set; sqlite-vec and
...
current set may focus the exact transcript row and audio playhead. Translation
...
| `StorageKit` | `MeetingStore` over GRDB 7 + FTS5, schema v11: the released meeting/transcript/summary/search/trash behavior plus schema-v6 durability, schema-v7 privacy receipts, schema-v8 canonical people, schema-v9 typed overview claims with ordered nullable segment evidence, schema-v10 current claim feedback, and schema-v11 position-typed decision evidence with ordered nullable segment links. New overview and decision evidence is validated and revision-stamped atomically with its immutable summary; decision positions must match the canonical rendered Markdown outline. Feedback writes are separately fenced to the newest cross-recipe overview claim and clear physically erases correction text before tombstoning. Summary observation includes overview/decision/link/feedback tables while transcript availability resolves against the independent core projection. Exact aliases remain candidate-only and ambiguity-preserving. Recording, recovery, import/export, Refine, Companion, Spotlight, job, provenance, egress, strict-decoding, live-root, and plain-BLOB vector contracts remain as detailed in the matching specs; `.portavoz` strips canonical people but carries and remaps portable overview and decision evidence plus current overview feedback. Outbox and per-meeting preferences remain unconsumed |
...
and per-bullet EN/ES navigation evidence. The current gate is 723 package tests
(13 gated), zero strict-lint violations across 266 Swift source files, and 29
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 723-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 5 — Evidence and people | In progress — 5A through 5D complete: explicit canonical people, source-revision-fenced overview evidence/navigation, reversible local claim feedback, and position-typed per-decision evidence/navigation (D86–D89) | Canonical identity preserves ambiguity and biometric separation. Typed overview and decision provenance admits only exact request tags, persists ordered same-meeting links in schema v9/v11, fails closed when stale/unavailable, and remaps through bundles. Schema v10 keeps the current overview assessment separate, active-claim fenced, text-erasing on clear, and explicitly portable. Action-item and Companion-card evidence remains; 5E starts with action items |

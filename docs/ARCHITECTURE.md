...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 683-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 4 — Detail and scale | In progress — 4A records the baseline; 4B brings 5k first content to 91.87 ms; 4C brings 100k lexical p95 to 66.89 ms; 4D measures semantic cost; 4E brings semantic wall/CPU p95 to 90.22/91.26 ms; 4F brings a 55.9-minute waveform repeat p95 to 70.11/71.33 ms wall/CPU (D79–D84) | Detail, lexical, exact semantic, and waveform budgets pass without a pool, cache, extension, sidecar, or schema change. Spotlight delivery/backlog remains the next separate evidence gate |

...
into the current schema v10), `DatabaseQueue`, and the MIT/no-GPL dependency set; sqlite-vec and
...
| `StorageKit` | `MeetingStore` over GRDB 7 + FTS5, schema v10: the released meeting/transcript/summary/search/trash behavior plus schema-v6 durability, schema-v7 privacy receipts, schema-v8 canonical people, schema-v9 typed overview claims with ordered nullable segment evidence, and schema-v10 current claim feedback. New claims are validated and revision-stamped atomically with their immutable summary; feedback writes are separately fenced to the newest cross-recipe claim and clear physically erases correction text before tombstoning. Summary observation includes claim/link/feedback tables while transcript availability resolves against the independent core projection. Exact aliases remain candidate-only and ambiguity-preserving. Recording, recovery, import/export, Refine, Companion, Spotlight, job, provenance, egress, strict-decoding, live-root, and plain-BLOB vector contracts remain as detailed in the matching specs; `.portavoz` strips canonical people but carries and remaps portable claim evidence plus its current feedback. Outbox and per-meeting preferences remain unconsumed |
...
route-model mutations, localization, and reversible EN/ES visual coverage. The current
gate is 714 package tests (13 gated), zero strict-lint violations across 262
Swift source files, and 28 XCUITest cases passing in both English and Spanish. Throwaway
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 714-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 5 — Evidence and people | In progress — 5A, 5B, and 5C complete: explicit canonical people, source-revision-fenced overview evidence/navigation, and reversible local claim feedback (D86/D87/D88) | Canonical identity preserves ambiguity and biometric separation; typed overview provenance admits only exact request tags, persists ordered same-meeting links in schema v9, fails closed when stale/unavailable, and remaps through bundles. Schema v10 keeps the current human assessment separate, active-claim fenced, text-erasing on clear, and explicitly portable. Broader decision/action/card evidence remains |

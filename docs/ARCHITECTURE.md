...
model discovery remain direct because they carry no meeting content. Gist,
GitHub Issue, and Linear Issue publishing remain on their characterized direct
paths until 3G-b (D68).
...
against direct OpenAI-compatible summary transport. The current gate is 628
package tests (13 gated), zero
...
- `IntegrationsKit` retains outbound adapters and implements Core's shared
  egress-policy port; Companion BYOK and OpenAI-compatible summary generation
  are the as-built verticals. Meeting-review and Insights read policy plus
...
publishing remains for 3G-b (D68).
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the 628-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |

...
into the current schema v12), `DatabaseQueue`, and the MIT/no-GPL dependency set; sqlite-vec and
...
stale or unavailable evidence cannot jump. Companion cards remain a separate
future artifact contract (D90).
...
EN/ES navigation evidence. The current gate is 731 package tests (13 gated),
zero strict-lint violations across 268 Swift source files, and 30
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 731-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 5 — Evidence and people | In progress — 5A through 5E complete: explicit canonical people, source-revision-fenced overview evidence/navigation, reversible local claim feedback, position-typed decision evidence, and identity-typed action-item evidence/navigation (D86–D90) | Canonical identity preserves ambiguity and biometric separation. Typed overview, decision, and action provenance admits only exact request tags, persists ordered same-meeting links in schema v9/v11/v12, fails closed when stale/unavailable, and remaps through bundles. Schema v10 keeps the current overview assessment separate, active-claim fenced, text-erasing on clear, and explicitly portable. Companion-card evidence remains; 5F characterizes it as a separate artifact |

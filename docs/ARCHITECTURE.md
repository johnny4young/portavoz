...
The current gate is 782 package tests (13 gated), zero strict-lint violations
across 284 Swift source files, and 31
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 750-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 6 — Platform expansion | In progress — 6A, 6B1, and 6B2 complete; dormant Apple transport boundary characterized end to end (D92–D95) | Schema v14 remains the mutation authority; deterministic text-first envelopes replay atomically and private-zone records store encrypted inline/protected-asset tombstones. Separate complete-protected transport state now preserves account-scoped consent/seed, opaque engine/system fields, exact attempts, bounded retries, replay cursors, and fetched deferrals across restart; a thin injected CKSyncEngine delegate owns callbacks but no app runtime creates a container or performs network work. Next: 6C macOS consent/readiness/progress/retry/disable UI and explicit runtime composition; no iOS shell or audio sync yet |

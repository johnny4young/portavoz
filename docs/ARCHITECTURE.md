...
rollback cases. The current gate is 603 package tests (13 gated), zero
strict-lint violations across 230 Swift source
...
- `IntegrationsKit` retains outbound adapters. Meeting-review and Insights
  read policy plus reminder, mirror, and brief policy already live in
  ApplicationKit.
...
provider unavailability or pre-attempt supersession creates no run. Import
summaries, transcript/refine artifacts, and Companion remain later Band 3
producers (D62/D63).
...
13. **Generated success is one atomic fact:** a successful model run and the immutable artifact it produced commit together; durable outputs share the same owner lease and source-revision fence as their processing job. Failed/cancelled attempts are separate terminal records, while cache hits, unavailable providers, and inputs rejected before a model attempt create no synthetic run. Provenance stores identities, fingerprints, configuration, timing, outcome, and aggregate metrics but never meeting content (D62/D63).
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the 603-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 3 — Provenance and privacy | In progress — 3A gives manual/post-refine regeneration typed attempt provenance; 3B makes durable post-capture provenance part of the existing job/artifact fence (D62/D63). Slice 3C adopts external-audio import summaries next | Generation provenance adoption, egress gateway, privacy receipt, typed errors and diagnostics |

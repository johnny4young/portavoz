...
DiarizationKit are admitted only with real vertical use cases. Seventeen
...
and topic behavior. Architecture gates prevent either cluster's source
ownership or consumer imports from drifting outward again. The current gate is
575 package tests (13 gated), zero strict-lint
...
- `IntegrationsKit` retains outbound adapters. Meeting-review and Insights
  read policy already lives in ApplicationKit; reminder, mirror, and brief
  policy moves in the next characterized slice.
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the 575-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |

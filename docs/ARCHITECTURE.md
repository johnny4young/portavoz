...
cases. The current gate is 600 package tests (13 gated), zero
strict-lint violations across 230 Swift
...
post-refine summary regeneration. Durable post-capture/import summaries,
transcript/refine artifacts, and Companion remain later Band 3 producers.
...
13. **Generated success is one atomic fact:** a successful model run and the immutable artifact it produced commit together; failed/cancelled attempts are separate terminal records, while cache hits create no synthetic run. Provenance stores identities, fingerprints, configuration, timing, outcome, and aggregate metrics but never meeting content (D62).
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the 600-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 3 — Provenance and privacy | In progress — 3A gives manual/post-refine regeneration typed attempt provenance and atomically links every newly generated immutable summary/action set; 3B adopts the durable post-capture summary executor next (D62) | Generation provenance adoption, egress gateway, privacy receipt, typed errors and diagnostics |

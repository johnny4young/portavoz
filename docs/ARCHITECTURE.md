...
into the current schema v9), `DatabaseQueue`, and the MIT/no-GPL dependency set; sqlite-vec and
...
| `StorageKit` | `MeetingStore` over GRDB 7 + FTS5, schema v9: the released meeting/transcript/summary/search/trash behavior plus schema-v6 durability, schema-v7 privacy receipts, schema-v8 canonical people, and schema-v9 typed overview claims with ordered nullable segment evidence. New claims are validated and revision-stamped atomically with their immutable summary; summary observation includes claim/link tables while transcript availability resolves against the independent core projection. Exact aliases remain candidate-only and ambiguity-preserving. Recording, recovery, import/export, Refine, Companion, Spotlight, job, provenance, egress, strict-decoding, live-root, and plain-BLOB vector contracts remain as detailed in the matching specs; `.portavoz` strips canonical people but carries and remaps portable claim evidence. Outbox and per-meeting preferences remain unconsumed |
...
localized transcript/audio navigation, and EN/ES visual coverage. The current
gate is 708 package tests (13 gated), zero strict-lint violations across 259
Swift source files, and 27 XCUITest cases passing in both English and Spanish. Throwaway
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 708-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 5 — Evidence and people | In progress — 5A and 5B complete: explicit canonical people plus source-revision-fenced overview evidence and transcript/audio navigation (D86/D87) | Canonical identity preserves ambiguity and biometric separation; typed overview provenance admits only exact request tags, persists ordered same-meeting links in schema v9, fails closed when stale/unavailable, and remaps through bundles. Local exportable correction feedback and broader decision/action/card evidence remain |

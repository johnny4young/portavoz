...
into the current schema v8), `DatabaseQueue`, and the MIT/no-GPL dependency set; sqlite-vec and
...
| `StorageKit` | `MeetingStore` over GRDB 7 + FTS5, schema v8: the released meeting/transcript/summary/search/trash behavior plus the schema-v6 lifecycle, audio-asset, durable-job, generation-run, outbox, and meeting-preference foundations, schema-v7 immutable content-free egress attempts and privacy-coverage boundary, and schema-v8 canonical people/aliases with nullable observed-speaker links. Exact normalized aliases return candidates only; create/link writes are atomic, reject `isMe`, preserve duplicate names across people, and canonicalize only the selected speaker display name. Recording reserves assets and atomically installs the captured meeting/assets/live content plus its initial exact job; recovery installs revalidated assets through a repeat-safe Unit of Work that protects ready meetings; `.portavoz` import validates and atomically installs meeting, cast, transcript, optional summary/actions, notes, and Companion cards while stripping canonical person IDs; export reads the same live content plus the newest cross-recipe summary as one projection and strips those device-local links. Library meeting/voice-mix, open-item, trash, and active-FTS reads expose separate explicit-region GRDB observations. Insights exposes separate live-meeting, participant/commitment, voice-balance, and scope-bounded finding observations. Meeting Detail exposes independent transcript/cast, newest-summary/action-item, Companion, and privacy-receipt observations. Spotlight uses one deterministic SQL snapshot for every live meeting, newest cross-recipe summary, and first 40 live segments. All feature families share query helpers with their one-shot APIs. The typed job queue enforces idempotent enqueue, owner-bound leases, retries, scheduled wake discovery, success/failure/cancellation terminal states, and lifecycle derivation; generated diarization/summary artifacts have stale-safe atomic completion boundaries consumed by the process executor. Manual/post-refine, durable post-capture, and external-audio import successful summaries commit with validated generation runs and linked action items in their owning transactions. Accepted Refine inserts one successful transcript run and links every replacement segment inside the same source-revision-fenced transaction; fresh refined speakers never inherit prior canonical links automatically. Retained live Companion artifacts share the captured Stop transaction; post-Refine artifacts and links replace the prior card snapshot only under the current transcript revision. Failed/cancelled attempts persist separately. MeetingStore also records validated egress attempts before transport and derives an honest complete-or-since receipt projection. Outbox and per-meeting preferences remain unconsumed. Persisted IDs/enums decode strictly, live library projections join the meeting root, and segment vectors remain plain BLOBs |
...
isolation, localization, and explicit EN/ES UI coverage. The current
gate is 699 package tests (13 gated), zero strict-lint violations across 259
Swift source files, and 26 XCUITest cases passing in both English and Spanish. Throwaway
...
|---|---|---|
| 0 — Integrity and truth | Complete — slices 0A/0B: strict decoding, live-meeting aggregate scope, independent language policies; retained by the current 699-test package baseline | Strict identity decoding, live-meeting aggregate scope, explicit transcript/summary language policies |
...
| 5 — Evidence and people | In progress — 5A complete: additive schema v8 canonical people/aliases and one explicit Meeting Detail create/link vertical (D86) | Canonical identity preserves ambiguous aliases, keeps biometrics/evidence separate, strips device-local links from bundles, and requires re-confirmation after Refine; typed claim evidence, navigation, and local feedback remain |

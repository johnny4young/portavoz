...
The shared FluidAudio dependency is pinned to exact **0.15.6** (D516), including
its diarization implementation. This freezes the already resolved engine, not
just the ASR adapter. Model hashes, clustering thresholds and per-operation
speaker/session ownership remain unchanged. A future engine upgrade must qualify
diarization as well as recognition; dictation-only averages cannot establish parity.

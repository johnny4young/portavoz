...
3. Siri phrase invocation remains a real-device field gate. The native
   `StartRecordingIntent`, bilingual App Shortcuts, URL route, post-meeting
   Shortcut hook, and Spotlight indexing are implemented.
...
- **AppIntents/Siri**: `StartRecordingIntent` opens the existing visible
  `portavoz://record` route, and `PortavozShortcuts` publishes English and
  Spanish phrases. The SPM shipping path compiles the SDK-only intents source
  under the shipping module name and runs `appintentsmetadataprocessor` out of
  band; `make-app.sh` fails if the resulting `Metadata.appintents` declares no
  action. XCUITest targets the generated test app explicitly and verifies that
  the production URL enters a visible active recording. Siri voice invocation
  remains field evidence because it depends on the user's system language,
  device state, and speech service.

...
- **Meeting sync envelope codec** (`MeetingSyncEnvelopeCodec`, IntegrationsKit, Band 6B1): deterministic sorted-key JSON with millisecond timestamps around StorageKit's exact-generation text-first envelope. Encoding and SHA-256 are pure and make no network request. The envelope carries every live portable summary/evidence version but no audio, local paths, embeddings, canonical people, generation provenance, jobs, receipts, secrets, or voiceprints. CloudKit record/asset encoding, CKSyncEngine state, account/consent, retries, and UI are not implemented yet (D93).

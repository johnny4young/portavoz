...
- **Correction boundary (D233):** search and Ask omit immutable accepted rows
  touched by an active correction rather than returning stale text. Transcript,
  summary, and open-item protocol reads remain accepted-only in CORR-4; they do
  not claim to expose the composed overlay or its freshness yet. CORR-5 owns
  that interface adoption together with composed export.
...
CORR-4 keeps every document renderer on the immutable accepted transcript and
retained generated artifacts. A corrected meeting therefore does not silently
export the Meeting Detail overlay. Composed Markdown/PDF/subtitle text and
optional correction provenance belong to CORR-5; original evidence remains
available throughout that adoption (D233).

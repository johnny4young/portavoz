...
before opening an older folded card below the viewport or folding a header
above it. Both call sites use the shared `revealVertically(in:)` mechanism:
bounded vertical steps observe actual frame changes and require stable viewport
containment and hittability before clicking. Each Companion step is bounded by
its actual viewport height, not the smaller default stride for dense transcript
rows. Visibility after one wheel event is not accepted as AX hit ownership; the existing bounded helper handles that
separate condition instead of introducing another one-shot reveal. Implicit
offscreen clicks can also issue diagonal scrolls against the wrong surface.
This does not change product scroll position
...
| RecordingAssistLayoutTests / CompanionCardWindowTests | The live assist area's pure policy (D504): the split grows both zones with the window and honors both floors, a too-short window shrinks both zones together rather than starving one, the divider drag clamps at its bounds, optional tabs appear only once they can show something and a vanished tab's selection falls back, the focus slot ranks catch-up and the suggested question, which have no other home, over a directed card, which is also reachable in the Companion tab, the recency window keeps the newest three open and folds the rest without dropping any, a hand-opened card folds the oldest recency-open one, only an answer too long to clamp offers to open fully, and the tab badge never goes negative when cards are dismissed |
...
governance, site, CLI, the reviewed dictation-validation corpus, and
...
TranscriptionKit files still expand to the complete English catalogue. Catalog

...
| SkillsControlCenterTests | Capability-derived catalogue disclosure, durable pause and per-Skill policy, independently typed policy/receipt read failures plus receipt cancellation, bounded Recent/Waiting/Attention/Completed receipts over exact v39 partial-index plans without a temporary sort, future unknown-state attention, malformed identity and policy rejection, one read-consistent receipt audit, strict retry-state replay, unknown failure-category rejection, predecessor/tail integrity, a 256-event materialization ceiling enforced with a 257-row probe, and D341 recovery classification/routing that remains inert for local subjects and verification-only for external outcomes |
| SkillActivityPresentationStateTests | Matching- and mismatched-scope loading, same-scope stale-row suppression, receipt-only unavailability, and mutually exclusive verified empty/receipt presentation |

...
Status: the package inventory contains 2,399 cases (14 environment-gated) + 81
...
Meeting Detail tests. The English and Spanish release gates each cover all 81
...
violation as a failure. The Aug 12 acceptance run is clean across all **662
...
| SkillsControlCenterTests | Capability-derived catalogue disclosure, durable pause and per-Skill policy, bounded Recent/Waiting/Attention/Completed receipts over exact v39 partial-index plans without a temporary sort, future unknown-state attention, malformed identity and policy rejection, one read-consistent receipt audit, strict retry-state replay, unknown failure-category rejection, predecessor/tail integrity, and a 256-event materialization ceiling enforced with a 257-row probe |
| SkillOfferAuthorityTests / SkillExecutionStoreTests | Content-free v40 review authority, bounded reconciliation and newest-first plan, opaque review-UUID dismissal, expired/repeated unavailable outcomes, tombstone-wins stale reconciliation, exact one-shot and reusable-offer claim relationships, claim-time dismissal fencing, and byte-preserving opaque EventKit execution identity |
...
`scripts/ui_test_scope.py` is the executable PR-impact policy for all 81 UI
...
The current inventory is 86 cases per locale (172 bilingual). The Aug 7 cost
sample above predates twenty cases per locale (forty executions) and remains a
timing model, not a claim that the smaller 132-execution inventory is current.

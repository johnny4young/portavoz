...
Status: the package inventory contains 1,654 cases (15 environment-gated) + 59
...
cover all 59 cases and retain app-only
...
**SwiftLint (`.swiftlint.yml`, `strict: true`)**: industry-recommended config (default rules + correctness/clarity opt-ins, industry thresholds: line 120, function-body 60/100, cyclomatic 12/20, type-body 400/600). `swiftlint lint --strict --no-cache` passes with **zero violations across 479 Swift source files**; in CI, any violation breaks the build. Inherent exceptions are suppressed inline with justification (catalog sha256 data lines, CLI arg-parser dispatchers, large stateful composition/view files) — splitting those files remains technical debt.
...
| RestructureMeetingTranscriptTests / MeetingTranscriptContentTests / TranscriptCorrectionCompositionTests / TranscriptCorrectionStorageTests | Exact accepted-snapshot structural commands; explicit adjacent same-speaker/channel/time-monotonic merges; complete split partitions; collision-safe generated and historical part identities; recoverable suppression; restore that releases active ownership without erasing lineage; precomputed 20k-row editor projection; timestamp-aware bidirectional split-source navigation; deterministic typed replace/speaker/split/merge/suppress/restore composition; wrong-meeting, stale, overlapping, branched, malformed, and tombstoned-terminal behavior; strict portable-envelope decoding; idempotent atomic append; source retirement; schema immutability; one journal generation per logical correction; and transactional rollback |
...
The current field-reliability inventory is 1,654 XCTest package cases (15
environment-gated), zero strict-lint violations across 479 Swift
files, a 108-case recording/recovery corpus passing 25 consecutive iterations,
and 59 XCUITest cases per locale. Release evidence requires the package
...
- `make test-meeting-detail-baseline`: verifies the canonical source-derived
  ...
  The contract currently covers 328 interaction signals across 29 source
  files, eleven owners, and all 25 Meeting Detail UI journeys; both missing and

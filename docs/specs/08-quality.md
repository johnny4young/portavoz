...
| SkillActivityPresentationStateTests | Matching- and mismatched-scope loading, same-scope stale-row suppression, receipt-only unavailability, mutually exclusive verified empty/receipt presentation, explicit refresh only for verified empty/receipt states, and the bounded 20-to-50 history-window transition plus scope reset |
...
**D371 bounded Skill-activity history gate.** Settings initially reads at most
20 receipts for the selected lifecycle scope and exposes the identified **Show
more runs** action only when that verified page is full. Activation replaces
the page with one bounded 50-receipt read; changing scope resets the requested

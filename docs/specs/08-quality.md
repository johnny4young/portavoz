...
**SwiftLint 0.65.0 `--strict`** on Linux, and a repository-hygiene/tooling gate
that runs before macOS allocation). Each Swift lane owns one test invocation,
so compilation and tests use the same flags instead of a build/test pair that
can compile twice. `.github/workflows/ui-tests.yml` computes feature-level selectors from

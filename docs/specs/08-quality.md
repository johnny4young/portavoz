...
Status: the package inventory contains 1,640 cases (15 environment-gated) + 58
...
cover all 58 cases and retain app-only
...
**SwiftLint (`.swiftlint.yml`, `strict: true`)**: industry-recommended config (default rules + correctness/clarity opt-ins, industry thresholds: line 120, function-body 60/100, cyclomatic 12/20, type-body 400/600). `swiftlint lint --strict --no-cache` passes with **zero violations across 473 Swift source files**; in CI, any violation breaks the build. Inherent exceptions are suppressed inline with justification (catalog sha256 data lines, CLI arg-parser dispatchers, large stateful composition/view files) — splitting those files remains technical debt.
...
| TranscriptCorrectionCompositionTests / TranscriptCorrectionStorageTests | Deterministic typed replace/speaker/split/merge/suppress/restore composition; accepted/composed lineage; wrong-meeting, stale, overlapping, branched, malformed, and tombstoned-terminal behavior; all-kind normalized round trips; strict portable-envelope decoding; idempotent atomic append and append-only undo; tombstone retention; source retirement; malformed payload detection; schema immutability; one journal generation per logical correction; and transactional rollback |
...
The current field-reliability inventory is 1,640 XCTest package cases (15
environment-gated), zero strict-lint violations across 477 production Swift
files, a 108-case recording/recovery corpus passing 25 consecutive iterations,
and 58 XCUITest cases per locale. Release evidence requires the package

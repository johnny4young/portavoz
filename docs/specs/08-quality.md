...
on GitHub Actions
(`.github/workflows/ci.yml`: explicit macos-26/Xcode 26.6 current-SDK
build/test, explicit macos-15/Xcode 26.3 Sequoia build/test, checksum-pinned
**SwiftLint 0.65.0 `--strict`** on Linux, a separate current-SDK iOS 17
Simulator compile for the four shared targets, and a repository-hygiene/tooling
gate that runs before macOS allocation). Each behavioral Swift lane owns one
...
| StatedPriorityDetectorTests / StatedPriorityUITests | D507 adds the regressions a max-effort review reproduced: negation and demotion on the precedence-verb path, intensifiers between the anchor and its qualifier, a demoting word belonging to the previous sentence, a demoting word past the copula, an interrogative scoped to the anchor's own clause (including a question in an earlier clause, and one whose mark the recognizer dropped), and a subject the speaker corrects away from. D506 adds: a demoting qualifier abstains on both sides of the anchor and in both languages, a promoting one still reads, the wider anchor vocabulary and the precedence verbs, a sentence split across captions read as one, captions never joined across speakers or beyond the join window, and the two regressions measurement found — a bare demonstrative subject, and joining extending a clause that already has its copula. Earlier: The D505 detector: the field shape it was built for, a time-qualified anchor, the subject before the anchor, both Spanish directions, and abstention on questions, negation, conditionals, past tense, a subject that names nothing, a clause that would have to be truncated, and a transcript that never says the word. Restating one priority resolves to one subject key. The journey seeds only the caption text and proves the real detector, the inert offer and the accept path adding the exact subject as an objective |
| RecordingAssistLayoutTests / CompanionCardWindowTests | The live assist area's pure policy (D504): the split grows both zones with the window and honors both floors, a too-short window keeps the assist panel usable, the divider drag clamps at its bounds, optional tabs appear only once they can show something and a vanished tab's selection falls back, the focus slot ranks catch-up over a card addressed to the user over a suggested question, the recency window keeps the newest three open and folds the rest without dropping any, a hand-opened card folds the oldest recency-open one, only an answer too long to clamp offers to open fully, and the tab badge never goes negative when cards are dismissed |
| RecordingAssistUITests | Real-app journey over a seeded live fixture: the adjustable divider exists, the directed question holds the focus slot and survives a tab switch, the newest three cards render open while older ones render as one-line folded rows, opening a folded row folds the oldest recency-open card instead of growing the panel, and only the selected tab occupies the area |
...
`scripts/ui_test_scope.py` is the executable PR-impact policy for all 103 UI
...
committed-range override. Pull requests advance from the newest successful,
first-attempt ancestor that published one exact-SHA `ui-verification-*`
artifact after complete selected functional evidence (or a verified no-UI
selection). The resolver rejects retries, duplicate/expired artifacts,
non-ancestors, and force-pushed history. Missing API/artifact/history proof
falls back to the repository base, and manual integration dispatches still
select the full catalogue.
...
xcresult plus content-free runtime and execution receipts per locale under
ignored `dist/ui-test-results/`. The runtime receipt contains only test
identity, result, duration distribution, build/wall duration, and budget
verdict. The execution receipt binds locale, selector count, xcodebuild exit,
result/runtime-receipt presence, log digest, and a closed classification. The
disposable launch installs selected preferences into an injected volatile
`NSArgumentDomain` before constructing the recording owner, which retains that
same defaults authority. Seeded Apuntador state therefore neither depends on
nor mutates a persistent host preference. The
pair rejects a
scoped result whose case count differs from its selector count, a full result
that differs from the 103-case catalogue, an explicitly selected 103-case run
...
signal. Product assertions, executed-but-incomplete suites, unknown exits, and
malformed evidence remain blocking. Only the exact pre-case
`Timed out while enabling automation mode` signature with no runtime receipt
is a hosted infrastructure advisory; it never publishes a verified-head
artifact, so the next push accumulates the missing diff from the last actual
proof. A rerun is diagnostic only and cannot satisfy the gate or publish an
anchor.
...
journey. The 103-case catalogue is therefore smaller while behavior coverage is
...
The first D431 Release-app run completed every frozen event and reliability
scenario with precision, recall, and abstention accuracy 1.0, zero false
...
shared-build complete bilingual UI gate. English passed 103/103 in 847.719
summed seconds (p95 17.851) and Spanish passed 103/103 in 854.580 seconds (p95
...
bounded heap. The two UI receipts must name the exact current 103-case budget
catalog,
...
Notification Center alerts plus active Xcode test commands and UI runners by
...
**D438 IOS-READY portability gate.** The exact candidate compiled
`PortavozCore`, `StorageKit`, `ApplicationKit`, and `IntegrationsKit`
sequentially against the installed iPhone Simulator 26.5 SDK for
`arm64-apple-ios17.0-simulator`; this is a real compiler boundary, not a mock
or a claim that an iOS app exists. The complete package command passed with a
2,903-case inventory, 15 explicit environment/model skips, and zero failures
in 128.216 summed XCTest seconds. The first exact-candidate run correctly
rejected an architecture assertion that still pinned the historical
2,880-package/103-UI inventory; the repaired ratchet requires independently
enumerated 2,903/104 truth. The current-SDK warnings-as-errors build passed,
strict SwiftLint reported zero violations across 774 Swift files, and
repository hygiene passed unchanged outside the outer sandbox that prohibited
the deterministic loopback fixture from binding. The five focused tooling
modules passed 115 cases, including the portability workflow and exact
changed-file ownership policies.

The ownership map selected 21 real-app canaries instead of expanding these
known lower-layer changes to all 104 UI journeys, while unknown production,
localization, copy, and shared-harness changes remain fail-safe expansions.
One 28-second build was reused sequentially: English passed 21/21 in 134.363
summed XCTest seconds (p95 12.999), and Spanish passed 21/21 in 135.361 seconds
(p95 13.144). Both unchanged budgets passed with zero retry and zero harness
adjustment. The explicit host override ignored only Notification Center
alerts; every other quiet-host blocker remained enforced. No product
presentation changed, so this band required neither a Developer-ID Dev-app
reinstall nor a user-facing changelog entry. Final review then tightened exact
success replay without changing presentation; one 16-second build fed the
minimum safe two-journey Background Work delta. English passed 2/2 in 13.984
seconds (p95 7.614), and Spanish passed 2/2 in 13.088 seconds (p95 6.753), again
with zero retry or harness adjustment. The dormant replica-merge and deferred-
Mac-work contracts plus four shared-target compiles are not an iOS app target,
runtime/device behavior, signed CloudKit capability, capture, two-device
convergence, physical iPhone/iPad/Watch evidence, notarization, distribution,
or field validation.
...
The initial D409 policy scoped synchronization from the immediately previous PR
head instead of replaying the whole accumulated branch diff. D429 replaces that
unproven shortcut with the newest first-attempt exact-SHA verification artifact
inside current ancestry; missing, expired, duplicate, failed, retried, or
force-pushed proof falls back to the target branch. Shared/localization/unknown
paths retain their fail-safe expansion, and manual integration dispatch remains
full. This
changes feature-band cost, not the release contract: a fresh complete bilingual
103-case run and its aggregate/per-case budgets remain mandatory before final
...
The final classifier accepts only exact content-free runtime and execution
receipt shapes with finite timings, matching locale/selector identity, and a
result for every executed case. Missing or malformed evidence, a selected
skipped/cancelled locale, any non-passing case, an executed-but-incomplete
suite, an unknown exit, and unknown infrastructure/harness uncertainty are
blocking and classified explicitly. The sole exception is the exact pre-case
automation-service timeout with no runtime receipt: hosted CI reports it as an
infrastructure advisory, marks the head unverified, and publishes no
incremental anchor. GitHub-hosted wall-clock budget drift is advisory only when
every functional case passed. The same unchanged budget file remains enforced
...
ancestry. API uncertainty, duplicate artifacts, retries, failed evidence,
expired retention, and force-push divergence expand from the PR base. A
diagnostic rerun can never satisfy the final gate or publish an anchor.
...
**D440 cancellation-shaped dependency failure.** The standing supervisor has a
focused regression in which the event resolver throws `CancellationError` while
the explicit caller remains active. Reconciliation must throw, retain its full
pending scope, and later complete the same durable owner. The disposable
real-app source can inject the same one-shot condition only with temporary
storage: the Settings journey must show the ambiguous-mutation error, keep the
stale Create control disabled, clear the error only after a verified reload,
and then expose the durable rule through its enabled control. The fixture never
reads EventKit, TCC, private meetings, credentials, or the network.

The focused supervisor and architecture selection passed 10/10 in 1.158
seconds, and the UI catalogue/runtime/CI policy tooling passed 122/122. The
first complete package run then failed one cross-document architecture ratchet:
the quality spec had advanced to 2,906 package + 105 UI cases while the ratchet
still required the historical 2,903 + 104 text. This deterministic contract
failure was corrected rather than retried away; its exact test passed and the
next complete run passed all 2,906 package tests with 15 explicit asset-gated
skips and zero failures in 130.790 seconds. Warnings-as-errors, strict
cache-free SwiftLint over 774 files, repository hygiene, and diff-check also
pass. The fail-safe selector chose all 25 Skills Settings journeys plus the
one Menu Bar brief journey because the changed UI-test contract owns the whole
class. One 20-second build then passed 26/26 English cases in 326.885 summed
seconds (p95 30.657, maximum 40.804) and 26/26 Spanish cases in 331.613 seconds
(p95 31.023, maximum 40.415), with zero retries, zero harness adjustments, and
every unchanged hard budget green. The signed Dev app was reinstalled and

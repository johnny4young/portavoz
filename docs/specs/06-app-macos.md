...
DB (`MeetingStore`) + lazy shared engines: `transcriber` (Parakeet), `diarizer` (with voiceprint if exists; `invalidateDiarizer()` after enroll/delete), `whisper` (lazy, first time downloads verified 1.6 GB with progress). `modelsState` drives UI downloads. Parakeet+diarizer preparation is deduplicated through one process task: callers join the same verified download/load, while recording samples only an already-resident transcriber and starts audio immediately. If it is absent, model preparation begins after capture and Stop hands the finalized channels to the durable transcription worker. `libraryVersion` remains only as a compatibility trigger for Spotlight and mutation paths not yet extracted. Library, Insights, and Meeting Detail no longer consume it for reads: their feature models receive storage-independent updates from query-scoped Store observations.

...
### Capture-safe staged whole-library backup (D180–D184)
...
The persisted reservation describes the move/manifest crash window, but the
app does not yet reopen the staged SQLite source or persist its keyset cursor.
After process termination, launch cleanup still discards an abandoned stage
and its matching journal rather than adopting it. Full relaunch continuation

...
Surface validated by MacParakeet: global hotkey → speak → hotkey again → text written where cursor is. `GlobalHotkey` (Carbon `RegisterEventHotKey` — the only API consuming keystroke WITHOUT Accessibility permission; registered from App init, not view, to survive without window), `DictationController` in AppServices (mic → Parakeet streaming with custom vocabulary → `CaptionCoalescer` reused with echo/noise hygiene; nothing persisted: no meeting, no DB, no file), `DictationPanel` (same non-activating pattern as HUD, bottom-center, live text, X cancels), `TextInserter` (paste-and-restore hardened Jul 2026: full multi-type pasteboard snapshot — rich content survives a dictation; synthetic ⌘V with a layout-aware keycode via UCKeyTranslate — Dvorak-QWERTY-⌘ and non-Latin layouts resolve correctly; the paste waits for the physical hotkey modifiers to be released, bounded at 1 s, so the chord cannot combine into a foreign shortcut; restore runs 1.5 s later — slow Electron/browser consumers service ⌘V late — and only when `changeCount` still matches our write, so a clipboard manager is never clobbered; the ⌘V DOES require Accessibility — checked BEFORE recording with system prompt to avoid dictating into void; delivery checks the focused AX element and refuses to type into `AXSecureTextField` — spoken text never becomes a plaintext secret in a password field, with a localized visible refusal instead of a silent drop). Toggle in Settings (off by default); `DictationAssembler` (TranscriptionKit, pure, tested) joins confirmed+partial and gates delivery on lexical content — bare-punctuation noise artifacts never paste. Capture edges (Jul 2026): the mic keeps recording 250 ms past the finish gesture so the last word's tail survives; a finish under 0.75 s is an accidental tap and cancels silently (with the tail that is still under ~1 s of audio). A pre-transcription VAD gate is deliberately absent: dictation streams through Parakeet live — silence produces no segments — so the batch-Whisper hallucination class this gate exists for elsewhere cannot occur here. Verified E2E: hotkey triggers with app in background and panel transcribes real live audio; final insertion verified in field.

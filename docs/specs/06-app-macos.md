...
### Capture-safe whole-library backup admission (D180)
...
before the D99 source snapshot, destination inspection, Markdown rendering, or
publication. Protected capture returns a typed suspended outcome rather than
an error.
...
This is deliberately an admission checkpoint, not a claim of durable
mid-export pause. Once the D99 coherent snapshot has been read, the current
export finishes even if capture begins. Pausing then would either retain the
complete content-heavy library in memory or reread aggregates from different
database moments. A future intra-export checkpoint therefore needs bounded
durable staging, explicit destination recovery, and cleanup semantics.

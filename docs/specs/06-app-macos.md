...
The stage is removed on completion or ordinary failure. This is process-local
mid-export pause, not relaunch durability. Each current-format stage holds an
exclusive kernel lease. Process launch performs one utility-priority scan,
serialized with stage creation by a root lock, and removes only a stage whose
...
content-free keyset cursor in optional format-v1 metadata. The cursor mutation
...
before any next source read without moving the destination file again. The
cursor stops advancing after the first source, render, or publication failure
because those typed outcomes are still process-local.
...
remain blocked; this includes old journals and new publications after an earlier
failure froze cursor advancement. A checkpoint-only retry uses immutable
completed evidence and does not reacquire, rehash, or republish the destination.

The app does not yet invoke reconciliation or staged-source adoption at launch.
Launch cleanup still discards an abandoned stage and its matching journal
rather than adopting it. Durable typed failure/render reconstruction also
remains absent, so full relaunch continuation remains unclaimed.

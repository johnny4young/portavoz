...
navigation controls reachable. The no-selection workspace offers explicit
Record/Return and Ask actions through the same route owner, with adaptive
horizontal/vertical layout and opt-in transfer copy. It does not start work
until an action is chosen. Meeting selection uses native sidebar styling;
voice-mix cues and all existing route/control identifiers remain intact.
Settings category buttons share readable type and restrained selected tint;
their accessible name is the category, their subtitle is a hint, and selected
state is semantic rather than color-only. The Settings search has a stable ID.
Summary-engine help describes the provider-neutral live checkpoint on Sequoia
and Tahoe and optional refinement by the selected local engine. Apple readiness
copy promises on-device processing, not a specific compute unit or an absence
of operating-system asset preparation.
...
Apuntador now uses D431's bundled bilingual detector on Sequoia and Tahoe;
Foundation Models is an optional challenger/answer lane rather than its
availability gate.
...
### Background activity center (D427)

`AppServices` constructs one `BackgroundWorkCenterModel` immediately after the
authoritative store opens and before composing the five publishers it observes:
interrupted-recording recovery, durable post-capture processing, Spotlight,
semantic indexing, and memory-graph projection. It is a process-lifetime
`@MainActor @Observable` presentation projection, not a sixth worker. It owns no
`Task`, timer, persistence, polling loop, scheduler, model runtime, or store
query. Each real owner keeps its existing coalescing, lease, wake, and replay
authority and emits typed events through injected closures.

Per-owner monotonic tokens fence an older processing or maintenance completion
after a newer run begins. The projection admits only closed phase, stage,
outcome, failure-category and aggregate-metric values plus attempt and retry
timestamps. Meeting IDs, titles, transcripts, notes, paths, model data, owner
strings, and raw errors never enter it. Post-capture's quarter-lease heartbeat
...
The real-app fixture is content-free and activates only when both
`-use-temp-store` and `-seed-background-work` are present. A separate
`-enable-background-work-fixture` flag enables the real semantic and graph
supervisors against disposable storage for the recording-exclusion journey; it
does not install sample status in a production graph.
...
D431 changes the operation injected into that existing owner, not the owner's
bound. `RecordingController` always composes
`ProviderNeutralProvenanceCompanion`; the mandatory bundled classifier admits
questions on Sequoia and Tahoe, while the app-owned Foundation Models
capability enables only the optional challenger. This same injection makes the
temporary-store `-simulate-sequoia-capabilities` path disable the challenger
on a Tahoe runner instead of accidentally exercising installed host assets.
The real-app fixture adds `-simulate-live-apuntador`, emits one frozen localized
remote question through the normal caption callback, and relies on the actual
compiled classifier. The fixture never supplies a card or a classification.
Settings presents `settings-apuntador-enabled` on Sequoia, distinguishes ready
offline detection from Tahoe answer enhancement, and never implies that the
separate post-meeting refresh lost its macOS-26 requirement.
...
`TextInserter` implements the fail-closed delivery boundary. It waits up to one second for all physical modifiers to lift and refuses delivery rather than posting a combined shortcut when they remain held or cancellation arrives. It then inspects the focused Accessibility element immediately before touching the clipboard: `AXSecureTextField`, lost trust, missing role, malformed values, and transient inspection errors all block insertion with localized feedback; an explicitly absent/unsupported subrole on an otherwise valid ordinary text control remains admissible. Only after that check does it attempt an all-or-nothing bounded snapshot of ordered clipboard items, write the dictation, recheck security and ownership, and post a complete layout-aware ⌘V pair. Borrowed Text Input Source properties are promoted while their owning source remains alive, must match their declared Core Foundation runtime type, and must contain a complete keyboard-layout header; a missing, wrong-typed, or truncated value falls back to the standard QWERTY shortcut instead of reaching typed Carbon translation. Clipboard-write or event-construction failure restores immediately. Successful delivery restores captured representations after 1.5 seconds only if `changeCount` still identifies Portavoz's write, preserving rich content without overwriting a clipboard manager.

**Clipboard loans (D546).** `DictationClipboard` owns one exclusive loan per
pasteboard in the app process. Another insertion waits cancellably for the prior
1.5-second restoration window; synchronous admission rechecks exclusivity after
modifier waiting. Otherwise a delayed editor Paste could read the next dictation,
even if eventual restoration preserved the original. Loan identity and AppKit
`changeCount`, never string equality, authorize restoration. A foreign write is
not overwritten. No restoration emits another event.

`PasteboardSnapshot` captures every ordered item and every advertised supported
representation, or refuses without changing the board. All item/type metadata is
admitted before any payload read; unsupported/private formats, missing bytes,
changed ownership or exceeded limits refuse insertion. Inclusive bounds are 16
items, 64 representations, 4 MiB per representation and 8 MiB retained overall.
Supported rich types remain opaque bytes; no file dereferencing or keyed-object
decoding occurs. These bounds limit retained bytes, not a lazy provider's allocation
or synchronous response time. Empty boards are complete restorable snapshots.
Cancellation during a provider read refuses before borrowing. Security and loan
ownership are checked again after capture and before dispatch. The addressed
process must still be available at that same boundary; a process disappearing
while a lazy provider serves bytes restores without posting to session focus.

Temporary dictated text carries `TransientType`, `ConcealedType` and
`AutoGeneratedType` pasteboard markers; borrow/restore writes use
`.currentHostOnly`. Those markers advise cooperating readers, not an access-control
boundary. Prior sharing provenance cannot be read and is not re-enabled during
restoration. A refusal explains that the clipboard was preserved and asks the user
to copy different content before trying again. No new clipboard preference,
history, external destination acknowledgement or recovery action is introduced.
Restoration remains memory-only and best-effort across app/pasteboard-server loss.
...
`-simulate-live-transcript-browsing`, `-simulate-live-apuntador`, and

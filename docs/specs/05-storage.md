...
identity.
D385 adds schema v43's `globalDataEgressEvent`, a separate content-free
authority for cross-library operations that cannot honestly name one meeting.
It admits only `ask-answer-generation` to a provable local-device destination,
classified as `meeting-answer-material`, selected through
`summary-engine-settings`, with a nonempty provider and model. It deliberately
has no `meetingID`, payload, URL, path, question, transcript, prompt, or answer
column. Meeting-owned receipts remain in the v7 table and keep their foreign
key. A missing identity for any other operation still fails closed.
D386 adds no schema. `MeetingStore.search` accepts an optional exact meeting
identity and binds it independently inside the accepted, corrected, and
structural FTS subqueries before their rank limits. The Library query and its
BM25 ordering remain unchanged. Behavioral coverage seeds all three serving
lanes plus a foreign accepted hit and proves that neither scope can publish the
other meeting.
D387 advances `StorageSchema.version` to 44. The migration rebuilds
`globalDataEgressEvent` without changing its content-free columns: prior local
Ask rows keep their required model, while a direct Web attempt requires
`web-source-retrieval`, `public-web-source-request`, `explicit-web-ask`, a nil
model, and a scope matching its host. Both local fixture and remote public hosts
are allowed; URL path/query, question, page, prompt, citation, answer, and
meeting identity have no column. The store validates the same two closed
contracts before insert, and a v43 Ask row is preserved through the rebuild.
The existing local `ask-answer-generation` operation also admits the distinct
`public-web-answer-material` classification for bounded excerpts sent to the
selected loopback model; it never labels that public material as meeting data.
Reads revalidate finite time, host/provider identity, derived scope, and the
operation matrix so corrupt content-free evidence fails closed.
D389 advances `StorageSchema.version` to 45. `contextItemSearch` is a
synchronized external-content FTS5 projection over the existing context-item
`content` column, and the migration rebuilds all preexisting rows without
rewriting source records. Standard insert/update/delete triggers keep the
projection current. Product `searchNotes` joins the source row and owning
meeting, admits only live `kind = note` rows from live meetings, and orders by
FTS rank, meeting-relative offset, then note identity under an explicit limit.
It returns exact raw content and UUIDs, derives `authoredAt` from meeting start
plus the durable offset, and rejects corrupt identity, negative/non-finite
offset, or blank content. Other context kinds may exist in the shared index but
are structurally excluded at the serving query; `enhancedNote` is a separate
table with no path into the projection.
D435 advances `StorageSchema.version` to 46. `standingSkillRule` is a
device-local, content-free authority table keyed by UUID. It stores the exact
Skill ID/version, one checked trigger, one checked subject predicate, one
checked action, a daily ceiling from one through eight, checked enabled state,
and creation/update times. It has no meeting/event identity, title, attendee,
transcript, provider, destination, credential, prompt, or result column. The
trigger/predicate/action tuple is unique, reads are capped at 32, duplicate
creation returns the existing authority without mutation, and strict decoding
rejects malformed identities, unknown enums, invalid bounds, non-finite dates,
or reversed time. Rules are intentionally absent from sync aggregates and
CloudKit envelopes. This slice writes no execution state/event; deleting a rule
therefore cannot erase any future immutable execution receipt.
D436 advances `StorageSchema.version` to 47. One append-once
`standingSkillExecutionAuthority` row binds an existing standing rule and the
normal Skill execution owner to one exact event occurrence. The occurrence is
identified by a SHA-256 fingerprint over the opaque event ID plus exact start
instant; a unique action/fingerprint pair prevents duplicate observer delivery
and automatic recursion. The same transaction re-reads the exact rule,
global/per-Skill policy, one-shot dismissal, existing manual event ownership,
per-rule single flight, and the explicit local-day window before inserting the
normal confirmed execution and consuming one daily slot. The authority is not
deleted with the rule, is bounded to 32 pending reads, and never syncs or
exports.

`standingSkillArtifact` stores one immutable, format-versioned local payload
per successful standing owner. Storage admits only a non-empty payload through
128 KiB with a matching 64-character SHA-256 digest and finite creation time.
Artifact insertion and the typed `begin`/`succeed` transitions commit in one
transaction; typed automatic failure commits `begin`/`fail` together. Only an
exact `confirmed` first attempt or a recoverable `failed` owner below the
three-attempt ceiling can advance. A persisted `executing` owner is
outcome-unknown and never re-executed. Storage treats the payload as opaque;
ApplicationKit alone validates and decodes the pre-meeting brief.
D437 advances `StorageSchema.version` to 48. GRDB persists `Date` values as UTC
text with millisecond precision, so standing occurrence fingerprints now hash
the opaque event ID plus the rounded reference-date millisecond rather than a
transient floating-point bit pattern. The migration derives that identity from
the preserved calendar subject and stored start, then atomically rekeys both
the authority fingerprint and normal Skill idempotency key through unique
proposal-scoped staging values. Reads continue to reject mismatched persisted
identity; ApplicationKit compares provider events at the same durable temporal
precision while retaining exact snapshot checks for all other fields.
D455 advances `StorageSchema.version` to 49. The migration adds one partial
index over `segment(meetingID, startTime, id)` for rows whose `deletedAt` is
null. This matches the core Meeting Detail predicate and its stable transcript
order, so SQLite does not build a temporary ordering B-tree for every initial
observation and transcript refresh. The index contains no content beyond the
existing keys, excludes tombstones, changes no sync/export surface, and leaves
the correction and refined-transcript projection unchanged. Migration tests
pin both the partial predicate and the no-temporary-sort query plan.
D374 adds no schema. Its optional inclusive absolute `updatedAt` lower bound is
composed with the existing lifecycle and exact-Skill predicates before ordering
and limiting. Time-only reads retain the v35/v39 indexes; combined Skill/time
reads retain the v42 indexes. Storage owns no wall clock, rejects a non-finite
cutoff, and query-plan tests require the selected index without a temporary
sort.
D375 adds no schema or count query. ApplicationKit asks the existing indexed
receipt query for exactly one row beyond its 20- or 50-row visible window, so
StorageKit still materializes one bounded result with a maximum limit of 51.
ApplicationKit consumes that final row only as continuation evidence and never
publishes it as a receipt.
...
| `contextItemSearch` (v45) | FTS5 external-content projection over `contextItem.content`, synchronized by insert/update/delete triggers; product note search joins the live source and owner, structurally admits only raw `kind = note` rows, and never indexes `enhancedNote` (D389) |
...
| `globalDataEgressEvent` (v43/v44) | immutable content-free attempt for local whole-library Ask generation or one consented direct-Web source request; checked operation/classification/consent/provider/model/scope matrix, with no meeting, URL path/query, question, page, prompt, citation, or answer column (D385/D387) |
...
| `skillExecutionState` (v31/v41) | one idempotency-keyed current projection per proposal with the latest event, attempt, monotonic updatedAt, and current typed failure category (added/backfilled in v41); triggers require the category exactly when state is `failed`. Unknown future states fail closed as possibly executed rather than retryable and remain visible in the v39 attention scope (D293/D336/D341). D338 changes no identity: the claim resolves an exact existing owner first, then validates its separate offer identity and durable dismissal before writing a new projection |
| `skillExecutionSubject` (v41) | exact content-free subject captured atomically for each newly confirmed execution: one checked meeting, commitment, or bounded opaque calendar identity keyed by proposal UUID. Meeting/commitment FKs cascade only this recovery authority while the receipt survives; legacy rows are deliberately not backfilled or inferred from idempotency/offer keys (D341) |
...
| `standingSkillRule` (v46) | device-local content-free UUID authority with exact Skill ID/version, closed trigger/predicate/action tuple, daily ceiling 1...8, enabled state, and timestamps; unique action tuple, bounded to 32 rows, strict fail-closed decoding, no sync/export payload, and no execution history ownership (D435) |
| `standingSkillExecutionAuthority` (v47/v48) | append-once exact-occurrence authority keyed by the normal Skill proposal owner, with rule snapshot identity (deliberately no rule FK), closed action, unique action/SHA-256 occurrence fingerprint over opaque event identity plus the persisted millisecond start, explicit bounded local-day window, and authorization time; preserves historical duplicate and daily-budget evidence after rule deletion, with no title, attendee, transcript, prompt, result, sync, or export payload (D436/D437) |
| `standingSkillArtifact` (v47) | one immutable local format-versioned artifact per standing proposal, capped at 128 KiB and protected by a persisted SHA-256 digest; insertion and successful Skill settlement are atomic, while ApplicationKit owns decoding (D436) |
| `skillExecutionState_on_recent` (v35) | `(updatedAt DESC, proposalID ASC)` index for the bounded newest-first Skills receipt projection; avoids a temporary full-history sort as execution history grows and serves D374 time-only ranges without a schema change (D317/D374) |
...
| `skillExecutionState_on_recent_skill` (v42) | complete `(skillID, updatedAt DESC, proposalID ASC)` index for exact-Skill newest-first review and combined exact-Skill/update-time ranges (D373/D374) |
| `skillExecutionState_on_waiting_skill` / `_attention_skill` / `_completed_skill` (v42) | partial `(skillID, updatedAt DESC, proposalID ASC)` indexes with the exact v39 lifecycle predicates; filtered and combined Skill/time queries pin the matching index and retain the same bounded order without a temporary sort (D373/D374). Each receipt gains one complete plus one state-scoped filtered entry in addition to its v35/v39 entries, an explicit write/disk tradeoff guarded by migration and query-plan tests rather than an inferred latency claim |
...
`MeetingStore.confirmedTopics(matching:limit:)` is a read-only selector input,
not a factual graph query. It accepts only 1...50 rows. An empty search returns
live unmerged topics in stable label/creation/identity order. A normalized text
search matches active `topicAlias` rows, escapes every SQL `LIKE` metacharacter,
and uses a recursive SQLite CTE to follow each matched merged child to its
current live root. Roots are de-duplicated and limited before Swift hydration;
deleted topics, deleted aliases, unavailable chains, and cycles produce no
candidate rather than an inferred identity.

StorageKit and ApplicationKit independently limit query text to 120 characters;
ApplicationKit exposes the read behind `ConfirmedTopicCatalogReading`. The
returned label is presentation metadata only: full Ask must submit the selected
exact `TopicID` to the existing `LoadDecisionHistory` boundary. This adds no
schema, migration, write path, FTS table, graph engine, sync/export field, or
model dependency.

...
`libraryMarkdownBackupSnapshots()` is the dedicated whole-library open-format
projection (D99). One `DatabaseQueue.read` selects every live meeting root
newest-first, then loads each strict cast/transcript aggregate in that same
SQLite snapshot. A corrupt required aggregate becomes a content-free
`MeetingMarkdownBackupReadFailure` and does not block healthy meetings. The
latest live General-recipe summary preserves the released Settings export
contract; optional summary decode failure degrades to no summary. StorageKit
returns values only—it does not render Markdown, inspect a destination folder,
or publish files.
...
coverage boundary, durable job, generation run, egress event, and content-free
sync acknowledgement inside one SQLite snapshot, then groups rows in memory to avoid one query per meeting. The
...
same allowlist again while encoding the public report and scopes an otherwise
all-local status to `all-tracked-processing-stayed-on-device` whenever the
separate sync disclosure records an acknowledged copy (D76/D115).

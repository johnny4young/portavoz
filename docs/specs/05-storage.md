...
### Schema (`v1`–`v46` migrations registered in `Sources/StorageKit/Schema.swift`)
...
| `standingSkillExecutionAuthority` (v47) | append-once exact-occurrence authority keyed by the normal Skill proposal owner, with rule snapshot identity (deliberately no rule FK), closed action, unique action/SHA-256 occurrence fingerprint, event start, explicit bounded local-day window, and authorization time; preserves historical duplicate and daily-budget evidence after rule deletion, with no title, attendee, transcript, prompt, result, sync, or export payload (D436) |

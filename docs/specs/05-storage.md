...
The whole projection is scale-gated (D312): at 10k meetings every longitudinal job answers between 2.3 and 76.1 ms p95 against the 250 ms interactive budget, recursive-CTE family/chain traversal stays under 6 ms, and the graph-engine decision gate stays closed. Full rebuild-from-zero is superlinear (17.6 min at 10k, driver: per-scope `liveTopicRecords`) — a GAPS throughput item, tolerable because the job is checkpointed and capture-yielding. Evidence: `docs/evidence/meeting-memory-graph-scale-20260807.json`; harness: `MeetingMemoryGraphScaleTests` (structural invariants always on, canonical runs behind `PORTAVOZ_GRAPH_SCALE_BENCHMARK=1`).

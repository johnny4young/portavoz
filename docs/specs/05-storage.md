...
D176/D177 change no StorageKit contract: `NULL` embedding rows remain the
durable retry ledger while ApplicationKit coalesces redundant Library/Ask
backfill flights and pauses between committed batches before they reach these
exact read/write methods. Process termination or policy suspension therefore
needs no in-memory cursor repair or vector rollback.

...
## Motor de resúmenes plural (D25/M12) — Apple FM · Ollama local · BYOK nube

`AppServices.summaryEngine` (UserDefaults `summaryEngine`: `appleOnDevice` default / `ollama`) + `configuredSummaryProvider()` → nil = Apple FM (map-reduce + scheduler + caché/pivote); Ollama = `OllamaService.summaryProvider(model:)` (un `OpenAICompatibleSummaryProvider` contra `localhost:11434/v1`, **sin API key** — Ollama la ignora, nada sale del equipo). `AppServices.summarize(_:)` centraliza: provider configurado, o FM si macOS 26, o throw. Cablea el resumen final (grabación + import) y regenerar; el **resumen rodante en vivo sigue siendo FM-only** (usa las APIs incrementales `condenseWindow`/`summarizeNotes` que Ollama no tiene). `OllamaService`: `isRunning()` (GET `/api/version`), `models()` (GET `/api/tags`, `parseModels` puro/testeado). Ajustes → "Motor de resúmenes": picker + detección + lista de modelos. **Cierra GAPS #7** (Mac sin Apple Intelligence resume 100% local); verificado E2E con gpt-oss:20b (resumen ES en 24 s). Para no-Apple el caché por fingerprint aplica igual, pero el pivote de traducción es FM-only.

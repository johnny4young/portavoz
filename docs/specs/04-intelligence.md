...
## Source-backed person commitments (D281)
...
`LoadPersonCommitments` is an injected ApplicationKit port. Ask does not yet
compose it, and canonical display-name resolution—including the ambiguous-
person abstention—remains a separate Application boundary. Canonical corpus
mapping, answer synthesis, cross-lane ranking, private evidence, and relational
scale budgets remain open.

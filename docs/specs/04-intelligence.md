...
**MLX embebido (D32, jul 2026)**: tercer motor `summaryEngine = "mlx"` — `MLXSummaryProvider` (IntelligenceKit) corre **Qwen3-4B-Instruct-2507 4-bit** (Apache-2.0, sha256-pineado en `ModelCatalog.mlxQwen3`, 2.3 GB) in-process en la GPU vía `mlx-swift-examples` (exact 2.29.1). Reusa el prompt y el contrato JSON de `OpenAICompatibleSummaryProvider.prompt/parseStructured` — mismo `StructuredSummary`, mismo fingerprint. `MLXModelCache` (actor) mantiene UN `ModelContainer` cargado y serializa la generación (`container.perform`, temperatura 0); no pasa por `IntelligenceScheduler` (GPU, no ANE). Ajustes → "Built-in (MLX)": fila `MLXModelRow` con descarga verificada/estado/eliminar (`AppServices.mlxDownloaded/downloadMLX/deleteMLXModel`); `HardwareRecommender` lo sugiere con RAM ≥ 8 GB sin Apple Intelligence ni Ollama. **Shipping**: SwiftPM no compila shaders Metal → `scripts/build-mlx-metallib.sh` cachea `mlx-swift_Cmlx.bundle` (xcodebuild one-time, keyed por versión de mlx-swift) y `make-app.sh` lo copia a `Contents/Resources`. **Verificación E2E**: `portavoz-app --mlx-smoke` (reunión sintética ES → resumen estructurado correcto, 3 s con la app instalada en un M-series); no hay test bajo `swift test` porque el runner CLI no puede tener metallib.

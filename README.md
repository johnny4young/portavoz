...
- **Live captions, lyrics-style** — sub-second partials on the Neural Engine; the newest line reads big and your voice glows amber. Matching speaker bleed is collapsed into the stronger direct-system row. Scroll up and history stays sharp and still until you choose **Jump to live**. Optional **live translation** routes each completed turn by its own language instead of repeatedly asking the framework to guess a source; an unsupported pair stays original without blocking later supported turns.

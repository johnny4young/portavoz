...
| `StorageKit` | GRDB/SQLite schema v19, FTS5 search, additive canonical people/aliases, typed immutable transcript-correction history, source-revision-fenced overview, decision, action-item, and role-separated Apuntador evidence with separate reversible overview feedback, a content-free generation-fenced per-meeting mutation journal plus exact-generation text-first aggregate projection/atomic remote replay (the separate CloudKit adapter cannot redefine these rules), scoped Library/Insights/Meeting Detail observations, one-read Spotlight and whole-library backup projections, versioned snapshots, atomic recovered/accepted transcripts, summary and Apuntador-card provenance, immutable content-free egress attempts and receipt-coverage boundary, atomic support-safe snapshots, durable leased meeting jobs plus independent derived-maintenance lease/retry ownership, and a compatibility-fenced local vector index |

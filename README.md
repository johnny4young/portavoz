...
- **Commitments become true only when you say so** — evidence-linked action items appear in a separate confirmation inbox. Jump to the exact source, edit the wording, assign it explicitly to yourself, choose an exact confirmed person, or leave it unassigned, add a deadline only when you know it, then confirm, defer, or dismiss. Commitment Radar keeps the confirmed work together and can enable generic local due-date reminders only after you explicitly grant permission; tapping an alert returns to Radar, while launch never prompts or polls. Generated output never becomes longitudinal user truth automatically.

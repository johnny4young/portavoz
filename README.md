...
| `StorageKit` | GRDB/SQLite schema v10, FTS5 search, additive canonical people/aliases, typed source-revision-fenced summary evidence with separate reversible claim feedback, scoped Library/Insights/Meeting Detail observations, a one-read Spotlight projection, versioned snapshots, atomic recovered/accepted transcripts, summary and Companion-card provenance, immutable content-free egress attempts and receipt-coverage boundary, atomic support-safe snapshots, durable leased job queue with bounded manual retry, local vector index |

...
- **Live captions, lyrics-style** — sub-second partials on the Neural Engine; the newest line reads big and your voice glows amber. Matching speaker bleed is collapsed into the stronger direct-system row. Scroll up and history stays sharp and still until you choose **Jump to live**. Optional **live translation** routes each completed turn by its own language instead of repeatedly asking the framework to guess a source.
...
- **Summaries you can inspect and correct** — an overview can link to the exact transcript/audio moments that support it, and a generated to-do names an owner only when that identity matches the meeting cast. Add a private correction or mark a claim unsupported without rewriting the generated text; clear erases the correction, and it travels only when you explicitly export the `.portavoz` meeting.

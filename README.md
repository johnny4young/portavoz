...
- **Commitments become true only when you say so** — evidence-linked action items appear in a separate confirmation inbox. Jump to the exact source, edit the wording, choose only a confirmed person or leave it unassigned, add a deadline only when you know it, then confirm, defer, or dismiss. Generated output never becomes longitudinal user truth automatically.
...
| `StorageKit` | GRDB/SQLite schema v21, FTS5 search, additive canonical people/aliases, typed immutable transcript-correction history, confirmed-only commitment continuity with exact evidence and append-only lifecycle events, source-bound dismiss/defer review feedback that never copies generated candidates, source-revision-fenced overview, decision, action-item, and role-separated Apuntador evidence with separate reversible overview feedback, a content-free generation-fenced per-meeting mutation journal plus exact-generation text-first aggregate projection/atomic remote replay (the separate CloudKit adapter cannot redefine these rules), scoped Library/Insights/Meeting Detail observations, one-read Spotlight and whole-library backup projections, versioned snapshots, atomic recovered/accepted transcripts, summary and Apuntador-card provenance, immutable content-free egress attempts and receipt-coverage boundary, atomic support-safe snapshots, durable leased meeting jobs plus independent derived-maintenance lease/retry ownership, and a compatibility-fenced local vector index |

...
- **🗑️ Safer permanent deletion** — stale trash entries cannot erase a restored meeting, and restoring then deleting again renews its trash grace period.
...
- **🛟 Remote audio that cannot disappear silently** — If the call channel stops delivering audio while your microphone keeps recording, Portavoz now warns you immediately, rebuilds the system tap without ending the recording, retries while the outage continues, and confirms when remote capture returns. A terminal capture failure stays distinct from a call-ended nudge and clearly asks you to restart recording.
...
- **⚡ Long meetings open without the wait** — Meeting health now skips transcript history that provably cannot overlap while preserving older long interruptions. A 5,000-segment meeting reaches first content in 92 ms instead of 522 ms, and the measured startup hang is gone.
